...
function buildPromotionOfferText(raffle) {
  ...
  return [
    ...
    'Если розыгрыш уже активный — бот опубликует его в General-канале сразу после оплаты. Если розыгрыш запланирован — публикация в General-канале включится при запуске.',
    '',
...
async function applyGeneralChannelPromoPayment(payment, row) {
  ...
  if (raffle.status === 'active') {
    ...
    await sendMessage(
      userId,
      [
        '✅ **Оплата прошла. Розыгрыш опубликован в нашем канале розыгрышей.**',
        '',
    ...
    await sendMessage(
      ...
      [
        ...
        '',
        `Розыгрыш № **${getRafflePublicNumber(raffle)}** будет опубликован в General-канале при запуске розыгрыша.`
      ].join('\n')
    ).catch(() => {});
  }
}
...
function buildLegalAcceptanceKeyboard() {
  ...
  if (LEGAL_PRIVACY_URL) {
    keyboard.push([{ text: 'Политика конфиденциальности', url: LEGAL_PRIVACY_URL }]);
  }

  if (LEGAL_OFFER_URL) {
    keyboard.push([{ text: 'Публичная оферта', url: LEGAL_OFFER_URL }]);
  }

  if (LEGAL_PERSONAL_DATA_URL) {
    keyboard.push([{ text: 'Обработка персональных данных', url: LEGAL_PERSONAL_DATA_URL }]);
  }

  keyboard.push([{ text: '✅ Принять и продолжить', callback_data: 'legal_accept' }]);
...
function buildLegalAcceptanceText() {
  return [
    ...
    '',
    'Бот использует ваш MAX ID, имя профиля, действия в розыгрышах, сведения о созданных розыгрышах, каналах и платежах только для работы сервиса: создания розыгрышей, проверки участия, публикации постов, статистики, антифлуда, уведомлений и оплаты услуг.',
    '',
...
async function createRaffleAndSendCreatedMessage(target, userId, sessionData) {
  ...
  await sendPromotionOffer(target, userId, createdRaffle);
...
async function sendRaffleStats(target, raffleId, requesterUserId = null) {
  ...
  if (!channels.length) {
    ...
    const channelStatsMap = await getChannelPeriodStatsMap(
