...
function setupApostModule(options) {
  ...
  function isApostStartCommand(text) {
    const clean = String(text || '').trim().toLowerCase();
    return clean === '/apost' || clean === 'автопост';
  }

  function shouldReleaseApostSession(text) {
    const clean = String(text || '').trim();
    const lower = clean.toLowerCase();

    if (!clean) return false;
    if (isApostStartCommand(clean)) return false;

    // Любая другая slash-команда не должна застревать внутри сценария /apost.
    // Сбрасываем apost-сессию и отдаём команду основному bot.js.
  ...
  async function handleMessage(message) {
    ...
    if (shouldReleaseApostSession(text)) {
      await clearApostSession(userId);
      return false;
    }
