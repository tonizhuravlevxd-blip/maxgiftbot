...
function setupApostModule(options) {
  ...
  const MINIAPP_BOT_URL = String(process.env.APOST_MINIAPP_BOT_URL || BOT_PUBLIC_URL || '').replace(/\/+$/, '');
  const USE_MAX_STARTAPP = String(process.env.APOST_USE_MAX_STARTAPP || 'true').toLowerCase() !== 'false';
  ...
  function extractPhotoAttachmentFromMessage(message = {}) {
    const photo = extractAttachmentsFromMessage(message).find(isImageLikeAttachment);
  ...
  function normalizeInlineKeyboard(keyboard) {
    ...
    const rows = keyboard
      .map(row => Array.isArray(row) ? row.filter(button => {
        if (!button || typeof button !== 'object') return false;
        const text = String(button.text || '').trim();
        const hasAction = Boolean(button.url || button.callback_data || button.payload || button.data);
        return Boolean(text && hasAction);
      }) : [])
  ...
  function normalizeButtonUrl(value) {
    ...
    return normalizeHttpLink(withScheme);
  }

  function customButtonsToKeyboard(buttons = []) {
    if (!Array.isArray(buttons)) return null;

    const rows = buttons
      .slice(0, 2)
      .map(button => {
        const text = String(button?.text || '').trim().slice(0, 40);
        const url = normalizeButtonUrl(button?.url || '');
        return text && url ? [{ text, url }] : null;
      })
      .filter(Boolean);

    return normalizeInlineKeyboard(rows);
  }

  function parseAutopostButtonInput(text) {
    const clean = String(text || '').trim();
    ...
    title = String(title || '').replace(/\s+/g, ' ').trim().slice(0, 40);
  ...
  function getPostAttachments(data = {}) {
    if (data.photo_attachment && typeof data.photo_attachment === 'object') {
      return [data.photo_attachment];
    }

    const image = buildImageAttachment(data.image_url);
    return image ? [image] : [];
  }


  async function sendMessageWithImageFallback(target, text, keyboard = null, attachments = [], fallbackImageUrl = '') {
    const safeAttachments = Array.isArray(attachments) ? attachments.filter(Boolean) : [];
    const safeKeyboard = normalizeInlineKeyboard(keyboard);

    try {
      return await sendMessage(target, text, safeKeyboard, safeAttachments);
    } catch (error) {
      const message = String(error?.message || '');
      const canFallback = safeAttachments.length && /Failed to upload image|upload image|image/i.test(message);

      if (!canFallback) throw error;

      const imageUrl = normalizeHttpLink(fallbackImageUrl || '');
      const fallbackText = [
        text,
        '',
        '⚠️ MAX не смог загрузить фото автоматически.',
        imageUrl ? `Фото: ${imageUrl}` : ''
      ].filter(Boolean).join('\n');

      return sendMessage(target, fallbackText, safeKeyboard, []);
    }
  }

  function webResultUrl(key) {
    const base = RESULT_BASE_URL || APP_BASE_URL;
    return `${base}/apost/test/${encodeURIComponent(key)}`;
  }

  function buildStartAppParam(key) {
    return `apost_result_${String(key || 'trust').replace(/[^a-zA-Z0-9_-]/g, '') || 'trust'}`;
  }

  function testResultUrl(key) {
    const maxBotUrl = String(MINIAPP_BOT_URL || '').trim();

    if (USE_MAX_STARTAPP && /^https:\/\/max\.ru\//i.test(maxBotUrl)) {
      const separator = maxBotUrl.includes('?') ? '&' : '?';
      return `${maxBotUrl}${separator}startapp=${encodeURIComponent(buildStartAppParam(key))}`;
    }

    return webResultUrl(key);
  }
  ...
  async function sendApostMenu(target) {
    return sendMessage(target, [
      ...
      'Выберите раздел:',
      '',
      '📝 **Автопост** — создать свой пост: фото + текст Markdown, предпросмотр, время и канал.',
  ...
  async function startAutopost(target, userId) {
    ...
    return sendMessage(target, [
      '📝 **Новый автопост**',
      '',
      'Отправьте **фото с телефона** одним сообщением, как при создании розыгрыша.',
      '',
      'Можно сразу добавить подпись к фото — она станет текстом поста.',
      'Если отправите только фото, бот следующим сообщением попросит текст поста.',
      '',
      'Текст поддерживает Markdown.'
    ].join('\n'), [[{ text: '⬅️ Назад', callback_data: 'apost_menu' }]]);
  }

  function parsePostInput(text) {
    const raw = String(text || '').trim();
    const parts = raw.split('|');
    if (parts.length < 2) return null;

    const imageUrl = normalizeHttpLink(parts.shift().trim());
    const postText = parts.join('|').trim();

    if (!imageUrl || !postText) return null;

    return {
      image_url: imageUrl,
      text: safeText(postText, 3500)
    };
  }
  ...
  async function sendPostPreview(target, userId, data, type = 'post') {
    ...
    return sendMessageWithImageFallback(target, previewText, keyboard, attachments, data.image_url);
  }

  async function sendAutopostButtonsMenu(target, userId, data = {}) {
    const buttons = Array.isArray(data.custom_buttons) ? data.custom_buttons.slice(0, 2) : [];
    const lines = buttons.length
      ? buttons.map((button, index) => `${index + 1}. ${button.text} — ${button.url}`)
      : ['Пока кнопок нет. Можно пропустить этот шаг.'];

    const keyboard = [];

    if (buttons.length < 2) {
      keyboard.push([{ text: '➕ Добавить кнопку', callback_data: 'apost_button_add' }]);
    }

    buttons.forEach((button, index) => {
      keyboard.push([{ text: `🗑 Удалить: ${String(button.text || '').slice(0, 24)}`, callback_data: `apost_button_del:${index}` }]);
    });
    ...
    await setApostSession(userId, 'apost_buttons', {
      ...data,
    ...
    return sendMessage(target, [
      ...
      '',
      'Можно добавить до **2 кнопок** со ссылками.',
      '',
      '**Текущие кнопки:**',
      ...lines,
      '',
      ...
      '`Кнопка: Название`',
      '`Ссылка: https://site.ru`',
      '',
      'Если ссылка без `https://`, бот добавит его сам.'
    ].join('\n'), keyboard);
  }
  ...
  async function sendFinalConfirm(target, userId, data) {
    ...
    const customButtons = Array.isArray(data.custom_buttons) ? data.custom_buttons.slice(0, 2) : [];
    const buttonLines = customButtons.length
      ? ['','**Кнопки:**', ...customButtons.map((button, i) => `${i + 1}. ${button.text} — ${button.url}`)]
      : [];
  ...
  async function createJobs(userId, data) {
    ...
    const buttons = data.type === 'test' ? buildTestButtons() : customButtonsToKeyboard(data.custom_buttons) || [];
    const values = [];
    const params = [];
    let p = 1;
  ...
  async function handleCallback(cb) {
    ...
    if (data === 'apost_button_add') {
      const session = await getApostSession(userId);
      if (!session || session.state !== 'apost_buttons') {
        await sendMessage(target, 'Сначала подтвердите предпросмотр автопоста.');
        return true;
      }

      const buttons = Array.isArray(session.data.custom_buttons) ? session.data.custom_buttons : [];
      if (buttons.length >= 2) {
    ...
    if (data.startsWith('apost_button_del:')) {
      const session = await getApostSession(userId);
      if (!session || session.state !== 'apost_buttons') {
        await sendMessage(target, 'Удаление кнопки сейчас не активно.');
        return true;
      }
      ...
      const next = { ...session.data, custom_buttons: buttons };
  ...
  async function handleMessage(message) {
    ...
    if (session.state === 'apost_wait_photo') {
      ...
      if (!photoAttachment) {
        await sendMessage(target, [
          '📷 Пришлите именно **фото** с телефона.',
          '',
          'Можно отправить фото с подписью — подпись станет текстом поста.'
        ].join('\n'), [[{ text: '⬅️ Назад', callback_data: 'apost_menu' }]]);
        return true;
      }
      ...
      ].join('\n'), [[{ text: '⬅️ Назад', callback_data: 'apost_autopost' }]]);
      return true;
    }

    if (session.state === 'apost_wait_text') {
      if (!text) {
        await sendMessage(target, 'Отправьте текст поста одним сообщением.');
        return true;
      }

      const data = {
        ...session.data,
        text: safeText(text, 3500)
      };
      await setApostSession(userId, 'apost_preview', data);
      await sendPostPreview(target, userId, data, 'post');
      return true;
    ...
    if (session.state === 'apost_wait_post') {
      const parsed = parsePostInput(text);
      if (!parsed) {
        await sendMessage(target, [
          'Старый формат больше не нужен.',
          '',
          'Нажмите **Автопост** и отправьте фото с телефона, потом текст поста.'
        ].join('\n'), [[{ text: '📝 Автопост', callback_data: 'apost_autopost' }]]);
        return true;
      }

      const data = { ...parsed, type: 'post', title: 'Автопост' };
      await setApostSession(userId, 'apost_preview', data);
      await sendPostPreview(target, userId, data, 'post');
      return true;
    }
    ...
    if (session.state === 'apost_wait_button') {
      const button = parseAutopostButtonInput(text);
      if (!button) {
        await sendMessage(target, [
          'Не понял кнопку. Отправьте так:',
          '',
          '`Кнопка: Купить`',
          '`Ссылка: gdgdgdg.ru`',
          '',
      ...
      const next = {
        ...session.data,
        custom_buttons: buttons.slice(0, 2)
      };
  ...
  async function publishJob(row) {
    const rawButtons = Array.isArray(row.buttons_json)
      ? row.buttons_json
      : (typeof row.buttons_json === 'string' ? JSON.parse(row.buttons_json || '[]') : row.buttons_json || []);
    const buttons = normalizeInlineKeyboard(rawButtons);
    const storedPhoto = row.photo_attachment && typeof row.photo_attachment === 'object'
      ? row.photo_attachment
      : (typeof row.photo_attachment === 'string' ? JSON.parse(row.photo_attachment || 'null') : null);
    const attachments = storedPhoto ? [storedPhoto] : getPostAttachments({ image_url: row.image_url });

    try {
      await sendMessageWithImageFallback(row.channel_id, row.text, buttons, attachments, row.image_url);
      await pool.query(`
        UPDATE apost_jobs
  ...
  function resolveResultKey(value) {
    const clean = String(value || '').trim();
    const fromStartApp = clean.match(/^apost_result_([a-zA-Z0-9_-]+)$/);
    const key = fromStartApp ? fromStartApp[1] : clean;
    return TEST_RESULTS[key] ? key : 'trust';
  }

  function renderResultPage(keyFromServer = 'trust') {
    const initialKey = resolveResultKey(keyFromServer);
    const resultsJson = JSON.stringify(TEST_RESULTS).replace(/</g, '\\u003c');
    const moreUrl = MORE_TESTS_URL || '';
    const moreLabel = MORE_TESTS_LABEL || 'тут';
...
<head>
  ...
  <style>
    :root{color-scheme:light dark;--bg:#f4f6fb;--card:#ffffff;--text:#111827;--muted:#667085;--accent:#6d5dfc;--accent2:#ff8a65;--shadow:rgba(20,25,40,.12);}
    @media (prefers-color-scheme: dark){:root{--bg:#0f1117;--card:#181b24;--text:#f7f7fb;--muted:#a9adbb;--shadow:rgba(0,0,0,.35)}}
    *{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif;background:radial-gradient(circle at 20% 0%,rgba(109,93,252,.22),transparent 32%),linear-gradient(180deg,var(--bg),var(--bg));color:var(--text);min-height:100vh;}
    .page{max-width:720px;margin:0 auto;padding:18px 14px 32px;}
    .top{display:flex;justify-content:center;margin:4px 0 14px;}
    .more{display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border-radius:999px;background:rgba(109,93,252,.12);font-size:15px;color:var(--text);text-decoration:none;font-weight:700;}
    .hero{position:relative;overflow:hidden;background:linear-gradient(135deg,var(--accent),var(--accent2));border-radius:28px;padding:24px 20px;color:white;box-shadow:0 18px 50px var(--shadow);}
    .hero:after{content:"";position:absolute;right:-40px;top:-40px;width:150px;height:150px;background:rgba(255,255,255,.18);border-radius:50%;}
    .badge{display:inline-flex;padding:8px 12px;border-radius:999px;background:rgba(255,255,255,.22);backdrop-filter:blur(8px);font-size:14px;font-weight:700;margin-bottom:14px;}
    h1{font-size:30px;line-height:1.12;margin:0 0 12px;letter-spacing:-.03em;}
    .subtitle{font-size:16px;line-height:1.45;margin:0;opacity:.92;}
    .card{margin-top:14px;background:var(--card);border-radius:26px;padding:22px 18px;box-shadow:0 12px 34px var(--shadow);}
    .result-text{font-size:18px;line-height:1.65;margin:0;white-space:pre-wrap;}
    .actions{display:grid;gap:10px;margin-top:16px;}
    .btn{border:0;border-radius:16px;padding:14px 16px;font-size:16px;font-weight:800;cursor:pointer;background:var(--accent);color:white;box-shadow:0 8px 20px rgba(109,93,252,.28);}
    .btn.secondary{background:rgba(109,93,252,.12);color:var(--text);box-shadow:none;}
    .meta{font-size:13px;color:var(--muted);text-align:center;margin-top:14px;}
  </style>
</head>
<body>
  <main class="page">
    <div class="top" id="moreWrap"></div>
    <section class="hero">
      <div class="badge">✨ Результат теста</div>
      <h1 id="title">Результат теста</h1>
      <p class="subtitle">Развлекательный психологический результат. Сохраните себе или пройдите ещё один тест.</p>
    </section>
    <section class="card">
      <p class="result-text" id="text"></p>
      <div class="actions">
        <button class="btn" id="moreBtn">Еще больше тестов</button>
        <button class="btn secondary" id="shareBtn">Поделиться результатом</button>
      </div>
      <div class="meta" id="platform"></div>
    </section>
  </main>
  <script>
    const RESULTS = ${resultsJson};
    const INITIAL_KEY = ${JSON.stringify(initialKey)};
    const MORE_URL = ${JSON.stringify(moreUrl)};
    const MORE_LABEL = ${JSON.stringify(moreLabel)};

    function getResultKey(){
      try{
        const webApp = window.WebApp;
        const startParam = webApp && webApp.initDataUnsafe && webApp.initDataUnsafe.start_param;
        const m = String(startParam || '').match(/^apost_result_([a-zA-Z0-9_-]+)$/);
        if(m && RESULTS[m[1]]) return m[1];
      }catch(e){}
      return RESULTS[INITIAL_KEY] ? INITIAL_KEY : 'trust';
    }

    function openMore(){
      if(!MORE_URL) return;
      try{
        if(window.WebApp && /^https:\/\/max\.ru\//i.test(MORE_URL)) return window.WebApp.openMaxLink(MORE_URL);
        if(window.WebApp && window.WebApp.openLink) return window.WebApp.openLink(MORE_URL);
      }catch(e){}
      location.href = MORE_URL;
    }

    function shareResult(result){
      const text = 'Мой результат: ' + result.title + '\n\n' + result.text;
      try{
        if(window.WebApp && window.WebApp.shareMaxContent){
          window.WebApp.shareMaxContent({ text }).catch(function(){});
          return;
        }
      }catch(e){}
    ...
    const result = RESULTS[key] || RESULTS.trust;
    document.getElementById('title').textContent = result.title;
    document.getElementById('text').textContent = result.text;
    document.getElementById('moreBtn').textContent = MORE_LABEL ? 'Еще больше тестов ' + MORE_LABEL : 'Еще больше тестов';
    document.getElementById('moreBtn').onclick = openMore;
    document.getElementById('shareBtn').onclick = function(){ shareResult(result); };
    if(MORE_URL){
      const a = document.createElement('button');
      a.className = 'more';
      a.type = 'button';
      a.textContent = 'Еще больше тестов ' + MORE_LABEL;
      a.onclick = openMore;
      document.getElementById('moreWrap').appendChild(a);
    }
    try{
      if(window.WebApp){
        window.WebApp.BackButton && window.WebApp.BackButton.show && window.WebApp.BackButton.show();
        window.WebApp.BackButton && window.WebApp.BackButton.onClick && window.WebApp.BackButton.onClick(function(){ history.length > 1 ? history.back() : window.close(); });
        document.getElementById('platform').textContent = 'Открыто в MAX mini app' + (window.WebApp.platform ? ' · ' + window.WebApp.platform : '');
      }
    }catch(e){}
  </script>
</body>
  ...
  app.get('/apost/mini', (req, res) => {
    res.status(200).type('html').send(renderResultPage(String(req.query.result || req.query.startapp || 'trust')));
  });
