...
function setupApostModule(options) {
  ...
  async function sendMessageWithImageFallback(target, text, inlineKeyboard = null, data = {}, extraLines = []) {
    ...
    if (!attachment) {
      return sendMessage(target, text, inlineKeyboard);
    }

    try {
      return await sendMessage(target, text, inlineKeyboard, [attachment]);
    } catch (error) {
      ...
      return sendMessage(target, fallbackText, inlineKeyboard);
    }
  }
  ...
  async function publishJob(row) {
    ...
    try {
      await sendMessageWithImageFallback(row.channel_id, row.text, buttons, row, [
        'Кнопки теста ниже работают, результат можно открыть без фото.'
      ]);
      await pool.query(`
        UPDATE apost_jobs
