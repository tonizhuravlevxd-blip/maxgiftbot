...
function formatDateTime(value) {
  ...
  return parsed.isValid() ? parsed.format('DD.MM.YYYY HH:mm') : 'не указано';
}
...
function getMinPublishAt() {
  return dayjs().add(MIN_RAFFLE_PUBLISH_DELAY_MINUTES, 'minute');
}

function getMinEndAtForPublish(publishAt) {
  const base = dayjs(publishAt);
  const safeBase = base.isValid() ? base : getMinPublishAt();
...
function applyQuickOffset(base, offset) {
  ...
  return dayjs(base).add(amount, parsed.unit);
}
...
async function sendPublishDatePrompt(target, data = {}) {
  ...
  return sendMessage(
    ...
    [
      ...
      `Минимальное время публикации: **${MIN_RAFFLE_PUBLISH_DELAY_MINUTES} минут** от текущего времени.`,
      `Ближайшее допустимое время: **${minPublishAt.format('DD.MM.YYYY HH:mm')}**.`,
      '',
      'Можно нажать быструю кнопку или ввести дату вручную:',
      '`2026-06-10 20:00`',
      '`10.06.2026 20:00`'
    ].join('\n'),
...
async function sendEndDatePrompt(target, data = {}) {
  ...
  return sendMessage(
    ...
    [
      ...
      `Ближайшее допустимое окончание: **${minEndAt.format('DD.MM.YYYY HH:mm')}**.`,
      '',
      'Можно нажать быструю кнопку или ввести дату вручную:',
      '`2026-06-10 20:00`',
      '`10.06.2026 20:00`'
...
function buildRaffleDraftPreviewText(data = {}) {
  ...
  return [
    ...
    '',
    '🤝 Соадмина можно пригласить сразу после создания: бот выдаст ссылку для совместного розыгрыша.'
  ].join('\n');
}
...
async function sendRaffleDraftPreview(target, userId, data = {}) {
  ...
  return sendMessage(
    ...
    [
      [{ text: '✅ Создать розыгрыш', callback_data: 'raffle_confirm_create' }],
      [{ text: '🤝 Создать и пригласить соадмина', callback_data: 'raffle_confirm_create' }],
      [
...
function buildCreatedRaffleMessage(sessionData, raffle, inviteLink) {
  return [
    ...
    inviteLink,
    '',
    'Отправьте её другому администратору. Он откроет бота, добавит бота администратором в свой канал и выберет: публиковать пост в его канале или нет, делать подписку обязательной или нет.'
  ].join('\n');
}
...
async function createRaffleAndSendCreatedMessage(target, userId, sessionData) {
  ...
  await sendMessage(
    target,
    buildCreatedRaffleMessage(sessionData, raffle, inviteLink),
    [
...
async function handleSessionMessage(message) {
  ...
  if (state === 'await_publish_date' || state === 'edit_publish_date') {
    ...
    if (parsed.isBefore(minPublishAt)) {
      await sendMessage(
        target,
        `Публикация должна быть минимум через ${MIN_RAFFLE_PUBLISH_DELAY_MINUTES} минут. Ближайшее допустимое время: **${minPublishAt.format('DD.MM.YYYY HH:mm')}**.`
      );
      return true;
    }
    ...
    if (state === 'edit_publish_date') {
      ...
      if (!data.end_at || dayjs(data.end_at).isBefore(minEndAt)) {
        data.end_at = null;
  ...
  if (state === 'await_end_date' || state === 'edit_end_date') {
    ...
    if (parsed.isBefore(minEndAt)) {
      await sendMessage(
        target,
        `Окончание должно быть минимум через ${MIN_RAFFLE_DURATION_AFTER_PUBLISH_MINUTES} минут после публикации. Ближайшее допустимое время: **${minEndAt.format('DD.MM.YYYY HH:mm')}**.`
      );
      return true;
    }
...
async function sendUserRaffles(target, userId) {
  ...
  for (const r of raffles) {
    text += `#${r.id} | ${r.title} | ${r.status} | публикация ${formatDateTime(getRafflePublishAt(r))} | до ${formatDateTime(r.end_at)}\n\n`;
...
async function handleCallbackQuery(cb) {
  ...
  if (data.startsWith('quick_publish:')) {
    ...
    const publishAt = applyQuickOffset(dayjs(), offset);
    const minPublishAt = getMinPublishAt();
    ...
    if (session.state === 'edit_publish_date') {
      ...
      if (!sessionData.end_at || dayjs(sessionData.end_at).isBefore(minEndAt)) {
  ...
  if (data === 'raffle_confirm_create') {
    ...
    const minPublishAt = getMinPublishAt();
    const publishAt = dayjs(sessionData.publish_at);

    if (!publishAt.isValid() || publishAt.isBefore(minPublishAt)) {
    ...
    const minEndAt = getMinEndAtForPublish(sessionData.publish_at);
    const endAt = dayjs(sessionData.end_at);

    if (!endAt.isValid() || endAt.isBefore(minEndAt)) {
      await setSession(userId, 'await_end_date', sessionData);
