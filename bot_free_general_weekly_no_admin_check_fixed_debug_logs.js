...
async function getQualifyingOrganizerChannelResult(raffle, options = {}) {
  ...
  for (const channel of res.rows) {
    ...
    if (refresh) {
      const botCanPublish = await checkBotCanPublishInChannel(channel.channel_id, info);
      if (!botCanPublish) {
        lastReason = 'bot_cannot_publish';
        continue;
      }
    }
...
async function validateFreeGeneralClaimForPublish(claim, raffle) {
  ...
  const botCanPublish = await checkBotCanPublishInChannel(channelId, info);
  if (!botCanPublish) {
    return { ok: false, temporary: true, reason: 'bot_cannot_publish' };
  }
