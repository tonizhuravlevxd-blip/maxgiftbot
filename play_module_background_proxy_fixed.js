...
function setupPlayModule(options = {}) {
  ...
  const PLAY_BACKGROUND_MAX_BYTES = Math.max(
    1024 * 1024,
    Number(process.env.PLAY_BACKGROUND_MAX_BYTES || 8 * 1024 * 1024)
  );
  let playBackgroundCache = null;

  console.log(
    playBackgroundImageUrl
      ? '🎮 PLAY_BACKGROUND_IMAGE_URL настроен: фон будет загружаться через /play/background'
      : '🎮 PLAY_BACKGROUND_IMAGE_URL не задан: используется рисованный фон игры'
  );
  ...
  (function () {
    ...
    const customBackgroundUrl = ${JSON.stringify(playBackgroundImageUrl ? '/play/background' : '')};
    const customBackgroundImage = new Image();
    let customBackgroundReady = false;
    let customBackgroundFailed = false;

    if (customBackgroundUrl) {
      customBackgroundImage.onload = function () {
        customBackgroundReady = true;
        customBackgroundFailed = false;
        console.log('PLAY background loaded');
      };
      customBackgroundImage.onerror = function () {
        customBackgroundReady = false;
        customBackgroundFailed = true;
        console.warn('PLAY background failed to load. Check /play/background-check');
      };
      customBackgroundImage.src = customBackgroundUrl + '?v=' + Date.now();
    }
    ...
    function drawEgyptBackground() {
      if (customBackgroundReady) {
        ...
        ctx.fillStyle = 'rgba(245, 158, 11, .045)';
  ...
  async function fetchPlayBackgroundImage(options = {}) {
    if (!playBackgroundImageUrl) {
      return {
        ok: false,
        status: 404,
        error: 'PLAY_BACKGROUND_IMAGE_URL не задан'
      };
    }

    const now = Date.now();
    if (!options.force && playBackgroundCache && playBackgroundCache.url === playBackgroundImageUrl && now - playBackgroundCache.loadedAt < 5 * 60_000) {
      return {
        ok: true,
        status: 200,
        contentType: playBackgroundCache.contentType,
        bytes: playBackgroundCache.buffer.length,
        buffer: playBackgroundCache.buffer,
        cached: true
      };
    }
    ...
    try {
      const response = await fetch(playBackgroundImageUrl, {
        method: 'GET',
        signal: controller.signal,
        headers: {
          'User-Agent': 'RaffleBotPlayBackground/1.0'
        }
      });
      ...
      if (!response.ok) {
        return {
          ...
          contentType,
          error: `Сервер картинки ответил ${response.status}`
        };
      }

      if (contentLength > PLAY_BACKGROUND_MAX_BYTES) {
        return {
          ok: false,
          status: 413,
          contentType,
          bytes: contentLength,
          error: `Картинка слишком большая: ${contentLength} байт`
        };
      }
      ...
      if (contentType && !contentType.startsWith('image/')) {
        return {
          ...
          status: 415,
          contentType,
          error: `Ссылка ведёт не на картинку, а на ${contentType}`
        };
      }
      ...
      if (!buffer.length) {
        return {
          ...
          status: 422,
          contentType,
          error: 'Картинка пустая'
        };
      }

      if (buffer.length > PLAY_BACKGROUND_MAX_BYTES) {
        return {
          ok: false,
          status: 413,
          contentType,
          bytes: buffer.length,
          error: `Картинка слишком большая: ${buffer.length} байт`
        };
      }

      playBackgroundCache = {
        url: playBackgroundImageUrl,
      ...
      return {
        ...
        error: error?.name === 'AbortError'
          ...
          : String(error?.message || error || 'Не удалось загрузить картинку')
      };
    } finally {
  ...
  app.get('/play/background-check', async (req, res) => {
    res.set('Cache-Control', 'no-store');

    const result = await fetchPlayBackgroundImage({ force: true });
    res.status(result.ok ? 200 : 200).json({
      ok: result.ok,
      configured: Boolean(playBackgroundImageUrl),
  ...
  app.get('/play/background', async (req, res) => {
    const result = await fetchPlayBackgroundImage();

    if (!result.ok || !result.buffer) {
      res.set('Cache-Control', 'no-store');
      return res.status(result.status || 404).type('text/plain').send(result.error || 'Background not available');
    }

    res.set('Cache-Control', 'public, max-age=300');
    res.type(result.contentType || 'image/jpeg').send(result.buffer);
  });
