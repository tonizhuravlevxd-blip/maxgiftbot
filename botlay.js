...
<body>
  <main class="wrap">
    <section class="card">
      ...
      <canvas id="game" width="820" height="430" aria-label="Игра: мультяшная собака перепрыгивает препятствия"></canvas>
      <button class="button" id="jumpBtn">Прыгнуть / Начать заново</button>
      <p class="hint">Нажмите кнопку, экран или пробел. Смелый пёс мчится мимо зловещего замка, а после 600 очков попадает на фестиваль еды. Скорость постепенно растёт.</p>
    </section>
  </main>
...
(function () {
  ...
  function getScene() {
    return score >= 600 ? 'festival' : 'haunted';
  }
  ...
  function drawHauntedBackground() {
    ...
    sky.addColorStop(0, '#0f1731');
    sky.addColorStop(.5, '#23345f');
    sky.addColorStop(1, '#536ea8');
    ctx.fillStyle = sky;
    ...
    drawCastle();
    drawScaredGuy(410, 256, ghostBob);
    drawGhost(458, 198, ghostBob + 0.5);
  ...
  function draw() {
    ...
    if (scene === 'festival') {
      ...
      ctx.fillText('После 600 очков открывается фестиваль еды!', 456, 32);
    }
    ...
    if (gameOver) {
      ...
      ctx.fillText('Игра окончена', canvas.width / 2, 172);
      ctx.font = '700 24px Arial';
      ctx.fillText('Ваш счёт: ' + Math.floor(score) + '  •  Рекорд: ' + best, canvas.width / 2, 215);
      ctx.font = '600 18px Arial';
      ctx.fillText('Нажмите, чтобы начать заново', canvas.width / 2, 255);
      ctx.textAlign = 'left';
    }
  }
