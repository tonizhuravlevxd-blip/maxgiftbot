...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    canvas { width: 100%; height: auto; display: block; border-radius: 18px; background: linear-gradient(#12203c 0%, #31487b 55%, #7696d5 100%); touch-action: manipulation; }
...
(function () {
  ...
  const groundY = 410;
  const dog = { x: 110, y: groundY - 88, w: 120, h: 88, vy: 0, onGround: true, frame: 0 };
  const gravity = 0.88;
  ...
  function getFestivalTransition() {
    if (score <= 560) return 0;
    if (score >= 720) return 1;
    const raw = (score - 560) / 160;
    return raw * raw * (3 - 2 * raw);
  }

  function getScene() {
    return getFestivalTransition() >= 0.55 ? 'festival' : 'haunted';
  }

  function addObstacle() {
    const scene = getScene();
    const h = scene === 'festival' ? 30 + Math.random() * 42 : 34 + Math.random() * 42;
    const w = scene === 'festival' ? 26 + Math.random() * 28 : 24 + Math.random() * 18;
  ...
  function drawMoon(x, y, r) {
    ...
    ctx.beginPath(); ctx.arc(x + 12, y - 6, r, 0, Math.PI * 2); ctx.fill();
  }

  function drawCloud(x, y, dark) {
    ctx.fillStyle = dark ? 'rgba(181, 193, 231, .26)' : 'rgba(255,255,255,.72)';
    ctx.beginPath();
  ...
  function drawCastle() {
    ...
    ctx.fillRect(baseX, baseY, 160, 95);
    ctx.fillRect(baseX + 18, baseY - 52, 26, 52);
    ctx.fillRect(baseX + 62, baseY - 70, 34, 70);
    ctx.fillRect(baseX + 116, baseY - 58, 26, 58);
    ctx.beginPath();
    ctx.moveTo(baseX + 18, baseY - 52); ctx.lineTo(baseX + 31, baseY - 78); ctx.lineTo(baseX + 44, baseY - 52);
    ctx.moveTo(baseX + 62, baseY - 70); ctx.lineTo(baseX + 79, baseY - 104); ctx.lineTo(baseX + 96, baseY - 70);
    ctx.moveTo(baseX + 116, baseY - 58); ctx.lineTo(baseX + 129, baseY - 83); ctx.lineTo(baseX + 142, baseY - 58);
    ctx.fill();
    ctx.fillStyle = '#f8c046';
    ctx.fillRect(baseX + 18, baseY + 18, 12, 18);
    ctx.fillRect(baseX + 72, baseY + 10, 12, 20);
    ctx.fillRect(baseX + 124, baseY + 22, 12, 18);
    ctx.fillStyle = '#0d1323';
    roundRect(baseX + 64, baseY + 40, 32, 55, 12); ctx.fill();
  }

  function drawGhost(x, y, bob) {
    ctx.save();
    ctx.translate(x, y + Math.sin(bob) * 8);
    ctx.fillStyle = 'rgba(235,245,255,.92)';
    ctx.beginPath();
    ctx.arc(0, -16, 18, Math.PI, 0);
    ctx.lineTo(18, 18);
    ctx.quadraticCurveTo(11, 8, 4, 18);
    ctx.quadraticCurveTo(0, 10, -5, 18);
    ctx.quadraticCurveTo(-11, 8, -18, 18);
    ctx.lineTo(-18, -16);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.beginPath(); ctx.arc(6, -18, 2.5, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(0, -9, 4, 0, Math.PI); ctx.strokeStyle = '#1f2937'; ctx.lineWidth = 2; ctx.stroke();
    ctx.restore();
  }

  function drawScaredGuy(x, y, bob) {
    ctx.save();
    ctx.translate(x, y + Math.sin(bob * 1.7) * 2);
    const shake = Math.sin(bob * 6) * 2.6;
    ctx.translate(shake, 0);
    ctx.strokeStyle = '#6d5131';
    ctx.lineWidth = 4;
    ctx.lineCap = 'round';
    ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, 48); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0, 12); ctx.lineTo(-18, 28); ctx.moveTo(0, 12); ctx.lineTo(18, 28); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0, 48); ctx.lineTo(-14, 75); ctx.moveTo(0, 48); ctx.lineTo(14, 75); ctx.stroke();
    ctx.fillStyle = '#8bc34a';
    roundRect(-15, 6, 30, 34, 10); ctx.fill();
    ctx.fillStyle = '#f2c39b';
    ctx.beginPath(); ctx.arc(0, -10, 14, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#7b4b2a';
    ctx.beginPath();
    ctx.moveTo(-10, -18); ctx.lineTo(-2, -29); ctx.lineTo(14, -23); ctx.lineTo(7, -8); ctx.closePath();
    ctx.fill();
    ctx.fillStyle = '#1f2937';
    ctx.beginPath(); ctx.arc(-5, -11, 1.7, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(5, -11, 1.7, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.moveTo(-6, -2); ctx.lineTo(0, 4); ctx.lineTo(6, -2); ctx.strokeStyle = '#1f2937'; ctx.lineWidth = 2; ctx.stroke();
    ctx.restore();
  }

  function drawBanner(x, y, w, h, color, text) {
    ctx.fillStyle = color;
  ...
  function drawBurger(x, y, s) {
    ...
    ctx.beginPath(); ctx.ellipse(0, -4, 22, 12, 0, Math.PI, 0); ctx.fill();
    ctx.fillStyle = '#5f3417'; ctx.fillRect(-22, -2, 44, 7);
    ctx.fillStyle = '#5cb85c'; ctx.fillRect(-20, 5, 40, 4);
    ctx.fillStyle = '#f1c85f'; ctx.fillRect(-18, 10, 36, 4);
    ctx.fillStyle = '#d89e42'; ctx.fillRect(-22, 14, 44, 9);
    ctx.restore();
  }
  ...
  function drawPizza(x, y, s) {
    ...
    ctx.beginPath(); ctx.moveTo(0, 12); ctx.lineTo(-16, -14); ctx.lineTo(16, -14); ctx.closePath(); ctx.fill();
  ...
  function drawFoodStand(x, y) {
    ctx.fillStyle = '#ef5350';
    roundRect(x, y - 62, 112, 24, 12); ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.font = '700 14px Arial';
    ctx.fillText('ЕДА', x + 39, y - 45);
    ctx.fillStyle = '#ffd54f';
    roundRect(x + 10, y - 38, 92, 48, 8); ctx.fill();
    ctx.fillStyle = '#795548';
    ctx.fillRect(x + 18, y + 10, 8, 30);
    ctx.fillRect(x + 86, y + 10, 8, 30);
    drawBurger(x + 36, y - 8, 0.72);
    drawPizza(x + 75, y - 6, 0.7);
  }

  function drawHauntedBackground() {
    const sky = ctx.createLinearGradient(0, 0, 0, groundY);
    sky.addColorStop(0, '#26365f');
    sky.addColorStop(.5, '#4668a8');
    sky.addColorStop(1, '#82a7e6');
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, canvas.width, groundY + 16);

    drawMoon(118, 74, 26);
    drawCloud(200, 74, true);
    drawCloud(640, 82, true);
    drawCastle();
    drawGhost(430, 230, ghostBob + 0.5);

    ctx.fillStyle = 'rgba(0,0,0,.18)';
    for (let i = 0; i < 6; i++) {
      const bx = 70 + i * 110;
      ctx.beginPath();
      ctx.moveTo(bx, groundY); ctx.lineTo(bx + 10, groundY - 28); ctx.lineTo(bx + 25, groundY); ctx.closePath();
      ctx.fill();
    }
  }

  function drawFestivalBackground() {
    const sky = ctx.createLinearGradient(0, 0, 0, groundY);
    sky.addColorStop(0, '#7dd3fc');
    sky.addColorStop(.45, '#c4f1ff');
    sky.addColorStop(1, '#fde68a');
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, canvas.width, groundY + 16);

    drawCloud(80, 72, false);
    drawCloud(270, 56, false);
    drawCloud(620, 82, false);
    drawBanner(90, 78, 138, 32, '#ef4444', 'ФЕСТИВАЛЬ');
    drawBanner(248, 96, 160, 32, '#3b82f6', 'БУРГЕРЫ & ПИЦЦА');
    drawFoodStand(500, 272);
    drawFoodStand(650, 292);
    drawBurger(180, 250, 1);
    drawPizza(275, 248, 1.05);
    drawBurger(360, 274, .86);
    drawPizza(430, 250, .8);
  }
  ...
  function drawGround(scene) {
    ...
    if (scene === 'festival') {
      ...
      g.addColorStop(0, '#678d58');
      g.addColorStop(1, '#365c37');
    }
    ctx.fillStyle = g;
    ctx.fillRect(0, groundY, canvas.width, canvas.height - groundY);
    ctx.strokeStyle = scene === 'festival' ? '#14532d' : '#213d25';
    ctx.lineWidth = 4;
    ctx.beginPath();
  ...
  function drawDog() {
    ...
    const legLift = dog.onGround ? Math.sin(dog.frame) * 5 : 2;
    const tailSwing = dog.onGround ? Math.sin(dog.frame * 1.1) * 0.35 : -0.15;

    ctx.save();
    ctx.translate(12, 36);
    ctx.rotate(-0.4 + tailSwing);
    ctx.strokeStyle = '#6d3b1d';
    ctx.lineWidth = 8;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(-22, -18, -26, -2);
    ctx.stroke();
    ctx.restore();

    ctx.fillStyle = '#8a4b20';
    roundRect(20, 28, 74, 34, 18); ctx.fill();
    ctx.fillStyle = '#b36a31';
    roundRect(30, 37, 34, 17, 9); ctx.fill();

    ctx.fillStyle = '#16b4d8';
    roundRect(83, 30, 10, 21, 6); ctx.fill();
    ctx.fillStyle = '#ffd54f';
    ctx.save();
    ctx.translate(90, 50);
    ctx.rotate(0.2);
    ctx.beginPath();
    ctx.moveTo(0, -6); ctx.lineTo(6, 0); ctx.lineTo(0, 6); ctx.lineTo(-6, 0); ctx.closePath(); ctx.fill();
    ctx.restore();

    ctx.fillStyle = '#8a4b20';
    roundRect(82, 8, 28, 32, 14); ctx.fill();
    roundRect(100, 16, 28, 20, 10); ctx.fill();
    ctx.fillStyle = '#6b3518';
    roundRect(84, 4, 12, 28, 8); ctx.fill();
    roundRect(94, 6, 12, 30, 8); ctx.fill();
    ctx.fillStyle = '#111';
    ctx.beginPath(); ctx.arc(127, 25, 4.6, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(106, 18, 5, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#111';
    ctx.beginPath(); ctx.arc(107.5, 19, 2.3, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = '#111';
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(116, 32); ctx.quadraticCurveTo(121, 38, 126, 31); ctx.stroke();

    ctx.fillStyle = '#4a2413';
    ctx.beginPath(); ctx.ellipse(43, 37, 10, 6, .18, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(69, 52, 8, 5, -.15, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(97, 24, 5, 7, 0, 0, Math.PI * 2); ctx.fill();

    ctx.fillStyle = '#6d3b1d';
    roundRect(31, 56, 10, 24 + legLift, 6); ctx.fill();
    roundRect(53, 56, 10, 24 - legLift, 6); ctx.fill();
    roundRect(75, 56, 10, 24 - legLift, 6); ctx.fill();
    roundRect(95, 56, 10, 24 + legLift, 6); ctx.fill();
  ...
  function drawObstacle(o, scene) {
    if (scene === 'festival') {
      ...
      roundRect(o.x + 4, o.y + 5, o.w - 8, 10, 5); ctx.fill();
      ctx.fillStyle = '#fff3e0';
      for (let i = 0; i < 3; i++) {
        ctx.fillRect(o.x + 7 + i * 10, o.y + 8, 3, 4);
      }
    } else {
      ctx.fillStyle = '#6d4c41';
      roundRect(o.x, o.y + 6, o.w, o.h - 6, 7); ctx.fill();
      ctx.fillStyle = '#9e9e9e';
      ctx.beginPath();
      ctx.moveTo(o.x + 2, o.y + 12);
      ctx.quadraticCurveTo(o.x + o.w / 2, o.y - 12, o.x + o.w - 2, o.y + 12);
      ctx.lineTo(o.x + o.w - 2, o.y + o.h);
      ctx.lineTo(o.x + 2, o.y + o.h);
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = '#616161';
      ctx.fillRect(o.x + o.w / 2 - 2, o.y + 16, 4, o.h - 24);
      ctx.fillRect(o.x + 7, o.y + 24, o.w - 14, 4);
    }
  }

  function drawSceneLabel(scene) {
    ctx.save();
    ctx.fillStyle = 'rgba(17,24,39,.26)';
    roundRect(16, 16, scene === 'festival' ? 160 : 190, 34, 12); ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.font = '800 16px Arial';
    ctx.fillText(scene === 'festival' ? 'Фон: фестиваль еды' : 'Фон: зловещий замок', 30, 39);
    ctx.restore();
  }

  function drawSceneLayer(scene, alpha = 1, offsetX = 0) {
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.translate(offsetX, 0);
    if (scene === 'festival') drawFestivalBackground();
    else drawHauntedBackground();
    drawGround(scene);
    ctx.restore();
  }
  ...
  function draw() {
    ...
    if (transition <= 0) {
      drawSceneLayer('haunted', 1, 0);
    } else if (transition >= 1) {
      drawSceneLayer('festival', 1, 0);
    } else {
      drawSceneLayer('haunted', 1, 0);
      drawSceneLayer('festival', transition, (1 - transition) * canvas.width * 0.38);
    }

    for (const o of obstacles) drawObstacle(o, scene);
    drawDog();
    ...
    if (gameOver) {
      ...
      ctx.fillText('Игра окончена', canvas.width / 2, 215);
      ctx.font = '700 24px Arial';
      ctx.fillText('Ваш счёт: ' + Math.floor(score) + '  •  Рекорд: ' + best, canvas.width / 2, 260);
      ctx.font = '600 18px Arial';
      ctx.fillText('Нажмите, чтобы начать заново', canvas.width / 2, 300);
      ctx.textAlign = 'left';
    }
  }
