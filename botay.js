...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .score { font-weight: 800; font-size: clamp(16px, 4vw, 24px); }
    canvas { width: 100%; height: auto; display: block; border-radius: 18px; background: #17366d; touch-action: manipulation; }
    .hint { margin: 12px 2px 0; opacity: .9; line-height: 1.4; font-size: 14px; text-align: center; }
    .button { display: block; width: 100%; margin-top: 12px; border: 0; border-radius: 16px; padding: 14px 16px; font-size: 17px; font-weight: 800; color: #111827; background: #facc15; cursor: pointer; }
...
<body>
  <main class="wrap">
    <section class="card">
      ...
      <canvas id="game" width="980" height="620" aria-label="Игра: мультяшная собака перепрыгивает препятствия"></canvas>
      <button class="button" id="jumpBtn">Прыгнуть / Начать заново</button>
      <p class="hint">Нажмите кнопку, экран или пробел. Перепрыгивайте препятствия и набирайте очки. Скорость постепенно растёт.</p>
    </section>
  </main>
...
(function () {
  ...
  const groundY = 485;
  const dog = { x: 138, y: groundY - 108, w: 158, h: 108, vy: 0, onGround: true, frame: 0 };
  const gravity = 0.76;
  const jumpPower = -17.8;
  let obstacles = [];
  let speed = 4.2;
  let nextObstacle = 0;
  let score = 0;
  ...
  function getFestivalTransition() {
    return smoothstep((score - 560) / 190);
  }

  function getScene() {
    return getFestivalTransition() >= 0.72 ? 'festival' : 'haunted';
  }

  function addObstacle() {
    const scene = getScene();
    const h = scene === 'festival' ? 34 + Math.random() * 46 : 38 + Math.random() * 46;
    const w = scene === 'festival' ? 30 + Math.random() * 30 : 28 + Math.random() * 20;
    obstacles.push({ x: canvas.width + 25, y: groundY - h, w, h, scene });
    nextObstacle = 98 + Math.random() * 105;
  }

  function collides(a, b) {
    const padX = 22;
    const padTop = 18;
    return a.x + padX < b.x + b.w && a.x + a.w - padX > b.x && a.y + padTop < b.y + b.h && a.y + a.h - 8 > b.y;
  }
  ...
  function drawMoon(x, y, r) {
    ...
    ctx.fillStyle = '#17366d';
    ctx.beginPath(); ctx.arc(x + 13, y - 8, r * .92, 0, Math.PI * 2); ctx.fill();
  }
  ...
  function drawCloud(x, y, scale, alpha) {
    ...
    ctx.fillStyle = 'rgba(126, 146, 205, ' + alpha + ')';
    ctx.beginPath();
    ctx.arc(0, 4, 18, 0, Math.PI * 2);
    ctx.arc(22, -7, 24, 0, Math.PI * 2);
    ctx.arc(49, 5, 18, 0, Math.PI * 2);
    ctx.arc(29, 8, 20, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }

  function drawStarField(offset, alpha) {
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.fillStyle = 'rgba(255,255,255,.72)';
    for (let i = 0; i < 26; i++) {
      const x = ((i * 89 + offset * .12) % (canvas.width + 80)) - 40;
      const y = 30 + ((i * 53) % 220);
      ctx.beginPath();
      ctx.arc(x, y, i % 4 === 0 ? 1.2 : .8, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }

  function drawGrave(x, y, s, cross) {
    ctx.save();
    ctx.translate(x, y);
    ctx.scale(s, s);
    ctx.fillStyle = '#566176';
    ctx.strokeStyle = '#2d374a';
    ctx.lineWidth = 3;
    if (cross) {
      ctx.fillRect(-5, -46, 10, 46);
      ctx.fillRect(-20, -34, 40, 10);
      ctx.strokeRect(-5, -46, 10, 46);
      ctx.strokeRect(-20, -34, 40, 10);
      ctx.fillStyle = '#465166';
      roundRect(-30, -8, 60, 58, 18); ctx.fill(); ctx.stroke();
    } else {
      roundRect(-25, -48, 50, 98, 20); ctx.fill(); ctx.stroke();
  ...
  function drawCastle(x, y, alpha) {
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.translate(x, y);

    ctx.fillStyle = 'rgba(13, 19, 32, .32)';
    ctx.beginPath();
    ctx.ellipse(112, 178, 128, 18, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = '#141c2b';
    roundRect(0, 48, 210, 150, 2); ctx.fill();
    ctx.fillRect(20, 0, 46, 198);
    ctx.fillRect(86, -42, 56, 240);
    ctx.fillRect(162, 14, 46, 184);

    ctx.fillStyle = '#0f1726';
    ctx.beginPath(); ctx.moveTo(20, 0); ctx.lineTo(43, -48); ctx.lineTo(66, 0); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(86, -42); ctx.lineTo(114, -104); ctx.lineTo(142, -42); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(162, 14); ctx.lineTo(185, -34); ctx.lineTo(208, 14); ctx.closePath(); ctx.fill();

    ctx.fillStyle = '#1d2739';
    for (let i = 0; i < 6; i++) {
      const bx = 9 + i * 34;
      ctx.fillRect(bx, 40, 18, 16);
    }

    function windowGlow(wx, wy) {
      ctx.fillStyle = '#ffc83d';
      roundRect(wx, wy, 28, 42, 14); ctx.fill();
      ctx.fillStyle = 'rgba(255, 238, 148, .85)';
      ctx.fillRect(wx + 13, wy + 4, 3, 34);
      ctx.fillRect(wx + 3, wy + 20, 22, 3);
    }
    windowGlow(32, 92);
    windowGlow(93, 74);
    windowGlow(161, 96);
    ...
    roundRect(91, 130, 58, 70, 26); ctx.fill();

    ctx.strokeStyle = 'rgba(255,255,255,.06)';
    ctx.lineWidth = 2;
    for (let i = 0; i < 10; i++) {
      const bx = 24 + (i * 31) % 160;
      const by = 58 + (i * 43) % 112;
  ...
  function drawGhost(x, y, bob, alpha) {
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.translate(x, y + Math.sin(bob) * 9);
    ctx.fillStyle = 'rgba(241,248,255,.96)';
    ctx.beginPath();
    ctx.arc(0, -22, 24, Math.PI, 0);
    ctx.lineTo(24, 28);
    ctx.quadraticCurveTo(15, 15, 7, 28);
    ctx.quadraticCurveTo(0, 17, -8, 28);
    ctx.quadraticCurveTo(-15, 15, -24, 28);
    ctx.lineTo(-24, -22);
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = '#162033';
    ctx.beginPath(); ctx.arc(-8, -24, 3.2, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(8, -24, 3.2, 0, Math.PI * 2); ctx.fill();
    ctx.lineWidth = 2.5;
    ctx.strokeStyle = '#162033';
    ctx.beginPath(); ctx.arc(0, -13, 7, 0, Math.PI); ctx.stroke();
    ctx.restore();
  }

  function drawBurger(x, y, s) {
    ctx.save(); ctx.translate(x, y); ctx.scale(s, s);
    ctx.fillStyle = '#e7a947'; ctx.beginPath(); ctx.ellipse(0, -8, 24, 14, 0, Math.PI, 0); ctx.fill();
    ctx.fillStyle = '#633519'; ctx.fillRect(-24, -6, 48, 8);
    ctx.fillStyle = '#55b85a'; ctx.fillRect(-22, 2, 44, 5);
    ctx.fillStyle = '#ffd35b'; ctx.fillRect(-20, 9, 40, 5);
    ctx.fillStyle = '#d99b3c'; ctx.fillRect(-24, 16, 48, 10);
    ctx.restore();
  }

  function drawPizza(x, y, s) {
    ctx.save(); ctx.translate(x, y); ctx.scale(s, s);
    ctx.fillStyle = '#f1d08a'; ctx.beginPath(); ctx.moveTo(0, 24); ctx.lineTo(-24, -22); ctx.lineTo(24, -22); ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#de6548'; ctx.beginPath(); ctx.moveTo(0, 14); ctx.lineTo(-18, -17); ctx.lineTo(18, -17); ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#ffd166';
    [[-7,-5],[8,-10],[4,5]].forEach(function (p) { ctx.beginPath(); ctx.arc(p[0], p[1], 4, 0, Math.PI * 2); ctx.fill(); });
    ctx.restore();
  }

  function drawFoodStand(x, y, alpha) {
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.fillStyle = '#ef4444'; roundRect(x, y - 72, 124, 28, 13); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = '800 15px Arial'; ctx.textAlign = 'center'; ctx.fillText('ЕДА', x + 62, y - 52); ctx.textAlign = 'left';
    ctx.fillStyle = '#ffd54f'; roundRect(x + 12, y - 44, 100, 54, 9); ctx.fill();
    ctx.fillStyle = '#795548'; ctx.fillRect(x + 20, y + 10, 9, 34); ctx.fillRect(x + 94, y + 10, 9, 34);
    drawBurger(x + 42, y - 12, .75);
    drawPizza(x + 82, y - 10, .72);
    ctx.restore();
  }

  function drawHauntedLayer(offsetX, alpha) {
    ...
    drawMoon(95, 75, 29);
    drawCloud(220, 76, 1.18, .32);
    drawCloud(642, 82, 1.12, .31);
    drawGhost(425, 220, ghostBob + worldX * .004, 1);

    drawCastle(495, 206, 1);
    drawGrave(46, groundY - 18, .92, true);
    drawGrave(340, groundY - 10, .70, false);
    drawGrave(405, groundY - 18, .76, true);
    drawGrave(462, groundY - 8, .54, false);
    drawGrave(778, groundY - 18, .80, false);
    ctx.restore();
  }

  function drawFestivalLayer(offsetX, alpha) {
    if (alpha <= 0.01) return;
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.translate(offsetX, 0);

    drawCloud(95, 72, 1.25, .46);
    drawCloud(305, 58, 1.05, .38);
    drawCloud(635, 76, 1.18, .44);

    ctx.fillStyle = 'rgba(255, 229, 128, .35)';
    for (let i = 0; i < 9; i++) {
      const x = 120 + i * 72;
      const y = 72 + (i % 3) * 26;
    ...
    roundRect(105, 90, 160, 34, 14); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = '800 16px Arial'; ctx.textAlign = 'center'; ctx.fillText('ФЕСТИВАЛЬ ЕДЫ', 185, 113); ctx.textAlign = 'left';
    drawFoodStand(470, groundY - 46, 1);
    drawFoodStand(635, groundY - 32, 1);
    drawBurger(202, groundY - 98, 1.02);
    drawPizza(306, groundY - 92, 1.03);
    drawBurger(386, groundY - 76, .78);
    ctx.restore();
  }

  function drawGround(transition) {
    const hauntedOffset = -transition * 500;
    const festivalOffset = canvas.width - transition * canvas.width;

    ctx.save();
    ctx.translate(hauntedOffset, 0);
    const gh = ctx.createLinearGradient(0, groundY, 0, canvas.height);
    gh.addColorStop(0, '#6a965a');
    gh.addColorStop(1, '#294d2e');
    ctx.fillStyle = gh;
    ctx.fillRect(-60, groundY, canvas.width + 160, canvas.height - groundY);
    ctx.restore();

    ctx.save();
    ctx.globalAlpha = transition;
    ctx.translate(festivalOffset, 0);
    const gf = ctx.createLinearGradient(0, groundY, 0, canvas.height);
    gf.addColorStop(0, '#76d69b');
    gf.addColorStop(1, '#2f7d3f');
    ctx.fillStyle = gf;
    ctx.fillRect(-40, groundY, canvas.width + 120, canvas.height - groundY);
    ctx.restore();

    ctx.strokeStyle = 'rgba(20,83,45,.95)';
    ctx.lineWidth = 4;
    ctx.beginPath(); ctx.moveTo(0, groundY); ctx.lineTo(canvas.width, groundY); ctx.stroke();

    ctx.fillStyle = 'rgba(27, 71, 38, .28)';
    for (let i = 0; i < 34; i++) {
      const x = ((i * 71 - worldX * .45) % (canvas.width + 80)) - 30;
      const y = groundY + 34 + ((i * 39) % 78);
      ctx.beginPath();
      ctx.moveTo(x, y); ctx.lineTo(x + 4, y - 9); ctx.lineTo(x + 8, y);
      ctx.moveTo(x + 12, y); ctx.lineTo(x + 14, y - 7); ctx.lineTo(x + 18, y);
      ctx.strokeStyle = 'rgba(21, 65, 31, .28)';
      ctx.lineWidth = 2;
      ctx.stroke();
    }
  }

  function drawSky(transition) {
    const night = ctx.createLinearGradient(0, 0, 0, groundY);
    night.addColorStop(0, '#0e2360');
    night.addColorStop(.45, '#23509a');
    night.addColorStop(1, '#79a6e4');
    ctx.fillStyle = night;
    ctx.fillRect(0, 0, canvas.width, groundY + 18);

    if (transition > 0) {
      ctx.save();
      ctx.globalAlpha = transition;
      const warm = ctx.createLinearGradient(0, 0, 0, groundY);
      warm.addColorStop(0, '#4da3e8');
      warm.addColorStop(.55, '#b6e8ff');
      warm.addColorStop(1, '#ffe49a');
      ctx.fillStyle = warm;
      ctx.fillRect(0, 0, canvas.width, groundY + 18);
      ctx.restore();
    }

    drawStarField(worldX, 1 - transition * .85);
  }
  ...
  function drawDog() {
    ...
    const run = dog.onGround ? dog.frame : 0;
    const stretch = dog.onGround ? Math.sin(run * .5) * 2 : -2;
    const tailSwing = dog.onGround ? Math.sin(run * 1.15) * .45 : -.25;

    ctx.save();
    ctx.translate(22, 42);
    ctx.rotate(-.7 + tailSwing);
    ctx.strokeStyle = '#8d4d21';
    ctx.lineWidth = 11;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(-34, -27, -37, 3);
    ctx.stroke();
    ctx.restore();

    ctx.fillStyle = '#9a5524';
    roundRect(26, 27 + stretch, 76, 41, 21); ctx.fill();
    ctx.fillStyle = '#c7742f';
    roundRect(38, 37 + stretch, 36, 18, 10); ctx.fill();

    ctx.fillStyle = '#19b9db';
    roundRect(91, 30, 11, 25, 6); ctx.fill();
    ctx.fillStyle = '#ffd447';
    ctx.save(); ctx.translate(99, 55); ctx.rotate(.15); ctx.beginPath(); ctx.moveTo(0,-8); ctx.lineTo(8,0); ctx.lineTo(0,8); ctx.lineTo(-8,0); ctx.closePath(); ctx.fill(); ctx.restore();

    ctx.fillStyle = '#9a5524';
    roundRect(86, -3, 45, 48, 22); ctx.fill();
    roundRect(120, 12, 42, 29, 15); ctx.fill();
    ctx.fillStyle = '#723916';
    roundRect(88, -9, 17, 43, 10); ctx.fill();
    roundRect(103, -7, 17, 45, 10); ctx.fill();
    ctx.fillStyle = '#3f1d0c';
    ctx.beginPath(); ctx.ellipse(155, 26, 9, 7, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(119, 15, 7.8, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(136, 16, 7.2, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#111827';
    ctx.beginPath(); ctx.arc(122, 16, 3.3, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(139, 17, 3.1, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = '#111827'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(135, 34); ctx.quadraticCurveTo(145, 43, 157, 33); ctx.stroke();

    ctx.fillStyle = '#5f2d13';
    ctx.beginPath(); ctx.ellipse(51, 39, 11, 7, .2, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(78, 57, 9, 6, -.1, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(107, 27, 7, 9, 0, 0, Math.PI * 2); ctx.fill();

    ctx.fillStyle = '#7b421d';
    function leg(x, y, phase, front) {
      const swing = Math.sin(run + phase) * 12;
      const bend = Math.cos(run + phase) * 8;
      ctx.save();
      ctx.translate(x, y);
      ctx.rotate(swing * Math.PI / 180);
      roundRect(-5, 0, 10, 28 + bend * .35, 6); ctx.fill();
      ctx.translate(0, 25 + bend * .35);
      ctx.rotate(-swing * Math.PI / 150);
      roundRect(front ? -3 : -8, 0, 22, 9, 5); ctx.fill();
      ctx.restore();
    }
    leg(39, 62, 0, false);
    leg(62, 62, Math.PI, false);
    leg(89, 62, Math.PI, true);
    leg(109, 62, 0, true);

    ctx.restore();
  }

  function drawObstacle(o, scene) {
    if (scene === 'festival') {
      ctx.fillStyle = '#8d6e63'; roundRect(o.x, o.y, o.w, o.h, 8); ctx.fill();
      ctx.fillStyle = '#ff7043'; roundRect(o.x + 4, o.y + 5, o.w - 8, 10, 5); ctx.fill();
      ctx.fillStyle = '#fff3e0';
      for (let i = 0; i < 3; i++) ctx.fillRect(o.x + 7 + i * 10, o.y + 8, 3, 4);
    } else {
      ctx.fillStyle = '#5f6677';
      roundRect(o.x, o.y, o.w, o.h, 9); ctx.fill();
      ctx.strokeStyle = '#30394d'; ctx.lineWidth = 3; ctx.stroke();
      ctx.fillStyle = 'rgba(28,34,48,.45)';
      ctx.fillRect(o.x + o.w / 2 - 2, o.y + 10, 4, o.h - 18);
      ctx.fillRect(o.x + 7, o.y + 23, o.w - 14, 4);
    }
  }
  ...
  function draw() {
    ...
    const transition = getFestivalTransition();
    const hauntedOffset = -transition * 500;
    const festivalOffset = canvas.width - transition * canvas.width;

    drawSky(transition);
    drawHauntedLayer(hauntedOffset, 1 - transition * .92);
    drawFestivalLayer(festivalOffset, transition);
    drawGround(transition);

    for (const o of obstacles) drawObstacle(o, o.scene || getScene());
    drawDog();
    ...
    if (!started && !gameOver) {
      ...
      ctx.fillText('Нажмите, чтобы начать', canvas.width / 2, 240);
      ctx.font = '700 22px Arial';
      ctx.fillText('Собака стоит спокойно до первого прыжка', canvas.width / 2, 278);
      ctx.textAlign = 'left';
    }
    ...
    if (gameOver) {
      ...
      ctx.fillText('Игра окончена', canvas.width / 2, 248);
      ctx.font = '700 26px Arial';
      ctx.fillText('Ваш счёт: ' + Math.floor(score) + '  •  Рекорд: ' + best, canvas.width / 2, 296);
      ctx.font = '600 20px Arial';
      ctx.fillText('Нажмите, чтобы начать заново', canvas.width / 2, 340);
      ctx.textAlign = 'left';
    }
  }
  ...
  function update(dt) {
    ...
    nextObstacle -= dt * 0.045;
    if (nextObstacle <= 0) addObstacle();

    for (const o of obstacles) o.x -= speed * dt * 0.052;
...
async function updateRafflePublishedPosts(raffleId, options = {}) {
  ...
  for (const post of postsRes.rows) {
    ...
    const ok = await editMaxMessageText(
      ...
      post.message_id,
      text,
      [[{ text: '🎁 Участвовать', url: buildBotDeepLink(buildRaffleJoinPayload(raffle.id, post.channel_id)) }]],
      getRafflePhotoAttachments(raffle)
    );
...
async function publishRaffleToChannel(raffle, channelId) {
  ...
  const data = await sendMessage(
    ...
    text,
    [
      [{ text: '🎁 Участвовать', url: buildBotDeepLink(buildRaffleJoinPayload(raffle.id, channelId)) }]
    ],
    getRafflePhotoAttachments(raffle)
  );
...
function buildJoinChannelsText(channels = []) {
  ...
  const lines = unique.map((channel, index) => {
    const mark = channel.is_required
...
function buildJoinIntroText(raffle, channels = []) {
  ...
  return [
    ...
    requiredChannels.length
      ? 'Чтобы принять участие, подпишитесь на каналы с пометкой **обязательная подписка**:'
      : 'В этом розыгрыше нет обязательных каналов для подписки.',
    channelsText || '',
    '',
    requiredChannels.length
      ? 'Каналы с пометкой **подписка не обязательна** показаны для информации и не влияют на проверку.'
      : 'Каналы показаны для информации и не влияют на проверку.',
    '',
    'После подписки нажмите кнопку **✅ Проверить подписку**.',
    'Билет будет создан только после успешной проверки обязательных подписок.',
    '',
    `Окончание: **${formatDateTime(raffle.end_at)}**`
  ].filter(Boolean).join('\n');
}
...
async function sendAdminActiveRaffles(target, userId) {
  ...
  for (const r of res.rows) {
    ...
    row.push({ text: `🏆 Завершить #${r.id}`, callback_data: `pick_raffle:${r.id}` });
...
async function handleMaxUpdate(update) {
  try {
    ...
    if (text === '/play' || text.toLowerCase() === 'play' || text.toLowerCase() === 'играть') {
      await sendPlayGameMessage(chatTarget);
      return;
    }
