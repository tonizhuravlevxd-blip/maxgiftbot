...
async function upsertBotSeenChat(chatId, chatTitle, chatLink, sourceUpdateType = '', actorUserId = null, payload = {}) {
  ...
  const res = await pool.query(`
    ...
    VALUES ($1, $2, $3, $4, $5, $6::jsonb, 1, true, NOW())
    ON CONFLICT (chat_id)
    DO UPDATE SET
  ...
  if (result.added.length) {
    ...
    await sendMessage(
      ...
      [
        ...
        '',
        `Проверено увиденных каналов: ${result.checked}.`,
        '',
        'Бот больше не использует /me/chats. Кнопка **Обновить** проверяет только те каналы, которые бот уже увидел через webhook-события MAX.',
        'Если канал не появился, значит MAX пока не прислал событие из этого канала. Попробуйте заново назначить бота администратором или проверьте, что webhook зарегистрирован без жёсткого фильтра update_types.'
      ].join('\n')
    );
  }
