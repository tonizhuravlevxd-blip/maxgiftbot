...
const MAX_UPDATE_TYPES = String(
  process.env.MAX_UPDATE_TYPES ||
  'bot_started,message_created,message_callback,chat_member_updated,chat_member_created,chat_member_removed,chat_updated,bot_added,bot_removed'
)
  .split(',')
  .map(x => x.trim())
  .filter(Boolean);

const MAX_CHANNEL_DISCOVERY_ENDPOINTS = String(
  process.env.MAX_CHANNEL_DISCOVERY_ENDPOINTS || '/chats,/me/chats'
)
  .split(',')
  .map(x => x.trim())
  .filter(Boolean);
...
function extractChatCandidatesFromResponse(body) {
  if (!body) return [];
  ...
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) {
      ...
      if (nestedArray) arrays.push(nestedArray);
      if (extractChatIdFromObject(candidate)) arrays.push([candidate]);
    }
  }
  ...
  for (const arr of arrays) {
    for (const item of arr) {
      const chatId = extractChatIdFromObject(item);
      if (!chatId) continue;
      map.set(chatId, item);
    }
  }
...
async function listBotChatsSafe() {
  const result = [];
  const seen = new Set();

  for (const endpoint of MAX_CHANNEL_DISCOVERY_ENDPOINTS) {
    try {
      const response = await maxRequest(endpoint, { method: 'GET', query: { count: 100 } });

      for (const item of extractChatCandidatesFromResponse(response)) {
        const chatId = extractChatIdFromObject(item);
      ...
      console.warn(`Поиск каналов через ${endpoint} недоступен:`, error.message);
    }
  }
...
async function discoverUserChannelsForUser(userId) {
  const candidates = await listBotChatsSafe();
  const added = [];
  const skipped = [];

  for (const candidate of candidates) {
    const channelId = extractChatIdFromObject(candidate);

    if (!channelId || !isProbablyChannelChat(candidate)) continue;
    ...
    const meta = {
      title: extractTitleFromChatInfo(info) || extractTitleFromChatInfo(candidate) || `Канал ${channelId}`,
      link: extractChatLinkFromObject(info) || extractChatLinkFromObject(candidate)
    };
  ...
  if (result.added.length) {
    ...
    await sendMessage(
      ...
      [
        ...
        '',
        'Если канал всё равно не появляется, значит MAX не отдаёт боту список каналов без события из канала. В таком случае бот сможет добавить канал только после системного события о назначении администратором или после любого входящего события из этого канала.'
      ].join('\n')
    );
  }
...
async function sendAddChannelInstruction(target) {
  return sendMessage(
    ...
    [
      ...
      '',
      'Важно: если MAX не отдаст боту список каналов без события из канала, канал появится после системного события о назначении бота администратором.'
    ].join('\n'),
    [
