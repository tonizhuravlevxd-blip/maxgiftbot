...
const RACE_COLLAB_STATUSES = new Set(['ready']);
...
function setupHorseRacesModule(options = {}) {
  ...
  const config = {
    ...
    postUpdateDebounceSeconds: Math.max(
      5,
      Number(
        process.env.HORSE_RACE_POST_UPDATE_DEBOUNCE_SECONDS ||
        process.env.RAFFLE_POST_UPDATE_DEBOUNCE_SECONDS ||
        30
      )
    ),
    postParticipantsUpdateSeconds: Math.max(
      0,
      Number(
        process.env.HORSE_RACE_POST_PARTICIPANTS_UPDATE_SECONDS ||
        process.env.POST_PARTICIPANTS_UPDATE_SECONDS ||
        60
      )
    ),
    referralBonusLimit: Math.max(
      0,
      Number(
        process.env.HORSE_RACE_REFERRAL_BONUS_LIMIT ||
        process.env.MAX_REFERRAL_BONUS_TICKETS ||
        5
      )
    ),
  ...
  let postParticipantsUpdateTimer = null;
  ...
  function buildRaceReferralLink(raceId, userId) {
    const id = normalizePositiveInt(raceId);
    const cleanUserId = String(userId || '').trim();

    if (!id || !/^\d+$/.test(cleanUserId)) return '';

    return getBotUrl(`horse_ref_${id}_${cleanUserId}`);
  }

  async function getRaceReferralBonusCount(raceId, userId) {
    const id = normalizePositiveInt(raceId);
    const cleanUserId = String(userId || '').trim();

    if (!id || !/^\d+$/.test(cleanUserId)) return 0;
    ...
    const result = await pool.query(`
      ...
      WHERE horse_race_id = $1
        AND user_id = $2
        AND is_valid = true
        AND ticket_type = 'referral_bonus'
    `, [id, cleanUserId]);
  ...
  async function initDb() {
    await pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS horse_race_channel_subscription_states (
        id SERIAL PRIMARY KEY,
        horse_race_id INT NOT NULL REFERENCES horse_races(id) ON DELETE CASCADE,
        channel_id BIGINT NOT NULL,
        ...
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE (horse_race_id, channel_id, user_id)
      );

      CREATE TABLE IF NOT EXISTS horse_race_channel_subscription_events (
        id SERIAL PRIMARY KEY,
        horse_race_id INT NOT NULL REFERENCES horse_races(id) ON DELETE CASCADE,
        channel_id BIGINT NOT NULL,
        user_id BIGINT NOT NULL REFERENCES users(max_user_id) ON DELETE CASCADE,
      ...
      CREATE INDEX IF NOT EXISTS ix_horse_race_subscription_states_race_channel
        ON horse_race_channel_subscription_states (horse_race_id, channel_id, is_subscribed);

      CREATE INDEX IF NOT EXISTS ix_horse_race_subscription_events_race_channel
        ON horse_race_channel_subscription_events (horse_race_id, channel_id, created_at, delta);

      CREATE TABLE IF NOT EXISTS horse_race_participants (
        id SERIAL PRIMARY KEY,
        horse_race_id INT NOT NULL REFERENCES horse_races(id) ON DELETE CASCADE,
        user_id BIGINT NOT NULL REFERENCES users(max_user_id) ON DELETE CASCADE,
        invited_by BIGINT REFERENCES users(max_user_id) ON DELETE SET NULL,
      ...
      ALTER TABLE horse_race_participants
        ADD COLUMN IF NOT EXISTS invited_by BIGINT REFERENCES users(max_user_id) ON DELETE SET NULL;

      ALTER TABLE horse_race_participants
        DROP CONSTRAINT IF EXISTS horse_race_participants_horse_race_id_user_id_key;

      CREATE UNIQUE INDEX IF NOT EXISTS ux_horse_race_participants_main_user
        ON horse_race_participants (horse_race_id, user_id)
        WHERE ticket_type = 'main';

      CREATE TABLE IF NOT EXISTS horse_race_pending_joins (
        id SERIAL PRIMARY KEY,
        horse_race_id INT NOT NULL REFERENCES horse_races(id) ON DELETE CASCADE,
        user_id BIGINT NOT NULL REFERENCES users(max_user_id) ON DELETE CASCADE,
        referrer_user_id BIGINT REFERENCES users(max_user_id) ON DELETE SET NULL,
      ...
      ALTER TABLE horse_race_pending_joins
        ADD COLUMN IF NOT EXISTS referrer_user_id BIGINT REFERENCES users(max_user_id) ON DELETE SET NULL;

      CREATE TABLE IF NOT EXISTS horse_race_referrals (
        id SERIAL PRIMARY KEY,
        horse_race_id INT NOT NULL REFERENCES horse_races(id) ON DELETE CASCADE,
        referred_user_id BIGINT NOT NULL REFERENCES users(max_user_id) ON DELETE CASCADE,
        referrer_user_id BIGINT NOT NULL REFERENCES users(max_user_id) ON DELETE CASCADE,
        bonus_participant_id INT REFERENCES horse_race_participants(id) ON DELETE SET NULL,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE (horse_race_id, referred_user_id)
      );

      CREATE INDEX IF NOT EXISTS ix_horse_race_referrals_referrer
        ON horse_race_referrals (horse_race_id, referrer_user_id);
      ...
      CREATE TABLE IF NOT EXISTS horse_race_post_updates (
        horse_race_id INT PRIMARY KEY REFERENCES horse_races(id) ON DELETE CASCADE,
      ...
      CREATE INDEX IF NOT EXISTS ix_horse_race_post_updates_due
        ON horse_race_post_updates (status, scheduled_at);
  ...
  async function getRaceChannelManagementContext(raceId, userId) {
    const id = normalizePositiveInt(raceId);
    const cleanUserId = String(userId || '').trim();

    if (!id) {
      throw new Error('Некорректный ID скачек');
    }
    ...
    if (!race) {
      throw new Error('Скачки не найдены');
    }

    const owner =
      String(race.creator_user_id || '') === cleanUserId;
    const admin = Boolean(isAdmin(cleanUserId));

    let ownsConnectedChannel = false;
    ...
    if (!owner && !admin) {
      ...
      ownsConnectedChannel = Boolean(access.rows[0]);
    }

    if (!owner && !admin && !ownsConnectedChannel) {
      throw new Error(
        'Управлять каналами может только организатор, администратор или владелец подключённого канала'
      );
    }

    if (
      String(race.status || '') !== 'ready' ||
      race.publish_at ||
      Boolean(race.published_in_general)
    ) {
      throw new Error(
        'Удалять каналы можно только до публикации скачек'
      );
    }

    const publishedPost = await pool.query(`
      SELECT 1
      FROM horse_race_posts
      WHERE horse_race_id = $1
    ...
    if (publishedPost.rows[0]) {
      throw new Error(
        'Посты этих скачек уже опубликованы. Удаление каналов запрещено'
      );
    }
    ...
    return {
      ...
      owner,
      admin,
      ownsConnectedChannel
    };
  }

  async function sendRaceChannelManagement(
    target,
    userId,
    raceId,
    notice = ''
    ...
    const id = normalizePositiveInt(raceId);
    const cleanUserId = String(userId || '').trim();
    const context = await getRaceChannelManagementContext(
      id,
      cleanUserId
    );
    const channels = await getRaceChannels(id);
    const canManageAll = context.owner || context.admin;
    const removableChannels = canManageAll
      ? channels
      : channels.filter(
          channel =>
            String(channel.ownerUserId || '') === cleanUserId
        );

    const lines = [
      '⚙️ **Управление каналами скачек**',
      '',
      `🐎 #${id} — **${cleanText(context.race.title, 180) || 'Скачки'}**`,
      `🏁 Гонка: **${formatRaceDate(context.race.start_at)}**`,
      '',
      'Каналы можно удалять только пока скачки имеют статус **«Готово к запуску»** и посты ещё не опубликованы.'
    ];
    ...
    if (!channels.length) {
      lines.push('', '📭 Подключённых каналов сейчас нет.');
    } else {
      lines.push('', '📢 **Подключённые каналы:**');

      channels.forEach((channel, index) => {
        const ownedByRequester =
          String(channel.ownerUserId || '') === cleanUserId;
        const canRemove = canManageAll || ownedByRequester;

        lines.push(
          `${index + 1}. ${channel.link
            ? `[${escapeMarkdownText(channel.title)}](${channel.link})`
            : escapeMarkdownText(channel.title)} — ${channel.publishPost
              ? 'с размещением 📣'
              : 'без размещения 🙈'}${canRemove ? '' : ' 🔒'}`
        );
      });
    }
    ...
    for (const channel of removableChannels) {
      keyboard.push([{
        text: `🗑 ${cleanText(channel.title, 34) || channel.channelId}`,
        callback_data: `horse_channel_remove_ask:${id}:${channel.channelId}`
      }]);
    }

    if (canManageAll) {
      keyboard.push([{
        text: '➕ Добавить или заменить канал',
        callback_data: `horse_collab_link:${id}`
      }]);
    }

    keyboard.push([{
      text: '📊 Статистика скачек',
      callback_data: `horse_stats:${id}`
    }]);

    keyboard.push([{
      text: context.owner
        ? '⬅️ Мои скачки'
        : '⬅️ Где участвуют мои каналы',
      callback_data: context.owner
        ? 'horse_my_races'
        : 'horse_my_collab'
    }]);
  ...
  async function removeRaceChannelBeforePublish(
    raceId,
    channelId,
    userId
  ) {
    const id = normalizePositiveInt(raceId);
    const cleanChannelId = normalizeHorseChannelId(channelId);
    const cleanUserId = String(userId || '').trim();

    if (!id || !cleanChannelId) {
      throw new Error('Некорректные данные канала');
    }
    ...
    try {
      ...
      const anyPublishedPost = await client.query(`
        SELECT 1
        FROM horse_race_posts
        WHERE horse_race_id = $1
          AND status = 'published'
      ...
      if (!channel) {
        throw new Error('Канал уже удалён или не найден');
      }

      const canDelete =
        isAdmin(cleanUserId) ||
        String(race.creator_user_id || '') === cleanUserId ||
        String(channel.owner_user_id || '') === cleanUserId;

      if (!canDelete) {
        throw new Error('Нет права удалять этот канал');
      }

      // Удаляем технические следы именно этого канала, чтобы после замены
      // старый канал не участвовал в проверках подписки и статистике.
      await client.query(`
        DELETE FROM horse_race_channel_subscription_events
        WHERE horse_race_id = $1
          AND channel_id::text = $2
      `, [id, cleanChannelId]);

      await client.query(`
        DELETE FROM horse_race_channel_subscription_states
        WHERE horse_race_id = $1
          AND channel_id::text = $2
      `, [id, cleanChannelId]);

      await client.query(`
        DELETE FROM horse_race_posts
        WHERE horse_race_id = $1
          AND channel_id::text = $2
      `, [id, cleanChannelId]);

      const deleted = await client.query(`
        DELETE FROM horse_race_channels
        WHERE horse_race_id = $1
          AND channel_id::text = $2
        RETURNING *
      `, [id, cleanChannelId]);
      ...
      return {
        race,
        channel: deleted.rows[0],
        canManageRaceAfterDelete:
          isAdmin(cleanUserId) ||
          String(race.creator_user_id || '') === cleanUserId
      };
    } catch (error) {
  ...
  async function recordHorseRaceSubscriptionCheck(
    raceId,
    userId,
    channelId,
    isSubscribed,
    ...
    const cleanRaceId = normalizePositiveInt(raceId);
    const cleanUserId = String(userId || '').trim();
    const cleanChannelId = String(channelId || '').trim();
    const subscribed = Boolean(isSubscribed);
    const countUnknownLeave = Boolean(options.countUnknownLeave);
    ...
    try {
      ...
      // Webhook может прийти раньше первого открытия бота пользователем.
      // Создаём базовую запись users, чтобы внешний ключ не мешал статистике.
      await client.query(`
      ...
      // Блокируем именно комбинацию «скачки + канал + пользователь».
      // Это защищает от двойного +1, если webhook и кнопка проверки пришли одновременно.
      await client.query(`
      ...
      `, [
        `horse-race:${cleanRaceId}:${cleanChannelId}`,
        cleanUserId
      ]);

      const stateRes = await client.query(`
        SELECT is_subscribed
        FROM horse_race_channel_subscription_states
        WHERE horse_race_id = $1
          AND channel_id::text = $2
          AND user_id = $3
        FOR UPDATE
      `, [cleanRaceId, cleanChannelId, cleanUserId]);

      const previous = stateRes.rows.length
        ? Boolean(stateRes.rows[0].is_subscribed)
        : null;
      ...
      if (previous === null && subscribed) {
        ...
        delta = 1;
        eventType = 'rejoined';
      } else if (previous === null && !subscribed && countUnknownLeave) {
        // Реальный webhook выхода учитываем как отписку даже тогда,
        // когда пользователь раньше не нажимал кнопку проверки в боте.
        delta = -1;
      ...
      if (stateRes.rows.length) {
        await client.query(`
          UPDATE horse_race_channel_subscription_states
          SET
            is_subscribed = $4,
            last_checked_at = NOW(),
            updated_at = NOW()
          WHERE horse_race_id = $1
            AND channel_id::text = $2
            AND user_id = $3
        `, [cleanRaceId, cleanChannelId, cleanUserId, subscribed]);
      } else {
        await client.query(`
          INSERT INTO horse_race_channel_subscription_states (
            horse_race_id,
            channel_id,
            user_id,
            is_subscribed,
      ...
      if (delta !== 0) {
        await client.query(`
          INSERT INTO horse_race_channel_subscription_events (
            horse_race_id,
            channel_id,
            user_id,
            event_type,
      ...
      await client.query('ROLLBACK').catch(() => {});
      console.warn('Не удалось записать статистику подписки скачек:', {
        raceId: cleanRaceId,
  ...
  async function recordHorseRaceMemberDeltaFromWebhook({
    channelId,
    userId,
    delta,
    source = 'channel_member_webhook'
  } = {}) {
    const cleanChannelId = String(channelId || '').trim();
    const cleanUserId = String(userId || '').trim();
    ...
    if (
      ...
      return { matched: 0, recordedPlus: 0, recordedMinus: 0 };
    }

    const racesRes = await pool.query(`
      SELECT DISTINCT hr.id AS horse_race_id
      FROM horse_races hr
    ...
    for (const row of racesRes.rows) {
      const result = await recordHorseRaceSubscriptionCheck(
        row.horse_race_id,
        cleanUserId,
        cleanChannelId,
        cleanDelta > 0,
        source,
        { countUnknownLeave: cleanDelta < 0 }
      );
    ...
    if (racesRes.rows.length) {
      console.log(
        cleanDelta > 0
          ? '📈 Подписка учтена в статистике скачек:'
          : '📉 Отписка учтена в статистике скачек:',
        {
          channel_id: cleanChannelId,
          user_id: cleanUserId,
          matched_races: racesRes.rows.map(row => Number(row.horse_race_id)),
  ...
  function isRaceCollabInviteActive(invite) {
    ...
    if (!RACE_COLLAB_STATUSES.has(String(invite.race_status || ''))) return false;
  ...
  async function getOrCreateRaceCollabInvite(raceId, userId) {
    ...
    if (
      !RACE_COLLAB_STATUSES.has(String(race.status || '')) ||
      new Date(race.start_at).getTime() <= Date.now()
    ) {
      throw new Error('Каналы можно приглашать только до запуска публикации скачек');
    }
  ...
  async function sendRaceCollabInvite(target, userId, raceId) {
    ...
    return sendMessage?.(
      ...
      [
        ...
        `🏁 Гонка начнётся: **${formatRaceDate(race?.start_at)}**`,
        '',
        'Скачки пока **не опубликованы**.',
        'По этой ссылке владельцы каналов могут присоединиться и выбрать:',
        '📣 **с размещением** поста или 🙈 **без размещения**.',
        '',
        'Когда все каналы добавятся, организатор нажмёт **🚀 Запустить сейчас**.',
        'Подписка на каждый добавленный канал будет обязательной.',
        '',
        link
      ].join('\n'),
      [
        [{ text: '🤝 Открыть приглашение', url: link }],
        [{ text: '⚙️ Управление каналами', callback_data: `horse_channel_manage:${raceId}` }],
        [{ text: '🚀 Запустить сейчас', callback_data: `horse_launch_now:${raceId}` }]
      ]
    );
  }
  ...
  async function addRaceCollabChannelsFromSession(target, userId, data = {}) {
    ...
    if (!isRaceCollabInviteActive(invite)) {
      ...
      return sendMessage?.(
        target,
        '🔒 Ссылка приглашения закрыта. Возможно, организатор уже запустил публикацию скачек.'
      );
    }
    ...
    const availableMap = new Map(
      available.map(channel => [Number(channel.id), channel])
    );
    const valid = [];

    for (const selectedItem of selected) {
      const channel = availableMap.get(
        Number(selectedItem.user_channel_id)
      );
      ...
      if (publishPost && !channel.can_publish) {
        throw new Error(
          `Для канала «${cleanText(channel.channel_title, 100) || channel.channel_id}» нет права публикации`
        );
      }
    ...
    if (!valid.length) {
      return sendMessage?.(
        target,
        'Выбранные каналы больше недоступны. Откройте приглашение ещё раз.'
      );
    }
    ...
    const withPlacementCount =
      valid.filter(item => item.publishPost).length;

    const noPlacementCount =
      valid.filter(item => !item.publishPost).length;
    ...
    await sendMessage?.(
      ...
      [
        ...
        '',
        `📣 С размещением после запуска: **${withPlacementCount}**`,
        `🙈 Без размещения: **${noPlacementCount}**`,
        '',
        'Посты пока не опубликованы.',
        'Организатор запустит публикацию, когда закончит набор каналов.'
      ].join('\n'),
      [
        [{
          text: '⚙️ Управление моими каналами',
          callback_data: `horse_channel_manage:${invite.horse_race_id}`
        }],
        [{
          text: '📊 Статистика скачек',
          callback_data: `horse_stats:${invite.horse_race_id}`
        }]
      ]
    );
    ...
    await sendMessage?.(
      ...
      [
        ...
        `🐎 ${cleanText(invite.race_title, 180) || `Скачки #${invite.horse_race_id}`}`,
        ...valid.map(item =>
          `• ${item.title} — ${item.publishPost
            ? 'с размещением после запуска 📣'
            : 'без размещения 🙈'}`
        ),
        '',
        'Когда все каналы добавятся, нажмите **🚀 Запустить сейчас**.'
      ].join('\n'),
      [
        [{
        ...
        [{
          ...
          callback_data: `horse_launch_now:${invite.horse_race_id}`
        }],
        [{
          text: '🤝 Ссылка приглашения',
          callback_data: `horse_collab_link:${invite.horse_race_id}`
        }]
      ]
    ).catch(() => {});
  ...
  async function markRaceReady(raceId, options = {}) {
    const id = normalizePositiveInt(raceId);
    ...
    if (new Date(row.start_at).getTime() <= Date.now() + 30_000) {
      throw new Error(
        'Выбранное время старта уже наступило. Создайте скачки с новым временем.'
      );
    }

    const actorUserId = String(
      options.userId || row.creator_user_id || ''
    );

    const adminFree = Boolean(
      options.adminFree || isAdmin(actorUserId)
    );

    const paymentStatus = adminFree
      ? 'admin_free'
      : String(
    ...
    if (!adminFree && paymentStatus !== 'succeeded') {
      throw new Error(
        'Скачки можно подготовить к запуску только после успешной оплаты'
      );
    }
    ...
    if (!result.rows[0]) {
      const current = await getRaceById(id);

      if (current && ['active', 'running', 'finished'].includes(
      ...
      throw new Error(
        'Не удалось перевести скачки в режим ожидания запуска'
      );
    }
  ...
  async function addParticipant(raceId, userId, referrerId = null) {
    const id = normalizePositiveInt(raceId);
    const cleanUserId = String(userId || '').trim();
    const cleanReferrerId = String(referrerId || '').trim();

    if (!id || !/^\d+$/.test(cleanUserId)) {
      throw new Error('Некорректные данные участия');
    }

    const validReferrerId =
      /^\d+$/.test(cleanReferrerId) &&
      cleanReferrerId !== cleanUserId
        ? cleanReferrerId
        : null;
    ...
    try {
      ...
      // Блокировка строки скачек сериализует выдачу билетов
      // и защищает от двойного начисления одного реферала.
      const raceRes = await client.query(`
        SELECT *
        FROM horse_races
        WHERE id = $1
      ...
      if (
        !race ||
        !ACTIVE_RACE_STATUSES.has(String(race.status || '')) ||
        new Date(race.start_at).getTime() <= Date.now()
      ) {
      ...
      if (existingRes.rows[0]) {
        ...
        return {
          ...
          created: false,
          bonusAdded: false,
          bonusTicketNumber: null,
          referrerId: null,
          referrerBonusCount: 0,
          bonusLimitReached: false
        };
      }

      // Цвета новых людей балансируем только по основным билетам.
      // Реферальные билеты намеренно не меняют распределение новых лошадей.
      ...
      for (const row of countsRes.rows) {
        if (counts.has(row.horse_color)) {
          counts.set(row.horse_color, Number(row.count || 0));
        }
      }

      const minCount = Math.min(...counts.values());
      const leastUsed = HORSE_COLORS.filter(
        color => counts.get(color.code) === minCount
      );
      const selectedColor =
        leastUsed[crypto.randomInt(0, leastUsed.length)].code;

      let referrerMainTicket = null;
      ...
      if (validReferrerId) {
        ...
        referrerMainTicket = referrerRes.rows[0] || null;
      }
      ...
      for (let attempt = 0; attempt < 8 && !participant; attempt += 1) {
        ...
        try {
          const insertRes = await client.query(`
            ...
            VALUES ($1, $2, $3, $4, $5, true, 'main', NOW())
            RETURNING *
          `, [
            id,
            cleanUserId,
            referrerMainTicket ? validReferrerId : null,
            ticket,
            selectedColor
          ]);
      ...
      if (!participant) {
        throw new Error('Не удалось создать уникальный билет');
      }
      ...
      let bonusParticipant = null;

      if (referrerMainTicket && config.referralBonusLimit > 0) {
        const existingReferralRes = await client.query(`
          SELECT id
          FROM horse_race_referrals
          WHERE horse_race_id = $1
            AND referred_user_id = $2
          LIMIT 1
        `, [id, cleanUserId]);

        if (!existingReferralRes.rows[0]) {
          const bonusCountRes = await client.query(`
            SELECT COUNT(*)::int AS count
            FROM horse_race_participants
            WHERE horse_race_id = $1
              AND user_id = $2
          ...
          referrerBonusCount = Number(
            bonusCountRes.rows[0]?.count || 0
          );

          if (referrerBonusCount < config.referralBonusLimit) {
            for (
              let attempt = 0;
              attempt < 8 && !bonusParticipant;
              attempt += 1
            ) {
              bonusTicketNumber = generateTicketNumber();
              ...
              try {
                const bonusRes = await client.query(`
                  ...
                  VALUES ($1, $2, $3, $4, $5, true, 'referral_bonus', NOW())
                  RETURNING *
                `, [
                  id,
                  validReferrerId,
                  cleanUserId,
                  bonusTicketNumber,
                  referrerMainTicket.horse_color
                ]);

                bonusParticipant = bonusRes.rows[0];
              } catch (error) {
                if (error?.code !== '23505') throw error;
              }
            }
            ...
            if (!bonusParticipant) {
              throw new Error('Не удалось создать реферальный билет');
            }

            await client.query(`
              INSERT INTO horse_race_referrals (
                horse_race_id,
                referred_user_id,
                referrer_user_id,
                bonus_participant_id
              )
              VALUES ($1, $2, $3, $4)
              ON CONFLICT (horse_race_id, referred_user_id) DO NOTHING
            `, [
              id,
              cleanUserId,
              validReferrerId,
      ...
      await client.query(`
        INSERT INTO horse_race_pending_joins (
          horse_race_id,
          user_id,
          referrer_user_id,
          status,
        ...
        DO UPDATE SET
          referrer_user_id = COALESCE(
            horse_race_pending_joins.referrer_user_id,
            EXCLUDED.referrer_user_id
          ),
          status = 'completed',
          updated_at = NOW()
      `, [
      ...
      await scheduleRacePostUpdate(id, {
        reason: 'new_participant',
        delaySeconds: config.postUpdateDebounceSeconds
      }).catch(error => {
        console.warn(
          `Не удалось поставить обновление постов скачек ${id} в очередь:`,
          error.message
        );
      });
      ...
      return {
        ...
        bonusAdded,
        bonusTicketNumber:
          bonusParticipant?.ticket_number || bonusTicketNumber,
        bonusHorseColor:
          bonusParticipant?.horse_color ||
          referrerMainTicket?.horse_color ||
          null,
        referrerId:
          bonusAdded ? validReferrerId : null,
        referrerBonusCount,
        bonusLimitReached
      };
    } catch (error) {
  ...
  async function finalizeRaceWinners(raceId) {
    ...
    try {
      ...
      // Каждый билет даёт отдельный шанс, но один пользователь
      // не может забрать больше одного приза в одних скачках.
      const winnerUserIds = new Set(
        roundsRes.rows
          .filter(round => round.winner_user_id)
          .map(round => String(round.winner_user_id))
      );

      let remaining = secureShuffle(
        allParticipants.filter(
          ticket => !winnerUserIds.has(String(ticket.user_id))
        )
      );
      ...
      for (const round of roundsRes.rows) {
        if (
          ...
          round.result_json?.winner_selected === true
        ) {
          continue;
        }
        ...
        if (winner) {
          const winnerUserId = String(winner.user_id);
          winnerUserIds.add(winnerUserId);

          // Убираем все остальные билеты уже победившего пользователя,
          // чтобы следующий приз достался другому человеку.
          remaining = remaining.filter(
            ticket => String(ticket.user_id) !== winnerUserId
          );
        }
        await client.query(`
  ...
  async function refreshRaceLifecycle(raceId = null) {
    ...
    // Если Render перезапустился во время публикации,
    // возвращаем скачки в режим ожидания запуска.
    await pool.query(`
      UPDATE horse_races
    ...
    // Неопубликованные скачки не должны стартовать сами.
    await pool.query(`
      UPDATE horse_races
      SET
        status = 'cancelled',
    ...
    const params = [];
    const where = normalizedRaceId
      ? 'AND hr.id = $1'
      : '';

    if (normalizedRaceId) {
      params.push(normalizedRaceId);
    }
    ...
    await pool.query(`
      ...
      SET
        status = CASE
          WHEN NOW() >= starts_at +
            (duration_ms::text || ' milliseconds')::interval
            THEN 'finished'
          WHEN NOW() >= starts_at
            THEN 'running'
          ELSE 'prepared'
  ...
  async function showRaceJoinStart(target, userId, raceId, referrerId = null) {
    const race = await getRaceById(raceId, userId);
    if (!race) {
    ...
    if (race.own_ticket_number) {
      ...
      const referralLink = buildRaceReferralLink(race.id, userId);
      const bonusCount = await getRaceReferralBonusCount(race.id, userId);
      ...
      await sendMessage?.(
        ...
        [
          ...
          '',
          `Вы уже участвуете. Ваш основной билет: **№${race.own_ticket_number}**`,
          `Цвет вашей лошади: **${color?.label || race.own_horse_color}**`,
          `Старт: **${formatRaceDate(race.start_at)}**`,
          '',
          `🎁 Реферальных билетов: **${bonusCount}/${config.referralBonusLimit}**`,
          referralLink ? '🔗 Ваша пригласительная ссылка:' : '',
          referralLink || '',
          '',
          'За каждого нового участника по вашей ссылке вы получаете +1 билет того же цвета.'
        ].filter(Boolean).join('\n'),
    ...
    await pool.query(`
      ...
      DO UPDATE SET
        ...
        status = 'pending',
        updated_at = NOW()
    `, [race.id, String(userId), pendingReferrerId]);
    ...
    lines.push(
      '',
      'После подписки нажмите кнопку проверки. Билет и цвет лошади сохранятся до конца скачек.',
      '',
      `После участия вы получите личную ссылку. За каждого нового участника по ней — +1 билет того же цвета, максимум ${config.referralBonusLimit}.`
    );
  ...
  function formatHorseRaceStatus(status) {
  ...
  function buildQuickRaceStartDate(days) {
    const safeDays = [3, 5].includes(Number(days))
      ? Number(days)
      : null;
  ...
  async function sendRaceStartSelectedNotice(target, startAt, presetDays = null) {
    const prefix = presetDays
      ? `Вы выбрали срок **${presetDays} дней**.`
      : 'Время старта сохранено.';
    ...
    return sendMessage?.(
      ...
      [
        `✅ ${prefix}`,
        '',
        `🏁 Скачки начнутся: **${formatRaceDate(startAt)}**`,
        '',
        'До этого времени пользователи смогут подписываться, получать билеты и приглашать друзей.',
        'Именно в указанное время в Mini App начнётся движение лошадей.'
      ].join('\n')
    );
  }
  ...
  function buildHorseMenuText(userId) {
    return [
      '🐎 **Скачки**',
      '',
      `Создание скачек стоит **${config.racePriceRub} ₽**.`,
      '',
      'После оплаты или бесплатного создания скачки сохраняются без публикации.',
      'Сначала пригласите нужные каналы, затем нажмите **🚀 Запустить сейчас**.',
      'После запуска пост появится в 🍟 РОЗЫГРЫШ И ТОЧКА и каналах с размещением.',
      '',
      'Выбранное время — это время начала самой гонки в Mini App.',
      '',
      isAdmin(userId)
  ...
  async function sendHorseMenu(target, userId) {
    const keyboard = [
      [{
        text: isAdmin(userId)
          ? '➕ Создать скачки бесплатно'
          : `➕ Создать скачки — ${config.racePriceRub} ₽`,
        callback_data: 'horse_create_start'
      }],
      [{ text: '📁 Мои скачки', callback_data: 'horse_my_races' }],
    ...
    if (isAdmin(userId)) {
      keyboard.push([{
        text: '👑 Активные скачки',
        callback_data: 'horse_admin_active'
      }]);
    }
  ...
  async function getCreatorActiveRaceCount(userId) {
    const res = await pool.query(`
      ...
      WHERE creator_user_id = $1
        AND status IN ('draft', 'ready', 'publishing', 'scheduled', 'active', 'running')
  ...
  async function sendRaceStartPrompt(target, userId, data = {}) {
    ...
    return sendMessage?.(
      target,
      horseStepText(5, 'Время старта', [
        'Выберите быстрый срок или введите дату и время вручную по МСК:',
        '',
        '`2026-06-18 20:00`',
        '`18.06.2026 20:00`',
        '',
        '⚠️ Это время начала самой гонки.',
        'До него участники будут получать билеты, а в указанное время лошади стартуют в Mini App.',
        '',
      ...
      [
        [
          {
            text: '📅 Через 3 дня',
            callback_data: 'horse_start_quick:3'
          },
          {
            text: '📅 Через 5 дней',
            callback_data: 'horse_start_quick:5'
          }
        ],
        [{
          text: '⬅️ Назад к фото',
          callback_data: 'horse_back:photo'
        }],
        [{
          text: '❌ Отмена',
          callback_data: 'horse_cancel'
        }]
      ]
    );
  }
  ...
  async function sendRaceChannelSelection(target, userId, data = {}) {
    ...
    const lines = [
      horseStepText(6, 'Каналы', [
        ...
        'После создания посты не публикуются автоматически.',
        'Сначала можно пригласить другие каналы, затем организатор нажмёт «🚀 Запустить сейчас».',
        'После запуска пост появится в выбранных каналах и автоматически в 🍟 РОЗЫГРЫШ И ТОЧКА.'
      ])
    ];
  ...
  async function sendRacePreview(target, userId, data = {}) {
    const channels = await resolveSelectedChannels(
      userId,
      data.channel_ids
    );
    ...
    const text = [
      ...
      '',
      `🏁 Сама гонка начнётся: **${formatRaceDate(data.start_at)}**`,
      '',
      `💳 Стоимость создания: **${isAdmin(userId) ? '0' : config.racePriceRub} ₽**`,
      '',
      'После создания скачки будут сохранены, но **не опубликуются автоматически**.',
      'Вы сможете пригласить дополнительные каналы и затем нажать **🚀 Запустить сейчас**.',
      isAdmin(userId)
        ? '👑 Для администратора оплата не требуется.'
        : 'После подтверждения бот попросит email для чека.'
    ].join('\n');
      ...
      [
        [{
          text: isAdmin(userId)
            ...
            : `💳 Перейти к оплате ${config.racePriceRub} ₽`,
          callback_data: 'horse_preview_confirm'
        }],
        [{
          text: '🔄 Начать заново',
          callback_data: 'horse_create_start'
  ...
  async function updateRacePublishedPosts(raceId, options = {}) {
    if (typeof editMaxMessageText !== 'function') {
    ...
    for (const post of posts.rows) {
      if (!force && Number(post.participants_count || 0) === count) {
        skipped += 1;
        continue;
      }
  ...
  async function scheduleRacePostUpdate(raceId, options = {}) {
    const id = normalizePositiveInt(raceId);
    if (!id) return false;

    const delaySeconds = Math.max(
      0,
      Number(options.delaySeconds ?? config.postUpdateDebounceSeconds)
    );

    const force = Boolean(options.force);
    const reason = cleanText(options.reason || 'participants_changed', 120);

    await pool.query(`
      INSERT INTO horse_race_post_updates (
        horse_race_id,
        scheduled_at,
        force,
        reason,
      ...
      ON CONFLICT (horse_race_id)
      DO UPDATE SET
        scheduled_at = LEAST(
          horse_race_post_updates.scheduled_at,
          EXCLUDED.scheduled_at
        ),
        force = horse_race_post_updates.force OR EXCLUDED.force,
        reason = EXCLUDED.reason,
        status = 'pending',
        version = horse_race_post_updates.version + 1,
        last_error = NULL,
        updated_at = NOW()
    `, [id, delaySeconds, force, reason || null]);

    return true;
  }

  async function processRacePostUpdateQueue(limit = 20) {
    await pool.query(`
      UPDATE horse_race_post_updates
      SET
        status = 'pending',
    ...
    const claimed = await pool.query(`
      ...
      UPDATE horse_race_post_updates q
      SET status = 'processing', updated_at = NOW()
      FROM picked
      WHERE q.horse_race_id = picked.horse_race_id
      RETURNING q.*
    `, [Math.max(1, Number(limit || 20))]);

    for (const item of claimed.rows) {
      try {
        const result = await updateRacePublishedPosts(
          item.horse_race_id,
          { force: Boolean(item.force) }
        );

        await pool.query(`
          DELETE FROM horse_race_post_updates
          WHERE horse_race_id = $1
            AND version = $2
            AND status = 'processing'
        `, [item.horse_race_id, item.version]);

        if (result.updated || result.failed) {
          console.log('🐎 Посты скачек обновлены пачкой:', {
            horse_race_id: Number(item.horse_race_id),
            reason: item.reason || 'participants_changed',
        ...
        console.warn(
          `Не удалось обновить посты скачек ${item.horse_race_id} из очереди:`,
          error.message
        );
      }
    }
  ...
  async function updateAllActiveRacePublishedPosts() {
    const races = await pool.query(`
      SELECT DISTINCT hr.id
      FROM horse_races hr
      JOIN horse_race_posts hrp
        ON hrp.horse_race_id = hr.id
      WHERE hr.status IN ('scheduled', 'active', 'running')
        AND hr.start_at > NOW()
        AND hrp.status = 'published'
        AND hrp.message_id IS NOT NULL
      ORDER BY hr.id ASC
      LIMIT 100
    `);

    for (const row of races.rows) {
      await updateRacePublishedPosts(row.id).catch(error => {
        console.warn(
          `Не удалось выполнить фоновую сверку постов скачек ${row.id}:`,
          error.message
        );
      });
    }

    return races.rowCount || 0;
  }
  ...
  async function activateAndPublishRace(raceId, options = {}) {
    ...
    const id = normalizePositiveInt(raceId);
    const requesterUserId = String(options.userId || '').trim();
    const race = await getRaceById(id);
    ...
    if (
      ...
      String(race.creator_user_id) !== requesterUserId
    ) {
      throw new Error(
        'Запустить скачки может только организатор или администратор'
      );
    }

    if (['active', 'running', 'finished'].includes(
      String(race.status || '')
    )) {
      throw new Error('Скачки уже были запущены');
    }

    if (String(race.status || '') !== 'ready') {
      throw new Error(
        'Скачки ещё не готовы к запуску. Сначала завершите создание или оплату.'
      );
    }

    if (new Date(race.start_at).getTime() <= Date.now() + 30_000) {
      throw new Error(
        'До старта осталось меньше 30 секунд. Создайте скачки с новым временем старта.'
      );
    }

    const paymentStatus = String(race.payment_status || '');

    if (
      !isAdmin(requesterUserId) &&
      !['succeeded', 'admin_free'].includes(paymentStatus)
    ) {
      throw new Error('Оплата скачек ещё не подтверждена');
    }
    ...
    if (!claimed.rows[0]) {
      throw new Error(
        'Скачки уже запускаются в другом процессе. Попробуйте открыть список ещё раз.'
      );
    }
    ...
    try {
      ...
      // General обязателен. Пока он не опубликован, другие каналы не трогаем.
      const generalResult = await publishRaceToChannel(
        id,
      ...
      results.push({
        channelId: String(config.generalChannelId),
        ...generalResult
      });
      ...
      for (const channel of channels) {
        if (!channel.publishPost) continue;
        if (String(channel.channelId) === String(config.generalChannelId)) continue;

        try {
          const result = await publishRaceToChannel(
            id,
            channel.channelId,
            { isGeneral: false }
          );
      ...
      // Передаём ID создателя, чтобы запуск администратором не менял
      // succeeded на admin_free у уже оплаченных скачек.
      const activated = await activateRace(id, {
        userId: String(race.creator_user_id || requesterUserId),
        adminFree: paymentStatus === 'admin_free',
        paymentStatus,
        paymentId: race.payment_id || null,
        publishedInGeneral: true,
      ...
      if (secondaryFailures.length) {
        const details = secondaryFailures
          .map(item => `${item.title || item.channelId}: ${item.error}`)
          .join('; ');

        await notifyAdmins?.(
          `⚠️ Скачки #${id} запущены, но часть каналов не опубликована: ${cleanText(details, 1200)}`
        );
      }
      ...
      await notifyAdmins?.(
        `⚠️ Не удалось запустить скачки #${id}: ${cleanText(error.message, 800)}`
      );
  ...
  async function applyYooKassaPayment(payment) {
    ...
    if (String(metadata.product || '') !== config.productCode) {
      return false;
    }
      ...
      DO UPDATE SET
        ...
        paid_at = CASE
          WHEN EXCLUDED.status = 'succeeded'
            THEN COALESCE(horse_race_payments.paid_at, NOW())
          ELSE horse_race_payments.paid_at
        END,
        updated_at = NOW()
    `, [
      paymentId,
      ...
      String(payment?.amount?.value || config.racePriceRub),
      String(payment?.amount?.currency || 'RUB'),
      metadata.receipt_email || null,
    ...
    try {
      const readyRace = await markRaceReady(raceId, {
        userId,
      ...
      await sendMessage?.(
        userId,
        [
          '✅ **Оплата прошла. Скачки подготовлены.**',
          '',
          `🐎 Название: **${cleanText(readyRace.title, 180)}**`,
          `🏁 Гонка начнётся: **${formatRaceDate(readyRace.start_at)}**`,
          '',
          'Скачки пока **не опубликованы** и не видны участникам.',
          'Сначала пригласите нужные каналы.',
          'Когда набор каналов завершён, нажмите **🚀 Запустить сейчас**.'
        ].join('\n'),
        [
          [{
            text: '⚙️ Управление каналами',
            callback_data: `horse_channel_manage:${raceId}`
          }],
          [{
            text: '🤝 Пригласить каналы',
            callback_data: `horse_collab_link:${raceId}`
          }],
          [{
            text: '🚀 Запустить сейчас',
            callback_data: `horse_launch_now:${raceId}`
          }],
          [{
            text: '📊 Статистика',
            callback_data: `horse_stats:${raceId}`
          }]
        ]
      ).catch(() => {});
    } catch (error) {
      ...
      await notifyAdmins?.(
        `⚠️ Ошибка подготовки оплаченных скачек #${raceId}, payment ${paymentId}: ${cleanText(error.message, 800)}`
      );

      await sendMessage?.(
        userId,
        '✅ Оплата прошла, но скачки не удалось подготовить автоматически. Администратор уже получил уведомление.'
      ).catch(() => {});
  ...
  async function handleSessionMessage(message, session = null) {
    ...
    if (state === 'horse_create_start_at') {
      ...
      await sendRaceStartSelectedNotice(
        target,
        startAt
      );
    ...
    if (state === 'horse_create_email') {
      try {
        ...
        await sendMessage?.(target, [
          ...
          '',
          'После успешной оплаты скачки сохранятся без публикации. Пригласите каналы и затем нажмите «🚀 Запустить сейчас».'
        ].join('\n'), [[{ text: '💳 Оплатить', url }]]);
      } catch (error) {
  ...
  async function sendCreatorRaceCollabList(target, userId) {
    ...
    if (!res.rows.length) {
      return sendMessage?.(
        target,
        [
          'У вас нет скачек в режиме ожидания запуска.',
          '',
          'Сначала создайте или оплатите скачки. После этого они появятся здесь, и вы сможете пригласить каналы до публикации.'
        ].join('\n'),
        [
          [{
            text: '📁 Мои скачки',
    ...
    const lines = [
      '🤝 **Выберите скачки для приглашения каналов:**',
      '',
      'Каналы можно добавлять только до нажатия «🚀 Запустить сейчас».',
      ''
    ];
    ...
    for (const race of res.rows) {
      lines.push(
        `• #${race.id} — **${cleanText(race.title, 120) || 'Скачки'}** | гонка ${formatRaceDate(race.start_at)}`
      );
  ...
  async function sendMyHorseRaces(target, userId) {
    const res = await pool.query(`
      SELECT
        hr.*,
        COALESCE(pc.participants_count, 0)::int AS participants_count,
        COALESCE(tc.tickets_count, 0)::int AS tickets_count,
        COALESCE(cc.channels_count, 0)::int AS channels_count
      FROM horse_races hr
      LEFT JOIN LATERAL (
        SELECT COUNT(DISTINCT p.user_id)::int AS participants_count
        FROM horse_race_participants p
        WHERE p.horse_race_id = hr.id
          AND p.is_valid = true
          AND p.ticket_type = 'main'
      ) pc ON true
      LEFT JOIN LATERAL (
      ...
      LEFT JOIN LATERAL (
        SELECT COUNT(*)::int AS channels_count
        FROM horse_race_channels c
        WHERE c.horse_race_id = hr.id
      ) cc ON true
      WHERE hr.creator_user_id = $1
      ORDER BY
        CASE
          WHEN hr.status = 'ready' THEN 0
          WHEN hr.status IN ('active', 'scheduled') THEN 1
          WHEN hr.status = 'running' THEN 2
          WHEN hr.status = 'finished' THEN 3
          ELSE 4
        END,
        hr.created_at DESC,
        hr.id DESC
    ...
    const lines = [
      '🐎 **Мои скачки**',
      '',
      'Статус **«Готово к запуску»** означает, что посты ещё не опубликованы.',
      ''
    ];
    ...
    for (const race of res.rows) {
      lines.push(
        `#${race.id} | **${cleanText(race.title, 100) || 'Скачки'}**`,
        `Статус: **${formatHorseRaceStatus(race.status)}**`,
        `🏁 Гонка: **${formatRaceDate(race.start_at)}**`,
        `👥 Участников: **${race.participants_count}** | 🎟 Билетов: **${race.tickets_count}** | 📢 Каналов: **${race.channels_count}**`,
        ''
      );

      keyboard.push([{
        text: `📊 Статистика #${race.id}`,
        callback_data: `horse_stats:${race.id}`
      }]);

      if (String(race.status) === 'ready') {
        keyboard.push([{
          text: `⚙️ Управление каналами #${race.id}`,
          callback_data: `horse_channel_manage:${race.id}`
        }]);

        keyboard.push([
          {
            text: `🤝 Добавить каналы #${race.id}`,
            callback_data: `horse_collab_link:${race.id}`
          },
          {
            text: `🚀 Запустить #${race.id}`,
            callback_data: `horse_launch_now:${race.id}`
          }
        ]);
      }

      if (['active', 'scheduled', 'running', 'finished'].includes(
        String(race.status)
      )) {
        keyboard.push([{
          text: `🏇 Открыть #${race.id}`,
          url: getLaunchUrl(`race_${race.id}`)
        }]);
      }
    }
  ...
  async function sendMyHorseCollabRaces(target, userId) {
    const res = await pool.query(`
      SELECT
        hr.id,
        hr.title,
        ...
        hr.created_at,
        hr.creator_user_id,
        COUNT(DISTINCT hrc.channel_id)::int AS channels_count,
        STRING_AGG(
          DISTINCT (
            COALESCE(hrc.channel_title, hrc.channel_id::text) ||
            CASE
              WHEN hrc.publish_post
                THEN ' — с размещением'
              ELSE ' — без размещения'
            END
          ),
    ...
    if (!res.rows.length) {
      return sendMessage?.(
        target,
        [
          '🤝 **Скачки: где участвуют мои каналы**',
          '',
          'Пока нет скачек других организаторов, к которым подключены ваши каналы.'
        ].join('\n'),
        [[{
          text: '⬅️ К разделам',
          callback_data: 'my_raffles'
    ...
    for (const race of res.rows) {
      lines.push(
        ...
        `Статус: **${formatHorseRaceStatus(race.status)}**`,
        `🏁 Гонка: **${formatRaceDate(race.start_at)}**`,
        `Ваших каналов: **${race.channels_count}**`,
        cleanText(race.channels_text, 1000) || 'Каналы не указаны',
        ''
      );
  ...
  async function canViewHorseRaceStats(race, userId) {
    const cleanUserId = String(userId || '').trim();

    if (!race || !cleanUserId) return false;
    if (isAdmin(cleanUserId)) return true;
  ...
  async function sendHorseRaceStats(target, raceId, requesterUserId) {
    const id = normalizePositiveInt(raceId);
    const race = await getRaceById(id);

    if (!race) {
      return sendMessage?.(target, 'Скачки не найдены.');
    }

    if (!(await canViewHorseRaceStats(race, requesterUserId))) {
      return sendMessage?.(
        target,
        '⛔ Статистика доступна только организатору, владельцам подключённых каналов и администраторам.'
      );
    }
    ...
    const [
      ...
      pool.query(`
        SELECT
          COUNT(*) FILTER (WHERE is_valid = true)::int AS valid_tickets,
          COUNT(*) FILTER (
            WHERE is_valid = true
              AND ticket_type = 'main'
          )::int AS main_tickets,
          COUNT(*) FILTER (
            WHERE is_valid = true
              AND ticket_type = 'referral_bonus'
          )::int AS referral_tickets,
          COUNT(*) FILTER (WHERE is_valid = false)::int AS invalid_tickets
        FROM horse_race_participants
        WHERE horse_race_id = $1
      `, [id]),
      pool.query(`
        SELECT
          ...
          hrc.is_required,
          hrc.publish_post,
          COALESCE(stats.subscriptions, 0)::int AS subscriptions,
          COALESCE(stats.unsubscriptions, 0)::int AS unsubscriptions,
          COALESCE(stats.net_growth, 0)::int AS net_growth,
          COALESCE(states.current_subscribers, 0)::int AS current_subscribers,
          COALESCE(states.tracked_users, 0)::int AS tracked_users
        FROM horse_race_channels hrc
        LEFT JOIN LATERAL (
          SELECT
            COUNT(*) FILTER (WHERE event.delta > 0)::int AS subscriptions,
            COUNT(*) FILTER (WHERE event.delta < 0)::int AS unsubscriptions,
            COALESCE(SUM(event.delta), 0)::int AS net_growth
          FROM horse_race_channel_subscription_events event
          WHERE event.horse_race_id = hrc.horse_race_id
            AND event.channel_id = hrc.channel_id
        ) stats ON true
        LEFT JOIN LATERAL (
          SELECT
            COUNT(*) FILTER (WHERE state.is_subscribed = true)::int AS current_subscribers,
            COUNT(*)::int AS tracked_users
          FROM horse_race_channel_subscription_states state
          WHERE state.horse_race_id = hrc.horse_race_id
            AND state.channel_id = hrc.channel_id
        ) states ON true
        WHERE hrc.horse_race_id = $1
        ORDER BY hrc.id ASC
      `, [id]),
      pool.query(`
        SELECT
        ...
        FROM horse_race_rounds hrr
        LEFT JOIN users u
          ON u.max_user_id = hrr.winner_user_id
        WHERE hrr.horse_race_id = $1
        ORDER BY hrr.prize_index ASC
      `, [id]),
      pool.query(`
        SELECT COUNT(*)::int AS count
        FROM horse_race_pending_joins
        WHERE horse_race_id = $1
          AND status = 'pending'
      `, [id]),
      pool.query(`
    ...
    const participants =
      Number(participantRes.rows[0]?.count || 0);

    const tickets = ticketRes.rows[0] || {};
    const channels = channelRes.rows;
    const winners = winnerRes.rows;
    const pending = Number(
      pendingRes.rows[0]?.count || 0
    );
    const posts = postRes.rows[0] || {};

    const withPlacement =
      channels.filter(ch => ch.publish_post).length;

    const withoutPlacement =
      channels.filter(ch => !ch.publish_post).length;

    const text = [
      `📊 **Статистика скачек #${id}**`,
      '',
      `Название: **${cleanText(race.title, 180) || 'Скачки'}**`,
      `Статус: **${formatHorseRaceStatus(race.status)}**`,
      `🏁 Начало гонки: **${formatRaceDate(race.start_at)}**`,
      `📣 Публикация: **${race.publish_at ? formatRaceDate(race.publish_at) : 'ещё не запущена'}**`,
      '',
      `👥 Участников: **${participants}**`,
      `🎟 Действительных билетов: **${Number(tickets.valid_tickets || 0)}**`,
      `├ Основных: **${Number(tickets.main_tickets || 0)}**`,
      `├ Реферальных: **${Number(tickets.referral_tickets || 0)}**`,
      `└ Недействительных: **${Number(tickets.invalid_tickets || 0)}**`,
      `⏳ Ожидают завершения проверки: **${pending}**`,
      '',
      `📢 Каналов: **${channels.length}**`,
      `├ С размещением: **${withPlacement}**`,
      `└ Без размещения: **${withoutPlacement}**`,
      `📝 Опубликованных постов: **${Number(posts.published_posts || 0)}**`,
      Number(posts.failed_posts || 0)
        ? `⚠️ Ошибок публикации: **${Number(posts.failed_posts || 0)}**`
        : '',
      '',
      '📈 **Подписки и отписки по каналам:**',
      ...(channels.length
        ? channels.flatMap((channel, index) => {
            const title =
              cleanText(channel.channel_title, 180) ||
              String(channel.channel_id);

            const link =
              normalizePublicHttpUrl(channel.channel_link);

            const formattedTitle = link
              ? `[${escapeMarkdownText(title)}](${link})`
              : escapeMarkdownText(title);

            const subscriptions = Number(channel.subscriptions || 0);
            const unsubscriptions = Number(channel.unsubscriptions || 0);
            const netGrowth = Number(channel.net_growth || 0);
            const currentSubscribers = Number(channel.current_subscribers || 0);
            const trackedUsers = Number(channel.tracked_users || 0);
            const signedNet = netGrowth > 0 ? `+${netGrowth}` : String(netGrowth);

            return [
              `${index + 1}. ${formattedTitle} — ${
                channel.publish_post
                  ? 'с размещением 📣'
                  : 'без размещения 🙈'
              }`,
      ...
      ...(winners.length
        ? [
            ...
            ...winners.map(row => {
              const name =
                [
                  cleanText(row.first_name, 100),
                  cleanText(row.last_name, 100)
                ].filter(Boolean).join(' ') ||
                (row.username
              ...
              const color =
                serializeHorseColor(row.winner_color);

              return row.winner_user_id
                ? `${row.prize_index}. ${cleanText(row.prize_text, 300) || `Приз ${row.prize_index}`} — ${escapeMarkdownText(name)}, билет №${row.winner_ticket_number}, ${color?.label || row.winner_color}`
                : `${row.prize_index}. ${cleanText(row.prize_text, 300) || `Приз ${row.prize_index}`} — победитель ещё не определён`;
            })
          ]
        : [])
    ].filter(Boolean).join('\n');
    ...
    const keyboard = [];
    const isOwner =
      String(race.creator_user_id || '') ===
      String(requesterUserId || '');

    if (String(race.status) === 'ready') {
      keyboard.push([{
        text: '⚙️ Управление каналами',
        callback_data: `horse_channel_manage:${id}`
      }]);

      if (isOwner || isAdmin(requesterUserId)) {
        keyboard.push([
          {
            text: '🤝 Пригласить каналы',
            callback_data: `horse_collab_link:${id}`
          ...
          {
            text: '🚀 Запустить сейчас',
            callback_data: `horse_launch_now:${id}`
          }
        ]);
      }
    }
  ...
  async function sendAdminRaces(target, userId) {
    ...
    const res = await pool.query(`
      SELECT
        hr.*,
        COALESCE(pc.count, 0)::int AS participants_count
      FROM horse_races hr
      LEFT JOIN LATERAL (
      ...
      ORDER BY
        CASE
          WHEN hr.status = 'ready' THEN 0
          WHEN hr.status = 'publishing' THEN 1
          ELSE 2
        END,
        hr.start_at ASC,
        hr.id DESC
    ...
    for (const race of res.rows) {
      lines.push(
        `#${race.id} | ${cleanText(race.title, 100)} | ${formatHorseRaceStatus(race.status)} | 👥 ${race.participants_count} | ${formatRaceDate(race.start_at)}`
      );
      ...
      if (
        !race.published_in_general &&
        ['scheduled', 'active', 'running'].includes(String(race.status || ''))
      ) {
        row.push({
          text: `📣 General #${race.id}`,
          callback_data: `horse_admin_publish:${race.id}`
        });
      }

      if (
        config.testMode &&
        String(race.payment_status) === 'pending'
      ) {
        row.push({
          text: `🧪 #${race.id}`,
          callback_data: `horse_admin_testpay:${race.id}`
        });
      }
      ...
      if (
        ['scheduled', 'active'].includes(String(race.status || '')) &&
        new Date(race.start_at).getTime() > Date.now()
      ) {
        keyboard.push([{
          text: `📨 В бот всем #${race.id}`,
          callback_data: `horse_admin_broadcast_preview:${race.id}`
        }]);
      }

      keyboard.push([{
        text: `❌ Отменить #${race.id}`,
        callback_data: `horse_admin_cancel:${race.id}`
      }]);
    }
  ...
  async function buildAdminRaceBroadcastPayload(raceId) {
    const id = normalizePositiveInt(raceId);
    if (!id) return { ok: false, error: 'Некорректный ID скачек.' };

    const race = await getRaceById(id);
    if (!race) return { ok: false, error: 'Скачки не найдены.' };

    if (!['scheduled', 'active'].includes(String(race.status || ''))) {
      return {
        ok: false,
        error: 'Рассылку можно отправить только до начала активных скачек.'
      };
    }

    const startAtMs = new Date(race.start_at).getTime();
    if (!Number.isFinite(startAtMs) || startAtMs <= Date.now()) {
      return { ok: false, error: 'Скачки уже начались или время старта некорректно.' };
    }

    const [participantsCount, channels] = await Promise.all([
      getRaceParticipantsCount(id),
      getRaceChannels(id)
    ]);
    ...
    return {
      ok: true,
      race,
      text: buildRacePostText(race, participantsCount, channels),
      keyboard: buildRacePostKeyboard(id),
      attachments: getRacePhotoAttachments(race),
      sourceType: 'horse_race',
      sourceId: id,
      sourceLabel: `Скачки #${id}: ${cleanText(race.title, 180) || 'Скачки'}`,
      expiresAt: race.start_at
    };
  }

  async function sendAdminRaceBroadcastPreview(target, userId, raceId) {
    if (!isAdmin(userId)) {
      return sendMessage?.(target, '⛔ Нет доступа.');
    }

    const payload = await buildAdminRaceBroadcastPayload(raceId);

    if (!payload.ok) {
      return sendMessage?.(
        target,
        `⚠️ ${payload.error}`,
        [[{ text: '⬅️ К активным скачкам', callback_data: 'horse_admin_active' }]]
      );
    }
    ...
    return sendMessage?.(
      target,
      [
        '📨 **Предпросмотр рассылки активных скачек**',
        '',
      ...
      [
        ...
        [{
          ...
          callback_data: `horse_admin_broadcast_send:${payload.sourceId}`
        }],
        [{ text: '❌ Отмена', callback_data: 'horse_admin_active' }]
      ],
      payload.attachments
    );
  }

  async function confirmAdminRaceBroadcast(target, userId, raceId) {
    if (!isAdmin(userId)) {
      return sendMessage?.(target, '⛔ Нет доступа.');
    }

    if (typeof queueAdminBroadcast !== 'function') {
      return sendMessage?.(target, '⚠️ Очередь рассылок недоступна. Проверьте подключение модуля скачек в bot.js.');
    }
    ...
    return queueAdminBroadcast({
      target,
      adminUserId: userId,
      text: payload.text,
      keyboard: payload.keyboard,
      attachments: payload.attachments,
      broadcastType: 'horse_race',
  ...
  async function handleStartPayload({ target, from, payload }) {
    ...
    const horseReferralMatch = normalized.match(
      /^horse_ref_(\d+)_(\d+)$/
    );

    if (horseReferralMatch) {
      const raceId = normalizePositiveInt(horseReferralMatch[1]);
      const referrerId = String(horseReferralMatch[2] || '').trim();

      if (!raceId || !/^\d+$/.test(referrerId)) return false;

      await showRaceJoinStart(
        target,
        from.id,
        raceId,
        referrerId
    ...
    if (normalized.startsWith('horse_join_')) {
      const raceId = normalizePositiveInt(
        normalized.replace(/^horse_join_/, '')
      );
  ...
  async function handleCallback(cb) {
    ...
    if (data.startsWith('horse_start_quick:')) {
      const days = Number(data.split(':')[1]);

      if (![3, 5].includes(days)) {
        await answerMaxCallback?.(
          cb.id,
          'Доступны только варианты 3 или 5 дней'
        );
        return true;
      }
      ...
      if (session?.state !== 'horse_create_start_at') {
        await answerMaxCallback?.(
          cb.id,
          'Сначала откройте шаг выбора времени'
        );
        return true;
      }
      ...
      const startAt = buildQuickRaceStartDate(days);
      ...
      draft.start_at = startAt.toISOString();
      draft.channel_ids = Array.isArray(draft.channel_ids)
        ? draft.channel_ids
        : [];

      await answerMaxCallback?.(
        cb.id,
        `✅ Выбрано: через ${days} дней`
      );
      ...
      await sendRaceChannelSelection(
        target,
        userId,
        draft
      );
    ...
    if (data === 'horse_my_races') {
      await sendMyHorseRaces(target, userId);
      return true;
    }

    if (data === 'horse_my_collab') {
      await sendMyHorseCollabRaces(target, userId);
      return true;
    }


    if (data.startsWith('horse_channel_manage:')) {
      const raceId = normalizePositiveInt(
        data.split(':')[1]
      );

      try {
        await sendRaceChannelManagement(
          target,
          userId,
          raceId
        );
    ...
    if (data.startsWith('horse_channel_remove_ask:')) {
      const parts = data.split(':');
      const raceId = normalizePositiveInt(parts[1]);
      const channelId = normalizeHorseChannelId(parts[2]);
      ...
      try {
        ...
        const canDelete =
          context.owner ||
          context.admin ||
          String(channel.ownerUserId || '') === userId;
        ...
        await sendMessage?.(
          target,
          [
            '⚠️ **Удалить канал из скачек?**',
            '',
            `🐎 #${raceId} — **${cleanText(context.race.title, 180) || 'Скачки'}**`,
            `📢 Канал: **${cleanText(channel.title, 180) || channel.channelId}**`,
            '',
            'Канал перестанет быть обязательным для подписки и не получит пост после запуска.',
            'Сам канал в MAX удалён не будет.'
          ].join('\n'),
          [
            [{
              text: '🗑 Да, удалить',
              callback_data: `horse_channel_remove_confirm:${raceId}:${channelId}`
            }],
            [{
              text: '⬅️ Отмена',
              callback_data: `horse_channel_manage:${raceId}`
    ...
    if (data.startsWith('horse_channel_remove_confirm:')) {
      const parts = data.split(':');
      const raceId = normalizePositiveInt(parts[1]);
      const channelId = normalizeHorseChannelId(parts[2]);
      ...
      try {
        ...
        const result = await removeRaceChannelBeforePublish(
          raceId,
          channelId,
          userId
        );
        const channelTitle =
          cleanText(result.channel.channel_title, 180) ||
          String(result.channel.channel_id);

        if (result.canManageRaceAfterDelete) {
          await sendRaceChannelManagement(
            target,
            userId,
            raceId,
            `✅ Канал **${escapeMarkdownText(channelTitle)}** удалён из скачек.`
          );
        } else {
          await sendMessage?.(
            target,
            [
              `✅ Канал **${escapeMarkdownText(channelTitle)}** удалён из скачек.`,
              '',
              'Он больше не является обязательным и не получит публикацию после запуска.'
            ].join('\n'),
            [[{
              text: '⬅️ Где участвуют мои каналы',
        ...
        await sendMessage?.(
          target,
          `⚠️ Не удалось удалить канал: ${cleanText(error.message, 500)}`
        );
      }
    ...
    if (data.startsWith('horse_stats:')) {
      const raceId = normalizePositiveInt(
        data.split(':')[1]
      );

      await sendHorseRaceStats(
        target,
        raceId,
        userId
      );
    ...
    if (data.startsWith('horse_launch_now:')) {
      const raceId = normalizePositiveInt(
        data.split(':')[1]
      );
      ...
      try {
        ...
        const launched = await activateAndPublishRace(
          raceId,
          { userId }
        );

        const failedChannels = Array.isArray(launched.secondaryFailures)
          ? launched.secondaryFailures
          : [];

        await sendMessage?.(
          target,
          [
            '🚀 **Скачки запущены и опубликованы!**',
            '',
            `🐎 ${cleanText(launched.race.title, 180) || `Скачки #${raceId}`}`,
            `🏁 Гонка начнётся: **${formatRaceDate(launched.race.start_at)}**`,
            '',
            failedChannels.length
              ? `Пост опубликован в **${config.generalChannelName}**. В ${failedChannels.length} подключённых каналах публикация не удалась — администратор получил уведомление.`
              : `Пост опубликован в **${config.generalChannelName}** и во всех подключённых каналах с размещением.`,
            'Новые каналы после запуска уже не добавляются.'
          ].join('\n'),
          [
            [{
              text: '🏇 Открыть скачки',
              url: getLaunchUrl(`race_${raceId}`)
            }],
            [{
              text: '📊 Статистика',
              callback_data: `horse_stats:${raceId}`
        ...
        await sendMessage?.(
          target,
          `⚠️ Не удалось запустить скачки: ${cleanText(error.message, 700)}`
        );
      }
    ...
    if (data === 'horse_preview_confirm') {
      ...
      try {
        ...
        if (isAdmin(userId)) {
          const readyRace = await markRaceReady(race.id, {
            userId,
            adminFree: true,
          ...
          await sendMessage?.(
            target,
            [
              '✅ **Скачки созданы бесплатно и сохранены.**',
              '',
              `Номер: **#${readyRace.id}**`,
              `🏁 Гонка начнётся: **${formatRaceDate(readyRace.start_at)}**`,
              '',
              'Посты пока **не опубликованы**.',
              'Пригласите нужные каналы, затем нажмите **🚀 Запустить сейчас**.'
            ].join('\n'),
            [
              [{
                text: '⚙️ Управление каналами',
                callback_data: `horse_channel_manage:${readyRace.id}`
              }],
              [{
                text: '🤝 Пригласить каналы',
                callback_data: `horse_collab_link:${readyRace.id}`
              }],
              [{
                text: '🚀 Запустить сейчас',
                callback_data: `horse_launch_now:${readyRace.id}`
              }],
              [{
                text: '📊 Статистика',
                callback_data: `horse_stats:${readyRace.id}`
              }]
            ]
          );
        } else {
    ...
    if (data.startsWith('horse_admin_broadcast_preview:')) {
      const raceId = normalizePositiveInt(data.split(':')[1]);
      await sendAdminRaceBroadcastPreview(target, userId, raceId);
      return true;
    }

    if (data.startsWith('horse_admin_broadcast_send:')) {
      const raceId = normalizePositiveInt(data.split(':')[1]);
      await confirmAdminRaceBroadcast(target, userId, raceId);
      return true;
    }
    ...
    if (data.startsWith('horse_admin_publish:')) {
      ...
      try {
        ...
        if (String(race?.status || '') === 'ready') {
          ...
          await answerMaxCallback?.(
            cb.id,
            '✅ Скачки полностью запущены'
          );
        } else {
          await publishRaceToChannel(
            raceId,
            config.generalChannelId,
            { isGeneral: true }
          );
          ...
          await answerMaxCallback?.(
            cb.id,
            '✅ Опубликовано в 🍟 РОЗЫГРЫШ И ТОЧКА'
          );
        }
      } catch (error) {
        await answerMaxCallback?.(
    ...
    if (data.startsWith('horse_join_check:')) {
      ...
      if (typeof checkUserSubscribedToChannelDetailed === 'function') {
        for (const channel of channels) {
          const check = await checkUserSubscribedToChannelDetailed(
            userId,
            channel.channelId
          );

          // Ошибку прав бота не записываем как отписку: иначе статистика даст ложный минус.
          if (!check?.permissionProblem) {
            await recordHorseRaceSubscriptionCheck(
              raceId,
              userId,
              channel.channelId,
              Boolean(check?.subscribed),
              'horse_join_check'
            );
          }
      ...
      try {
        ...
        const referrerId =
          pendingRes.rows[0]?.referrer_user_id || null;

        const entry = await addParticipant(
          raceId,
          userId,
          referrerId
        );
        ...
        const referralLink = buildRaceReferralLink(raceId, userId);
        const bonusCount = await getRaceReferralBonusCount(
          raceId,
          userId
        );

        if (entry.bonusAdded && entry.referrerId) {
          const bonusColor = serializeHorseColor(
            entry.bonusHorseColor
          );

          await sendMessage?.(
            String(entry.referrerId),
            [
              '🎁 **Вам начислен реферальный билет!**',
              '',
              `🐎 Скачки: **${cleanText(race.title, 180) || 'Скачки'}**`,
              `🎟 Новый билет: **№${entry.bonusTicketNumber}**`,
              `🎨 Цвет: **${bonusColor?.label || entry.bonusHorseColor}**`,
              `📊 Бонусных билетов: **${entry.referrerBonusCount}/${config.referralBonusLimit}**`,
              '',
              'Цвет бонусного билета совпадает с цветом вашей основной лошади.'
            ].join('\n')
          ).catch(() => {});
        }
        ...
        await sendMessage?.(
          ...
          [
            ...
            '',
            `🎟 Ваш основной билет: **№${participant.ticket_number}**`,
...
lines.push(
  ...
  async function notifyRaceResults() {
    const roundsRes = await pool.query(`
      SELECT
        ...
        organizer.profile_link AS organizer_profile_link
      FROM horse_race_rounds hrr
      JOIN horse_races hr
    ...
    for (const round of roundsRes.rows) {
      ...
      const organizerUser = {
        max_user_id: String(round.creator_user_id || ''),
        first_name: organizerFirstName || null,
        last_name: organizerLastName || null,
        username: organizerUsername || null,
        profile_link: cleanText(round.organizer_profile_link, 1000) || null
      };

      const organizerMentionLink =
        typeof buildMaxUserMentionLink === 'function'
          ? buildMaxUserMentionLink(organizerUser)
          : (round.creator_user_id
              ? `max://user/${encodeURIComponent(String(round.creator_user_id))}`
              : '');

      const organizerProfileLink =
        typeof buildUserProfileLink === 'function'
          ? buildUserProfileLink(organizerUser)
          : (organizerUsername
              ? `https://max.ru/${encodeURIComponent(organizerUsername)}`
              : '');

      const organizerDisplay = organizerMentionLink
        ? `[${escapeMarkdownText(organizerName)}](${organizerMentionLink})`
      ...
      if (organizerProfileLink) {
        winnerKeyboard.push([{
          ...
          url: organizerProfileLink
        }]);
      }

      winnerKeyboard.push([{
        text: '🐎 Посмотреть результат',
        url: getLaunchUrl(`race_${round.horse_race_id}`)
      }]);
      ...
      try {
        await sendMessage?.(String(round.winner_user_id), [
          ...
          `👤 Приз выдаёт: ${organizerDisplay}`,
          'Нажмите на имя организатора, чтобы открыть его профиль MAX.',
    ...
    for (const race of finishedRes.rows) {
      const winners = await pool.query(`
        ...
        LEFT JOIN users winner
          ON winner.max_user_id = hrr.winner_user_id
        WHERE hrr.horse_race_id = $1
        ORDER BY hrr.prize_index ASC
      `, [race.id]);

      const winnerLines = winners.rows.flatMap(row => {
        const prizeText =
          cleanText(row.prize_text, 300) ||
          `Приз ${row.prize_index}`;

        if (!row.winner_user_id) {
          return [
            `${row.prize_index}. 🎁 **${escapeMarkdownText(prizeText)}**`,
            '   Победитель: нет участников'
          ];
        }

        const firstName =
          cleanText(row.winner_first_name, 100);

        const lastName =
          cleanText(row.winner_last_name, 100);

        const username =
          cleanText(row.winner_username, 100)
            .replace(/^@/, '');
        ...
        const displayName =
          fullName ||
          (username ? `@${username}` : `ID ${row.winner_user_id}`);
        ...
        return [
          `${row.prize_index}. 🎁 **${escapeMarkdownText(prizeText)}**`,
          `   👤 Победитель: **${escapeMarkdownText(displayName)}**`,
          ...(username && fullName
            ? [`   🔗 Username: **@${escapeMarkdownText(username)}**`]
            : []),
          `   🆔 ID: \`${row.winner_user_id}\``,
          `   🎟 Билет: **№${row.winner_ticket_number}**`,
          `   🐎 Лошадь: **${escapeMarkdownText(color?.label || row.winner_color || 'не указана')}**`
        ];
      });
      ...
      const lines = [
        ...
        ...winnerLines,
        '',
        'Свяжитесь с победителем и договоритесь о выдаче приза.'
      ];
      try {
  ...
  function startWorker() {
    ...
    const run = async () => {
      try {
        ...
        await processRacePostUpdateQueue();
    ...
    if (
      config.postParticipantsUpdateSeconds > 0 &&
      !postParticipantsUpdateTimer
    ) {
      const periodicRun = () => {
        updateAllActiveRacePublishedPosts().catch(error => {
          console.warn(
            'Фоновое обновление счётчиков скачек не удалось:',
            error.message
          );
        });
      };

      postParticipantsUpdateTimer = setInterval(
        periodicRun,
        config.postParticipantsUpdateSeconds * 1000
      );
      postParticipantsUpdateTimer.unref?.();
    }
  ...
  function stopWorker() {
    ...
    if (postParticipantsUpdateTimer) clearInterval(postParticipantsUpdateTimer);
    clearInterval(rateCleanupTimer);
    workerTimer = null;
    postParticipantsUpdateTimer = null;
  }
  ...
  return {
    ...
    scheduleRacePostUpdate,
    processRacePostUpdateQueue,
    updateAllActiveRacePublishedPosts,
    activateAndPublishRace,
    sendHorseMenu,
    sendMyHorseRaces,
    sendMyHorseCollabRaces,
    sendHorseRaceStats,
    recordHorseRaceSubscriptionCheck,
    recordHorseRaceMemberDeltaFromWebhook,
...
function normalizeHorseChannelId(value) {
  const text = String(value || '').trim();
