...
function buildRafflePermissionAlertText(row, reason = 'permissions_lost') {
  ...
  return [
    ...
    '3. Если канал больше не участвует, организатор может удалить его командой:',
    `\`/delcollaber ${row.channel_id}\``,
    '',
    `Пока права не вернутся, бот будет напоминать примерно раз в **${RAFFLE_PERMISSION_ALERT_INTERVAL_MINUTES} мин.**`
  ].join('\n');
}
