...
async function publishRaffleToChannel(raffle, channelId) {
  ...
  const postUrl = buildRafflePostUrl(channelId, messageId, '', extractMaxPostUrl(data));
...
async function sendRaffleStats(target, raffleId, requesterUserId = null) {
  ...
  if (!channels.length) {
    ...
    text += '\n🟢/🔴 считаются по webhook-событиям MAX о вступлениях/выходах. “Участников из поста” считается точно по deep-link с channel_id для новых постов.\n';
  }
...
`;
    ...
    // В MAX нельзя делать слишком длинные строки кнопок. Поэтому максимум 2 кнопки в ряд.
    keyboard.push([
    ...
    if (r.status === 'scheduled') {
      keyboard.push([
        { text: `🚀 Сейчас #${r.id}`, callback_data: `start_raffle_now:${r.id}` },
        { text: `✏️ #${r.id}`, callback_data: `edit_raffle:${r.id}` }
      ]);
    }

    if (['scheduled', 'active'].includes(r.status)) {
      keyboard.push([{ text: `⏹ Стоп #${r.id}`, callback_data: `stop_raffle:${r.id}` }]);
    }

    if (r.status === 'finished') {
      keyboard.push([{ text: `🔁 Переиграть #${r.id}`, callback_data: `reroll_winners:${r.id}` }]);
    }
  }
