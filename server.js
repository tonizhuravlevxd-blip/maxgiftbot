...
async function editMaxMessageText(target, messageId, text, inlineKeyboard = null) {
  const id = String(messageId || '').trim();
  if (!id) return false;
  ...
  try {
    ...
    `, [
      ...
      JSON.stringify({
        ...
        reason: options.reason || 'paid_general_promo'
      })
    ]);
...
async function publishRaffleToChannel(raffle, channelId) {
  ...
  try {
    await pool.query(`
      INSERT INTO raffle_posts (raffle_id, channel_id, message_id, participants_count, updated_at)
      VALUES ($1, $2, $3::text, $4, NOW())
    `, [raffle.id, channelId, messageId, participantsCount]);
  } catch (error) {
