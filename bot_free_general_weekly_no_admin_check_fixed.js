...
async function sendFreeGeneralOfferAfterCreate(target, userId, raffle) {
  const eligibility = await getFreeGeneralEligibility(raffle.id, userId, { refresh: true });
  if (!eligibility.eligible) return false;

  const channel = eligibility.qualifyingChannel;
  ...
  await sendMessage(
    ...
    [[{ text: '🎁 БЕСПЛАТНО', callback_data: `free_general_claim:${raffle.id}` }]]
  );
...
async function processFreeGeneralPublishRequestQueuedItem(item) {
  ...
  const claim = claimRes.rows[0] || null;
  if (!claim) return { status: 'missing_free_claim' };

  const raffle = await getRaffleById(claim.raffle_id);
  if (!raffle) return { status: 'missing_raffle', claim };
