...
function parseEndDate(value) {
  ...
  for (const format of formats) {
    ...
    if (parsed.isValid()) {
      // Пользователь вводит время по МСК. Храним момент времени в UTC,
      // чтобы очередь публикации и завершения работала одинаково на любом сервере.
      return dayjs
...
// =========================
function botNow() {
  return dayjs().utcOffset(BOT_UTC_OFFSET_MINUTES);
}

function asBotTime(value) {
  const parsed = dayjs(value);
  return parsed.isValid() ? parsed.utcOffset(BOT_UTC_OFFSET_MINUTES) : null;
}
...
function getMinPublishAt() {
  return botNow().add(MIN_RAFFLE_PUBLISH_DELAY_MINUTES, 'minute');
}

function getMinEndAtForPublish(publishAt) {
  const base = asBotTime(publishAt);
...
function applyQuickOffset(base, offset) {
  ...
  const start = asBotTime(base) || botNow();
...
function buildCreatedRaffleMessage(sessionData, raffle, inviteLink) {
  return [
    ...
    inviteLink,
    '',
    'Логика соадмина: ссылка создаётся после сохранения розыгрыша, потому что ей нужен номер розыгрыша. Но это всё ещё до публикации: другой администратор может открыть ссылку, добавить свои каналы и выбрать — публиковать пост в его канале или нет, делать подписку обязательной или нет. Если он подключится уже после публикации, бот попробует опубликовать пост в его канале сразу.'
  ].join('\n');
}
...
async function handleSessionMessage(message) {
  ...
  if (state === 'await_publish_date' || state === 'edit_publish_date') {
    ...
    if (parsed.isBefore(minPublishAt)) {
      await sendMessage(
        target,
        `Публикация должна быть минимум через ${MIN_RAFFLE_PUBLISH_DELAY_MINUTES} минут. Ближайшее допустимое время: **${formatDateTime(minPublishAt)}**.`
      );
      return true;
    }
...
async function sendAdminActiveRaffles(target, userId) {
  ...
  for (const r of res.rows) {
    text += `#${r.id} | ${r.title} | ${r.status} | до ${formatDateTime(r.end_at)}\n`;
