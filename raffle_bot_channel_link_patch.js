...
function parseChannelLinkCommand(text) {
  ...
  // /chlink ID_РОЗЫГРЫША ID_КАНАЛА ССЫЛКА
  const m = clean.match(/^\/(?:chlink|channel_link|каналссылка|linkchannel)\s+(?:(\d+)\s+)?(-?\d+)\s+(https?:\/\/\S+)/i);
  if (!m) return null;

  return {
    raffleId: m[1] ? Number(m[1]) : null,
    channelId: String(m[2]).trim(),
    url: m[3].trim()
  };
}
...
async function updateRaffleChannelPublicLink(target, userId, channelId, newLink, raffleId = null) {
  const cleanChannelId = String(channelId || '').trim();
  const cleanRaffleId = raffleId !== null && raffleId !== undefined && raffleId !== ''
    ? Number(raffleId)
    : null;
  ...
  if (!/^-?\d+$/.test(cleanChannelId)) {
    return sendMessage(
      ...
      [
        ...
        '`/chlink ID_КАНАЛА ССЫЛКА`',
        '',
        'Если этот канал есть в нескольких активных розыгрышах:',
        '`/chlink ID_РОЗЫГРЫША ID_КАНАЛА ССЫЛКА`'
      ].join('\n')
    );
  }
  ...
  if (cleanRaffleId !== null && !Number.isInteger(cleanRaffleId)) {
    return sendMessage(target, 'Некорректный ID розыгрыша. Пример: `/chlink 38 -743535735754494 https://max.ru/join/...`');
  }

  if (!cleanUrl) {
    return sendMessage(target, 'Некорректная ссылка. Нужна полная ссылка, начинающаяся с `https://`.');
  }

  const params = [cleanChannelId, String(userId), isAdmin(userId)];
  let raffleFilter = '';
  ...
  if (!res.rows.length) {
    return sendMessage(
      target,
      [
        'Канал не найден в активных/запланированных розыгрышах, где вы организатор или владелец этого канала.',
        '',
        'Используйте:',
        '`/chlink ID_КАНАЛА ССЫЛКА`',
        '',
        'Если у канала несколько розыгрышей:',
        '`/chlink ID_РОЗЫГРЫША ID_КАНАЛА ССЫЛКА`'
      ].join('\n')
    );
  }
  ...
  if (res.rows.length > 1 && cleanRaffleId === null) {
    const lines = res.rows
      .slice(0, 10)
      .map(row => `• № ${getRafflePublicNumber({ id: row.raffle_id, public_number: row.public_number })} — ${displayValue(row.raffle_title, 'Без названия')}: \`/chlink ${row.raffle_id} ${cleanChannelId} ${cleanUrl}\``);

    return sendMessage(
      target,
      [
        'Нашла этот канал в нескольких активных/запланированных розыгрышах. Укажите, где поменять ссылку:',
        '',
        ...lines
      ].join('\n')
    );
  ...
  await pool.query(`
    ...
    SET channel_link = $2,
        updated_at = NOW()
    WHERE id = $1
  `, [row.raffle_channel_row_id, cleanUrl]);
  ...
  const updateResult = String(row.raffle_status || '') === 'active'
    ? await updateRafflePublishedPosts(row.raffle_id, { force: true }).catch(error => ({
        updated: 0,
        failed: 0,
        skipped: 0,
        error: error.message
  ...
  const lines = [
    ...
    row.old_channel_link ? `Старая ссылка: ${markdownLink('открыть', row.old_channel_link)}` : '',
    `Новая ссылка: ${markdownLink('открыть', cleanUrl)}`,
    '',
    String(row.raffle_status || '') === 'active'
      ? `Активные посты обновлены принудительно: обновлено **${Number(updateResult.updated || 0)}**, ошибок **${Number(updateResult.failed || 0)}**, пропущено **${Number(updateResult.skipped || 0)}**.`
      : 'Розыгрыш ещё запланирован, поэтому будущий пост уже выйдет с новой ссылкой.'
  ];
