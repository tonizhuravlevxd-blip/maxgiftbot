...
function setupPlayModule(options = {}) {
  ...
  (function () {
    ...
    function drawPyramid(x, baseY, w, h, shade) {
      ...
      ctx.fillStyle = 'rgba(93,64,35,.23)';
      ctx.beginPath();
      ctx.moveTo(x + w / 2, baseY - h);
      ctx.lineTo(x + w, baseY);
      ctx.lineTo(x + w * .58, baseY);
      ctx.closePath();
      ctx.fill();

      ctx.strokeStyle = 'rgba(105,69,31,.18)';
      ctx.lineWidth = 3;
      for (let i = 1; i < 6; i++) {
        const yy = baseY - (h / 6) * i;
        const inset = (baseY - yy) / h * w / 2;
        ctx.beginPath();
        ctx.moveTo(x + inset, yy);
        ctx.lineTo(x + w - inset, yy);
        ctx.stroke();
      }
    }
    ...
    function drawEgyptBackground() {
      ...
      const far = -((worldOffset * .14) % 900);
      for (let i = -1; i < 5; i++) {
        drawPyramid(far + i * 460 + 80, groundY - 52, 300, 180, '#c98f35');
        drawPyramid(far + i * 460 + 320, groundY - 42, 190, 115, '#d5a24a');
      }

      const near = -((worldOffset * .32) % 760);
      for (let i = -1; i < 5; i++) {
        drawPyramid(near + i * 520 + 260, groundY - 8, 250, 140, '#e0ac52');
      }
