...
async function recordPendingRaffleJoin(raffleId, userId, invitedBy = null) {
  ...
  await pool.query(`
    ...
    DO UPDATE SET
      invited_by = COALESCE(raffle_pending_joins.invited_by, EXCLUDED.invited_by),
      status = CASE
        WHEN raffle_pending_joins.status = 'completed' THEN 'completed'
        ELSE 'pending'
      END,
      updated_at = NOW()
  `, [raffleId, userId, ref]);
...
async function handleCallbackQuery(cb) {
  ...
  if (data.startsWith('check_join:')) {
    ...
    await completeJoinRaffleAfterCheck(target, userId, raffleId, ref);
    return;
  }
