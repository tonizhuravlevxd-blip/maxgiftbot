...
function getMinPublishAt() {
  return dayjs.utc().add(MIN_RAFFLE_PUBLISH_DELAY_MINUTES, 'minute');
}
...
async function startRaffleNow(target, userId, raffleId) {
  ...
  if (raffle.status !== 'scheduled') {
    return sendMessage(target, `Этот розыгрыш нельзя запустить сейчас. Текущий статус: ${raffle.status}.`);
  }
...
async function sendSubscriptionReminderToUser(raffle, user, missing, finalNotice = false) {
  const text = finalNotice
    ? [
        `⚠️ Розыгрыш **${raffle.title}** завершился.`,
        '',
    ...
    : [
        `⏰ Напоминание по розыгрышу **${raffle.title}**`,
        '',
