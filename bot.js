...
function buildWelcomeText(userOrId) {
  ...
  return [
    ...
    '',
    'Чтобы посмотреть инструкцию по правам, нажмите кнопку **⚙️ Права** ниже.',
    '',
...
async function sendWelcome(target, userOrId) {
  return sendMessage(
    target,
    buildWelcomeText(userOrId),
    [[{ text: '⚙️ Права', callback_data: 'permissions_help' }]]
  );
}
...
async function handleCallbackQuery(cb) {
  ...
  if (data === 'permissions_help') {
    await answerMaxCallback(cb.id, 'Открываю инструкцию');
    await deleteMaxMessageSafe(target, cb.message.id);
    await sendPermissionsHelp(target);
    return;
  }

  if (data === 'welcome_back') {
    await answerMaxCallback(cb.id, 'Назад');
    await deleteMaxMessageSafe(target, cb.message.id);
    await sendWelcome(target, cb.from);
    return;
  }
