...
function buildPromotionOfferText(raffle) {
  ...
  return [
    ...
    '',
    'Перед оплатой бот попросит email для чека. Если в сообщении нет корректного email, будет использован резервный email для чека.'
  ].join('\n');
}
...
function buildCreatedRaffleMessage(sessionData, raffle, inviteLink) {
  return [
    ...
    '',
    '📣 **Хотите увеличить охват?** У нас есть платные размещения: публикация в нашем канале розыгрышей — 500 ₽, размещение в боте с 7000+ пользователей — 1200 ₽. Ниже появятся кнопки покупки.'
  ].join('\n');
}
