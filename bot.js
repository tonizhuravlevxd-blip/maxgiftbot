...
async function sendAdminBroadcastToAll(target, adminUserId, text) {
  ...
  const recipients = usersRes.rows
    .map(row => String(row.max_user_id || '').trim())
    .filter(Boolean);
  ...
  await sendMessage(
    ...
    [
      ...
      'Бот отправляет сообщение всем пользователям с кнопкой **🏠 Меню**.'
    ].join('\n')
  );
  ...
  for (const recipientId of recipients) {
    try {
      await sendMessage(recipientId, cleanText, buildUserBroadcastKeyboard());
      ...
      if (isBroadcastDialogUnavailableError(error)) {
  ...
  return sendMessage(
    ...
    [
      ...
      `Отправлено: **${sent}**`,
      `Ошибок: **${failed}**`,
      'Если ошибка была `dialog.not.found`, пользователь автоматически исключён из следующих рассылок.'
    ].join('\n'),
