...
const MAX_USER_LINK_TEMPLATE = String(
  // Шаблон ссылки на профиль пользователя MAX по numeric user_id.
  // Если в вашем MAX используется другой формат ссылок, переопределите в .env:
  // MAX_USER_LINK_TEMPLATE=https://max.ru/id{id}
  process.env.MAX_USER_LINK_TEMPLATE || 'https://max.ru/id{id}'
).trim();
...
async function getUserByMaxId(userId) {
  ...
  const res = await pool.query(`
    SELECT max_user_id, username, first_name, last_name
    FROM users
    WHERE max_user_id = $1
    LIMIT 1
  `, [id]);
