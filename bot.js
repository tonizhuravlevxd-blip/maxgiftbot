...
async function checkActiveRaffleChannelPermissionsOnce() {
  ...
  for (const row of rows) {
    ...
    if (row.seen_is_removed || row.saved_is_active === false || row.saved_can_publish === false) {
      canPublish = false;
    } else {
      canPublish = await checkBotCanPublishInChannel(row.channel_id).catch(error => {
...
async function removeCollaboratorChannelFromActiveRaffle(target, userId, channelId, raffleId = null) {
  ...
  const params = [cleanChannelId];
  let ownerFilter = '';
  let raffleFilter = '';

  if (!isAdmin(userId)) {
    params.push(String(userId));
    ownerFilter = `AND r.creator_user_id::text = $${params.length}`;
  }
  ...
  if (!res.rows.length) {
    return sendMessage(
      target,
      [
        'Канал не найден в ваших активных розыгрышах.',
        '',
        'Используйте:',
