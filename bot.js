...
// Как часто повторять предупреждение организатору и владельцу проблемного канала, если активный розыгрыш под угрозой
...
// Как часто фоново проверять права бота в каналах активных розыгрышей.
...
function getPermissionAlertRecipients(row) {
  // Предупреждение получает только:
  // 1) организатор розыгрыша;
  // 2) владелец/соадмин именно того канала, где у бота пропали права.
  // Другим соадминам этого розыгрыша сообщение не отправляем.
  const recipients = new Set();
  const creatorId = String(row?.creator_user_id || '').trim();
  const channelOwnerId = String(row?.owner_user_id || '').trim();

  if (creatorId) recipients.add(creatorId);
  if (channelOwnerId) recipients.add(channelOwnerId);
...
async function resolveRafflePermissionAlerts(raffleId, channelId) {
  // Только помечаем проблему как решённую в БД.
  // Сообщение «✅ Права бота проверены...» никому не отправляем, чтобы не спамить.
  await pool.query(`
...
async function checkActiveRaffleChannelPermissionsOnce() {
  ...
  for (const row of rows) {
    // Важно: не доверяем только старым флагам из БД (can_publish/is_removed),
    // потому что права могли уже вернуть, а запись осталась старой.
    // Сначала делаем реальную проверку через MAX API.
    const canPublish = await checkBotCanPublishInChannel(row.channel_id).catch(error => {
      console.warn('Не удалось проверить права бота в канале активного розыгрыша:', {
        raffle_id: row.raffle_id,
        channel_id: String(row.channel_id),
        error: error.message
      });
    ...
    if (canPublish) {
      ...
      // Если раньше канал был помечен как удалённый/без прав, но проверка снова прошла,
      // очищаем старые флаги, чтобы бот больше не слал ложные предупреждения.
      await pool.query(`
        UPDATE bot_seen_chats
...
async function completeJoinRaffleAfterCheck(target, userId, raffleId, invitedBy = null, sourceChannelId = null) {
  ...
  if (!entry.alreadyJoined) {
    await updateRafflePublishedPosts(raffle.id).catch(error => {
      console.warn(`Не удалось обновить счётчик участников в постах розыгрыша ${raffle.id}:`, error.message);
    });
  }
...
`;
  ...
  for (const r of raffles) {
    ...
    const postLine = post.url
      ? `пост: ${post.isGeneral ? 'General' : displayValue(post.channelTitle, 'канал')}`
      : post.hasPost
        ? 'пост опубликован, но ссылка на конкретный пост не сохранена'
        : 'поста пока нет';

    text += `#${r.id} | ${titleForLine} | ${formatRaffleStatus(r.status)} | публикация ${formatDateTime(getRafflePublishAt(r))} | до ${formatDateTime(r.end_at)}
↳ ${postLine}
...
async function startServer() {
  try {
    ...
    setInterval(() => {
      checkActiveRaffleChannelPermissionsOnce().catch(error => {
        console.error('active raffle permission watch error:', error.message);
      });
    }, RAFFLE_PERMISSION_WATCH_SECONDS * 1000).unref?.();

    setTimeout(() => {
      checkActiveRaffleChannelPermissionsOnce().catch(error => {
        console.error('active raffle permission watch warmup error:', error.message);
      });
    }, 10000).unref?.();
