...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    canvas { width: 100%; height: auto; display: block; border-radius: 18px; background: linear-gradient(#8bd3ff 0%, #dbf7ff 58%, #89d48a 59%, #4caf50 100%); touch-action: manipulation; }
    .hint { margin: 12px 2px 0; opacity: .86; line-height: 1.35; font-size: 14px; text-align: center; }
...
<body>
  <main class="wrap">
    <section class="card">
      ...
      <canvas id="game" width="760" height="360" aria-label="Игра: собака перепрыгивает препятствия"></canvas>
      <button class="button" id="jumpBtn">Прыгнуть / Начать заново</button>
      <p class="hint">Нажмите кнопку, экран или пробел. Собака бежит бесконечно, скорость растёт. Не врежьтесь в препятствие.</p>
    </section>
  </main>
...
(function () {
  ...
  const groundY = 286;
  const dog = { x: 88, y: groundY - 64, w: 82, h: 64, vy: 0, onGround: true, frame: 0 };
  const gravity = 0.82;
  const jumpPower = -16;
  let obstacles = [];
  let speed = 6;
  ...
  function addObstacle() {
    ...
    nextObstacle = 62 + Math.random() * 68;
  }

  function collides(a, b) {
    const pad = 10;
    return a.x + pad < b.x + b.w && a.x + a.w - pad > b.x && a.y + pad < b.y + b.h && a.y + a.h - 6 > b.y;
  }
  ...
  function drawDog() {
    ...
    roundRect(8, 24, 54, 30, 15);
    ctx.fill();

    // chest
    ctx.fillStyle = '#c5864b';
    roundRect(18, 31, 24, 16, 8);
    ctx.fill();

    // head / long muzzle cartoon dog, inspired by a friendly mystery dog, not an exact copy
    ctx.fillStyle = '#8b4a20';
    roundRect(48, 8, 31, 30, 14);
    ctx.fill();
    ctx.fillStyle = '#b86f35';
    roundRect(62, 22, 28, 16, 8);
    ctx.fill();

    // ears
    ctx.fillStyle = '#5f3218';
    roundRect(49, 6, 14, 28, 8);
    ctx.fill();
    roundRect(39, 9, 12, 30, 8);
    ctx.fill();

    // collar
    ctx.fillStyle = '#29b6f6';
    ctx.fillRect(48, 36, 17, 6);
    ctx.fillStyle = '#ffd54f';
    ctx.beginPath();
    ctx.arc(57, 45, 4, 0, Math.PI * 2);
    ctx.fill();

    // eye and nose
    ctx.fillStyle = '#fff';
    ctx.beginPath();
    ctx.arc(68, 18, 5, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#111';
    ctx.beginPath();
    ctx.arc(70, 19, 2.2, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(86, 26, 4, 0, Math.PI * 2);
    ctx.fill();

    // spots
    ctx.fillStyle = '#3f2415';
    ctx.beginPath(); ctx.ellipse(25, 30, 8, 5, .2, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(43, 44, 7, 4, -.2, 0, Math.PI * 2); ctx.fill();

    // legs with small running animation
    const lift = dog.onGround ? Math.sin(dog.frame) * 4 : 2;
    ctx.fillStyle = '#6f3b1b';
    roundRect(17, 48, 9, 18 + lift, 5); ctx.fill();
    roundRect(48, 48, 9, 18 - lift, 5); ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(10, 31);
    ctx.quadraticCurveTo(-7, 16, -13, 24);
    ctx.stroke();
  ...
  function drawCloud(x, y) {
    ...
    ctx.arc(x + 18, y - 7, 22, 0, Math.PI * 2);
  ...
  function draw() {
    ...
    if (gameOver) {
      ...
      ctx.fillText('Игра окончена', canvas.width / 2, 145);
      ctx.font = '700 24px Arial';
      ctx.fillText('Ваш счёт: ' + Math.floor(score) + '  •  Рекорд: ' + best, canvas.width / 2, 190);
      ctx.font = '600 18px Arial';
      ctx.fillText('Нажмите, чтобы начать заново', canvas.width / 2, 230);
      ctx.textAlign = 'left';
    }
  }
