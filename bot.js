...
async function recordRafflePotentialLeaveFromUserRemoved(update, target) {
  if (!isChannelTarget(target)) return { matched: 0, recorded: 0 };

  const updateType = String(update?.update_type || '').toLowerCase();
  if (!updateType.includes('user_removed')) return { matched: 0, recorded: 0 };

  const userId = getActorUserIdFromUpdate(update);
  const channelId = String(target.id || '').trim();

  if (!userId || !channelId) return { matched: 0, recorded: 0 };
  if (BOT_USER_ID && String(userId) === String(BOT_USER_ID)) return { matched: 0, recorded: 0 };
      ...
      AND (
        EXISTS (
          ...
          WHERE rue.raffle_id = r.id
            AND rue.user_id::text = $2
        )
        OR EXISTS (
          SELECT 1
          FROM raffle_channel_subscription_states rcss
          WHERE rcss.raffle_id = r.id
            AND rcss.channel_id::text = $1
            AND rcss.user_id::text = $2
        )
      )
  ...
  for (const row of res.rows) {
    const result = await recordRaffleSubscriptionLeaveFromWebhook(
      row.raffle_id,
      userId,
      channelId,
  ...
  if (res.rows.length) {
    console.log('📉 Потенциальная отписка участника учтена для статистики розыгрыша:', {
      channel_id: channelId,
      user_id: String(userId),
      matched_raffles: res.rows.map(row => Number(row.raffle_id)),
      recorded_minus: recorded
    });
  }

  return { matched: res.rows.length, recorded };
}
...
async function handleMaxUpdate(update) {
  try {
    ...
    await recordRafflePotentialLeaveFromUserRemoved(update, target);
