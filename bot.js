...
const { setupHorseRacesModule } = require('./horse_races_module');
const { setupPlayModule } = require('./play_module');
...
// Сертификаты НУЦ Минцифры для нового домена MAX API.
// По умолчанию бот ищет сертификаты в ./certs рядом с bot.js.
// Можно передать один или несколько файлов через MAX_MINTSIFRY_CA_FILES,
// разделяя пути системным разделителем (":" в Linux, ";" в Windows).
const MAX_MINTSIFRY_CA_FILES = String(
  process.env.MAX_MINTSIFRY_CA_FILES ||
  process.env.MAX_MINTSIFRY_CA_PATH ||
  ''
).trim();
...
// Лимиты создания и сроки хранения данных розыгрышей.
const MAX_RAFFLES_PER_DAY = Math.max(
  1,
  Number(process.env.MAX_RAFFLES_PER_DAY || 5)
);
const MAX_ACTIVE_RAFFLES_PER_USER = Math.max(
  1,
  Number(process.env.MAX_ACTIVE_RAFFLES_PER_USER || 4)
);
const FINISHED_RAFFLE_FULL_DATA_DAYS = Math.max(
  1,
  Number(process.env.FINISHED_RAFFLE_FULL_DATA_DAYS || 7)
);
const FINISHED_RAFFLE_ARCHIVE_DAYS = Math.max(
  FINISHED_RAFFLE_FULL_DATA_DAYS + 1,
  Number(process.env.FINISHED_RAFFLE_ARCHIVE_DAYS || 30)
);
const CANCELLED_RAFFLE_RETENTION_DAYS = Math.max(
  1,
  Number(process.env.CANCELLED_RAFFLE_RETENTION_DAYS || 2)
);
const DRAFT_SESSION_RETENTION_HOURS = Math.max(
  1,
  Number(process.env.DRAFT_SESSION_RETENTION_HOURS || 24)
);
const DONE_QUEUE_RETENTION_DAYS = Math.max(
  1,
  Number(process.env.DONE_QUEUE_RETENTION_DAYS || 3)
);
const FAILED_QUEUE_RETENTION_DAYS = Math.max(
  DONE_QUEUE_RETENTION_DAYS,
  Number(process.env.FAILED_QUEUE_RETENTION_DAYS || 7)
);
const ADMIN_COMMUNITY_INVITE_COOLDOWN_DAYS = Math.max(
  1,
  Number(process.env.ADMIN_COMMUNITY_INVITE_COOLDOWN_DAYS || 30)
);
const ADMIN_COMMUNITY_MIN_SUBSCRIBERS = Math.max(
  1,
  Number(process.env.ADMIN_COMMUNITY_MIN_SUBSCRIBERS || 100)
);
const RAFFLE_CLEANUP_INTERVAL_HOURS = Math.max(
  1,
  Number(process.env.RAFFLE_CLEANUP_INTERVAL_HOURS || 6)
);
...
const PROMO_GENERAL_REPEAT_PRICE_RUB = String(process.env.PROMO_GENERAL_REPEAT_PRICE_RUB || '150.00');
const PROMO_BOT_PRICE_RUB = String(process.env.PROMO_BOT_PRICE_RUB || '1200.00');
const PROMO_TOP1_PRICE_RUB = String(process.env.PROMO_TOP1_PRICE_RUB || '1000.00');
const PROMO_GENERAL_PRODUCT_CODE = 'raffle_general_channel_publish';
const PROMO_BOT_PRODUCT_CODE = 'raffle_bot_7000_publish';
const PROMO_TOP1_PRODUCT_CODE = 'raffle_top1_boost';
const ACTIVE_RAFFLES_JOIN_LIMIT = Math.max(
  1,
  Number(process.env.ACTIVE_RAFFLES_JOIN_LIMIT || USER_RAFFLES_VISIBLE_LIMIT || 5)
);
...
// Пауза между повторными публикациями в выбранные каналы.
// Не даём одной кнопке устроить всплеск запросов к MAX API.
const RAFFLE_REPUBLISH_SEND_DELAY_MS = Math.max(
  150,
  Number(process.env.RAFFLE_REPUBLISH_SEND_DELAY_MS || 400)
);

const RAFFLE_REPUBLISH_PAGE_SIZE = Math.min(
  24,
  Math.max(5, Number(process.env.RAFFLE_REPUBLISH_PAGE_SIZE || 20))
);

// Бесплатная публикация в General для организаторов с каналом > 2000 подписчиков.
// Лимит скользящий: по умолчанию 1 раз каждые 7 суток по пользователю и каналу.
const FREE_GENERAL_MIN_SUBSCRIBERS = Math.max(
  1,
  Number(process.env.FREE_GENERAL_MIN_SUBSCRIBERS || 2000)
);
const FREE_GENERAL_ENABLED = String(
  process.env.FREE_GENERAL_ENABLED || 'true'
).toLowerCase() !== 'false';

// Скользящий лимит бесплатного размещения: по умолчанию 1 раз в 7 суток.
// Ограничение хранится одновременно по организатору и по qualifying-каналу.
const FREE_GENERAL_COOLDOWN_DAYS = Math.max(
  1,
  Number(process.env.FREE_GENERAL_COOLDOWN_DAYS || 7)
);

// Через сколько минут повторять временно не прошедшую техническую проверку перед публикацией.
const FREE_GENERAL_VALIDATION_RETRY_MINUTES = Math.max(
  1,
  Number(process.env.FREE_GENERAL_VALIDATION_RETRY_MINUTES || 10)
);
...
async function queueAdminRichBroadcastFromHorseModule(payload = {}) {
  return createAdminBroadcastJob(
    payload.target,
    payload.adminUserId,
    payload.text,
    {
      ...
      broadcastType: payload.broadcastType || 'horse_race',
      sourceType: payload.sourceType || 'horse_race',
...
const horseRacesModule = setupHorseRacesModule({
  app,
  pool,
  sendMessage,
  answerMaxCallback,
  ...
  BOT_USERNAME,
  MAX_BOT_TOKEN,
  MAX_MINIAPP_LAUNCH_URL: process.env.MAX_MINIAPP_LAUNCH_URL || '',
  MINIAPP_WEB_URL: process.env.MINIAPP_WEB_URL || '',
  GENERAL_CHANNEL_ID,
  GENERAL_CHANNEL_PUBLIC_NAME,
  setSession,
  clearSession,
  parseDateTime: parseEndDate,
  extractPhotoAttachment: extractFirstPhotoAttachmentFromMessage,
  yookassaRequest,
  buildYooKassaReceipt,
  normalizeReceiptEmail,
  isYooKassaReady,
  notifyAdmins: notifyPromoAdmins,
  extractMaxMessageId,
  editMaxMessageText,
  queueAdminBroadcast: queueAdminRichBroadcastFromHorseModule,
  buildMaxUserMentionLink,
  buildUserProfileLink
});

const playModule = setupPlayModule({
  app,
  pool,
  sendMessage,
  APP_BASE_URL,
...
const RAFFLE_CREATE_TOTAL_STEPS = 9;
...
async function sendRafflePhotoStepPrompt(target, data = {}) {
  const hasPhoto = Boolean(data.photo_attachment);
  const aiPhotoUrl = 'https://max.ru/id236700415542_bot';

  const keyboard = [];

  if (hasPhoto) {
    keyboard.push([
      { text: '✅ Оставить текущее фото', callback_data: 'raffle_photo_continue' }
    ]);
  }

  keyboard.push([
    {
      text: hasPhoto ? '🚫 Продолжить без фото' : '⏭ Пропустить',
      callback_data: 'raffle_photo_skip'
    }
  ]);
  ...
  keyboard.push(
    [{ text: '⬅️ Назад к призам', callback_data: 'raffle_step_back:prizes' }],
    [{ text: '❌ Отмена', callback_data: 'cancel_session' }]
  );

  return sendMessage(
    target,
    buildCreateStepMessage(4, 'Фото', [
      '🖼 **Добавьте фото к розыгрышу**',
      '',
      hasPhoto
        ? 'Фото уже добавлено. Можно отправить другое, оставить текущее или продолжить без.'
        : '*Отправьте фото в чат.*',
      '',
      `👆 Нету своего крутого фото❓ **${markdownLink('ИИ', aiPhotoUrl)}** сделает его бесплатно за секунды.`,
      '',
      'Фото необязательно.'
    ]),
    keyboard
...
function buildCreateStepBackKeyboard(backStep, backText = '⬅️ Назад') {
  return [
    [{ text: backText, callback_data: `raffle_step_back:${backStep}` }],
    [{ text: '❌ Отмена', callback_data: 'cancel_session' }]
  ];
}

function enableCreateChannelBack(data = {}) {
  data.__create_channel_back = true;
  return data;
}

function clearCreateFlowBackFlags(data = {}) {
  delete data.__create_channel_back;
  return data;
}
...
// MAX API
// =========================
function getMaxMintsifryCertificateFiles() {
  const configured = MAX_MINTSIFRY_CA_FILES
    ? MAX_MINTSIFRY_CA_FILES
  ...
  const defaults = [
    path.join(__dirname, 'certs', 'russian_trusted_root_ca.cer'),
    path.join(__dirname, 'certs', 'russian_trusted_sub_ca.cer')
  ];

  // NODE_EXTRA_CA_CERTS тоже учитываем, если переменная уже задана в Render.
  const candidates = [
    ...configured,
  ...
  return [...new Set(candidates)].filter(filePath => {
    try {
...
function createMaxHttpsAgent() {
  const certificateFiles = getMaxMintsifryCertificateFiles();
  ...
  for (const filePath of certificateFiles) {
    try {
      ...
      if (!certificate.includes('-----BEGIN CERTIFICATE-----')) {
        console.warn('⚠️ Файл сертификата Минцифры не похож на PEM:', filePath);
        continue;
      }

      extraCertificates.push(certificate);
    } catch (error) {
      console.warn('⚠️ Не удалось прочитать сертификат Минцифры:', {
        file: filePath,
        error: error.message
      });
    }
  ...
  if (extraCertificates.length) {
    console.log('✅ Сертификаты Минцифры подключены для MAX API:', certificateFiles);
  } else {
    console.warn(
      '⚠️ Сертификат Минцифры для MAX API не найден. ' +
      'Положите russian_trusted_root_ca.cer в папку certs рядом с bot.js ' +
      'или задайте MAX_MINTSIFRY_CA_PATH.'
    );
  }
  ...
  return new https.Agent({
    keepAlive: true,
    maxSockets: 30,
    // Не заменяем стандартные корневые сертификаты Node.js —
    // добавляем Минцифры к обычному доверенному набору.
    ca: extraCertificates.length
      ? [...tls.rootCertificates, ...extraCertificates]
      : undefined,
    rejectUnauthorized: true
  });
}

const MAX_HTTPS_AGENT = createMaxHttpsAgent();

async function maxRequest(apiPath, options = {}) {
  const url = new URL(`${MAX_API_BASE}${apiPath}`);
  ...
  return new Promise((resolve, reject) => {
    const request = https.request(url, {
      ...
      response.on('end', () => {
        ...
        if (statusCode < 200 || statusCode >= 300) {
          ...
          reject(new Error(`MAX API ${statusCode}: ${details}`));
          return;
        }
    ...
    request.setTimeout(timeoutMs, () => {
      request.destroy(
        new Error(`MAX API timeout after ${timeoutMs}ms: ${apiPath}`)
      );
    });
...
function getPromotionProductInfo(product, options = {}) {
  ...
  if (code === PROMO_GENERAL_PRODUCT_CODE) {
    return {
      ...
      priceRub: String(options.priceRub || PROMO_GENERAL_PRICE_RUB),
...
function isGeneralPublicationProduct(product) {
  return [PROMO_GENERAL_PRODUCT_CODE, PROMO_TOP1_PRODUCT_CODE].includes(String(product || '').trim());
}

async function getSucceededGeneralPromoCount(raffleId) {
  ...
  `, [id, PROMO_GENERAL_PRODUCT_CODE]);

  return Number(res.rows[0]?.cnt || 0);
}

async function getGeneralPromoPriceRub(raffleId) {
  const succeededCount = await getSucceededGeneralPromoCount(raffleId);

  // Цена чередуется внутри одного розыгрыша:
  // 1-я покупка = 500 ₽, 2-я = 150 ₽, 3-я = 500 ₽, 4-я = 150 ₽ и так далее.
  // succeededCount — это уже оплаченные General-публикации до создания нового счёта.
  const nextPurchaseNumber = succeededCount + 1;
  return nextPurchaseNumber % 2 === 0
    ? PROMO_GENERAL_REPEAT_PRICE_RUB
    : PROMO_GENERAL_PRICE_RUB;
}

async function getPromotionProductInfoForRaffle(product, raffleId) {
  const code = String(product || '').trim();

  if (code === PROMO_GENERAL_PRODUCT_CODE) {
    return getPromotionProductInfo(code, {
      priceRub: await getGeneralPromoPriceRub(raffleId)
    });
  }
...
async function createPromotionPayment({ userId, raffleId, product, receiptEmail }) {
  const info = await getPromotionProductInfoForRaffle(product, raffleId);
  ...
  if (!(await isRaffleOrganizer(raffle, userId))) {
    throw new Error('Оплатить продвижение может только организатор, соадмин подключённого канала или админ бота');
  }

  if (product === PROMO_TOP1_PRODUCT_CODE && String(raffle.status || '') !== 'active') {
    throw new Error('ТОП 1 можно купить только для активного розыгрыша');
  }
...
async function buildPromotionOfferText(raffle) {
  ...
  return [
    ...
    `🤫АКЦИЯ: **1-я публикация — ${Number(PROMO_GENERAL_PRICE_RUB).toFixed(0)} ₽**, **2-я — ${Number(PROMO_GENERAL_REPEAT_PRICE_RUB).toFixed(0)} ₽**`,
    `После оплаты бот сначала попросит выбрать время. Когда выбранное время наступит, бот найдёт ближайшее свободное окно в ${generalChannel} с интервалом **${GENERAL_PROMO_SPACING_MINUTES} мин.** и опубликует пост.`,
    '',
...
async function buildPromotionOfferKeyboard(raffle) {
  const generalPriceRub = await getGeneralPromoPriceRub(raffle.id);
...
async function sendPromotionOffer(target, userId, raffleIdOrRow) {
  ...
  if (!(await isRaffleOrganizer(raffle, userId))) {
    return sendMessage(target, '⛔ Купить продвижение может только организатор, соадмин подключённого канала или админ бота.');
  }

  return sendMessage(
    target,
    await buildPromotionOfferText(raffle),
    await buildPromotionOfferKeyboard(raffle)
  );
}
...
async function startPromotionPaymentEmailFlow(target, userId, raffleId, product) {
  ...
  if (!(await isRaffleOrganizer(raffle, userId))) {
    return sendMessage(target, '⛔ Оплатить продвижение может только организатор, соадмин подключённого канала или админ бота.');
  }

  if (product === PROMO_TOP1_PRODUCT_CODE && String(raffle.status || '') !== 'active') {
    return sendMessage(target, '🥇 ТОП 1 можно купить только для активного розыгрыша.');
  }

  if (isGeneralPublicationProduct(product) && !GENERAL_CHANNEL_ID) {
...
async function getPromoPaymentByPaymentId(paymentId) {
...
async function sendPaidGeneralPublishTimeChoice(target, paymentOrRow, raffle, product = PROMO_GENERAL_PRODUCT_CODE) {
...
const productTitle = productCode === PROMO_TOP1_PRODUCT_CODE
  ...
  await sendMessage(
    ...
    [
      ...
      [{ text: '🕒 Ввести время', callback_data: `paid_general_input:${paymentId}` }],
      [{ text: '🏠 Главное меню', callback_data: 'main_menu' }]
    ]
  ).catch(error => {
    console.warn(`Не удалось отправить выбор времени публикации General пользователю ${userId}:`, error.message);
  });
...
async function applyGeneralChannelPromoPayment(payment, row) {
  ...
  await sendPaidGeneralPublishTimeChoice(
    userId,
    row || payment,
    raffle,
    PROMO_GENERAL_PRODUCT_CODE
  );
}

async function applyTop1BoostPayment(payment, row) {
  const raffleId = Number(row?.raffle_id || payment?.metadata?.raffle_id);
  const userId = String(row?.user_id || payment?.metadata?.user_id || '').trim();
  const paymentId = String(row?.payment_id || payment?.id || '').trim();
  const raffle = await getRaffleById(raffleId);

  if (!raffle) {
    await sendMessage(userId, '✅ Оплата ТОП 1 прошла, но розыгрыш не найден. Администратор проверит оплату вручную.').catch(() => {});
    await notifyPromoAdmins(`⚠️ Оплата ТОП 1 прошла, но розыгрыш #${raffleId} не найден. Payment: ${paymentId}`);
    return;
  }

  await pool.query(`
    INSERT INTO raffle_top_boosts (raffle_id, user_id, payment_id, created_at)
    VALUES ($1, $2, $3, NOW())
    ON CONFLICT (payment_id) DO NOTHING
  `, [raffle.id, userId || raffle.creator_user_id, paymentId || null]);
  ...
  await sendMessage(
    ...
    [
      '🥇 **ТОП 1 активирован.**',
      '',
      `Розыгрыш № **${getRafflePublicNumber(raffle)}** теперь будет выше в меню «Участвовать».`,
      'Он останется вверху, пока другой пользователь не купит более новый ТОП 1.'
    ].join('\n')
  ).catch(() => {});

  if (!GENERAL_CHANNEL_ID) {
    await sendMessage(userId || raffle.creator_user_id, '⚠️ GENERAL_CHANNEL_ID не задан, поэтому дополнительная публикация в General пока невозможна.').catch(() => {});
    return;
  }
...
async function applyPromotionPayment(payment) {
  ...
  if (![PROMO_GENERAL_PRODUCT_CODE, PROMO_BOT_PRODUCT_CODE, PROMO_TOP1_PRODUCT_CODE].includes(product)) {
    return false;
  }
  ...
  if (product === PROMO_TOP1_PRODUCT_CODE) {
    await applyTop1BoostPayment(payment, saved || row);
    return true;
  }
...
async function handleYooKassaWebhook(req, res) {
  ...
  (async () => {
    try {
      ...
      const handledByHorseRaces = await horseRacesModule.applyYooKassaPayment(payment);
      if (!handledByHorseRaces) {
...
function parseChannelLinkCommand(text) {
  const clean = String(text || '').trim();

  // Явный тип устраняет неоднозначность, если у обычного розыгрыша
  // и скачек совпали числовые ID.
  // /chlink horse ID_СКАЧЕК ID_КАНАЛА ССЫЛКА
  // /chlink raffle ID_РОЗЫГРЫША ID_КАНАЛА ССЫЛКА
  const typed = clean.match(
    /^\/(?:chlink|channel_link|каналссылка|linkchannel)\s+(horse|race|скачки|raffle|розыгрыш)\s+(\d+)\s+(-?\d+)\s+(https?:\/\/\S+)$/i
  );

  if (typed) {
    const rawType = String(typed[1] || '').toLowerCase();
    const entityType = ['horse', 'race', 'скачки'].includes(rawType)
      ? 'horse'
      : 'raffle';

    return {
      entityType,
      raffleId: Number(typed[2]),
      channelId: String(typed[3]).trim(),
      url: typed[4].trim()
    };
  }

  // Старые форматы продолжают работать:
  // /chlink ID_КАНАЛА ССЫЛКА
  // /chlink ID_ОБЪЕКТА ID_КАНАЛА ССЫЛКА
  const legacy = clean.match(
    /^\/(?:chlink|channel_link|каналссылка|linkchannel)\s+(?:(\d+)\s+)?(-?\d+)\s+(https?:\/\/\S+)$/i
  );
  ...
  return {
    entityType: null,
    raffleId: legacy[1] ? Number(legacy[1]) : null,
    channelId: String(legacy[2]).trim(),
    url: legacy[3].trim()
  };
}
...
function extractParticipantsCountFromChatInfo(info) {
  if (!info || typeof info !== 'object') return null;

  // Официальный объект Chat MAX содержит participants_count.
  // Дополнительные варианты оставлены для совместимости с обёртками API.
  const candidates = [
    ...
    info.chat?.participants_count,
    info.chat?.participantsCount,
    info.result?.participants_count,
    info.result?.participantsCount,
    info.payload?.participants_count,
    info.payload?.participantsCount
  ];
...
async function updateRaffleChannelPublicLink(
  target,
  userId,
  channelId,
  newLink,
  ...
  const cleanChannelId = String(channelId || '').trim();
  const cleanItemId = raffleId !== null && raffleId !== undefined && raffleId !== ''
    ? Number(raffleId)
    : null;
  const cleanUrl = normalizeHttpLink(newLink);
  const cleanEntityType = ['raffle', 'horse'].includes(String(entityType || '').toLowerCase())
    ? String(entityType).toLowerCase()
    : null;
  const cleanUserId = String(userId || '').trim();
  const admin = Boolean(isAdmin(userId));
  ...
  if (!/^-?\d+$/.test(cleanChannelId)) {
    return sendMessage(
      ...
      [
        ...
        '`/chlink ID_КАНАЛА ССЫЛКА`',
        '',
        'Для конкретного обычного розыгрыша:',
        '`/chlink raffle ID_РОЗЫГРЫША ID_КАНАЛА ССЫЛКА`',
        '',
        'Для конкретных скачек:',
        '`/chlink horse ID_СКАЧЕК ID_КАНАЛА ССЫЛКА`'
      ].join('\n')
    );
  }

  if (cleanItemId !== null && !Number.isInteger(cleanItemId)) {
    return sendMessage(
      target,
      'Некорректный ID. Пример для скачек: `/chlink horse 12 -743535735754494 https://max.ru/join/...`'
    );
  }
  ...
  if (!cleanUrl) {
    return sendMessage(
      target,
      'Некорректная ссылка. Нужна полная ссылка, начинающаяся с `https://`.'
    );
  }

  const matches = [];

  if (cleanEntityType !== 'horse') {
    const raffleParams = [cleanChannelId, cleanUserId, admin];
    let raffleFilter = '';

    if (cleanItemId !== null) {
      raffleParams.push(cleanItemId);
      raffleFilter = `AND r.id = $${raffleParams.length}`;
    }

    const raffleRes = await pool.query(`
      SELECT
        'raffle'::text AS entity_type,
        r.id AS item_id,
        r.id AS public_number,
        r.title AS item_title,
        r.status AS item_status,
        r.creator_user_id,
        rc.id AS channel_row_id,
        rc.channel_id,
        rc.channel_title,
        rc.channel_link AS old_channel_link,
        rc.owner_user_id
      FROM raffles r
      JOIN raffle_channels rc ON rc.raffle_id = r.id
      WHERE r.status IN ('scheduled', 'active')
        ...
        AND (
          r.creator_user_id::text = $2
          OR rc.owner_user_id::text = $2
          OR $3::boolean = true
        )
        ${raffleFilter}
      ORDER BY
    ...
    matches.push(...raffleRes.rows);
  }

  if (cleanEntityType !== 'raffle') {
    const horseParams = [cleanChannelId, cleanUserId, admin];
    let horseFilter = '';

    if (cleanItemId !== null) {
      horseParams.push(cleanItemId);
      horseFilter = `AND hr.id = $${horseParams.length}`;
    }

    try {
      const horseRes = await pool.query(`
        SELECT
          'horse'::text AS entity_type,
          hr.id AS item_id,
          hr.id AS public_number,
          hr.title AS item_title,
          hr.status AS item_status,
          hr.creator_user_id,
          hrc.id AS channel_row_id,
          hrc.channel_id,
          hrc.channel_title,
          hrc.channel_link AS old_channel_link,
          hrc.owner_user_id
        FROM horse_races hr
        JOIN horse_race_channels hrc
          ON hrc.horse_race_id = hr.id
        WHERE hr.status IN ('ready', 'scheduled', 'active', 'running')
          AND hr.cancelled_at IS NULL
          AND hrc.channel_id::text = $1
          AND (
            hr.creator_user_id::text = $2
            OR hrc.owner_user_id::text = $2
            OR $3::boolean = true
          )
          ${horseFilter}
        ORDER BY
          CASE
            ...
            WHEN hr.status = 'active' THEN 1
            WHEN hr.status = 'scheduled' THEN 2
            ELSE 3
          END,
          hr.id DESC,
          hrc.id DESC
      ...
      matches.push(...horseRes.rows);
    } catch (error) {
      // Не ломаем старые установки, где таблицы скачек ещё не созданы.
      if (error?.code !== '42P01') throw error;
      console.warn('Таблицы скачек ещё не созданы, /chlink проверил только обычные розыгрыши.');
    }
  }
  ...
  if (!matches.length) {
    return sendMessage(
      target,
      [
        'Канал не найден ни в доступных обычных розыгрышах, ни в скачках.',
        '',
        'Бот ищет только объекты, где вы организатор, владелец подключённого канала или администратор.',
        '',
        'Обычный розыгрыш:',
        '`/chlink raffle ID_РОЗЫГРЫША ID_КАНАЛА ССЫЛКА`',
        '',
  ...
  if (matches.length > 1) {
    const lines = matches
      .slice(0, 15)
      .map(row => {
        const type = row.entity_type === 'horse' ? 'horse' : 'raffle';
        const label = row.entity_type === 'horse' ? '🐎 Скачки' : '🎉 Розыгрыш';
        return `${label} #${row.item_id} — ${displayValue(row.item_title, 'Без названия')}: ` +
          `\`/chlink ${type} ${row.item_id} ${cleanChannelId} ${cleanUrl}\``;
      });

    return sendMessage(
      target,
      [
        'Этот канал найден в нескольких объектах. Укажите точно, где изменить ссылку:',
        '',
        ...lines
      ].join('\n')
    );
  ...
  const row = matches[0];
  const isHorseRace = row.entity_type === 'horse';

  if (isHorseRace) {
    await pool.query(`
      UPDATE horse_race_channels
      SET channel_link = $2,
          updated_at = NOW()
      WHERE id = $1
    `, [row.channel_row_id, cleanUrl]);
  } else {
    await pool.query(`
      UPDATE raffle_channels
      SET channel_link = $2,
          updated_at = NOW()
      WHERE id = $1
    `, [row.channel_row_id, cleanUrl]);
  ...
  // Сохраняем актуальную ссылку и в общем справочнике каналов.
  await pool.query(`
    UPDATE user_channels
    SET channel_link = $3,
        updated_at = NOW()
  ...
  `, [
    ...
    console.warn('Не удалось обновить user_channels.channel_link:', error.message);
  });
  ...
  await pool.query(`
    ...
    SET chat_link = $2,
        updated_at = NOW()
    WHERE chat_id::text = $1
  `, [cleanChannelId, cleanUrl]).catch(error => {
    console.warn('Не удалось обновить bot_seen_chats.chat_link:', error.message);
  });

  let updateResult = { updated: 0, failed: 0, skipped: 0 };

  if (isHorseRace) {
    if (['scheduled', 'active', 'running'].includes(String(row.item_status || ''))) {
      updateResult = await horseRacesModule.updateRacePublishedPosts(
        row.item_id,
        { force: true }
      ).catch(error => ({
        updated: 0,
  ...
  } else if (String(row.item_status || '') === 'active') {
    updateResult = await updateRafflePublishedPosts(
      row.item_id,
      { force: true }
    ).catch(error => ({
      updated: 0,
  ...
  const objectLabel = isHorseRace ? 'Скачки' : 'Розыгрыш';
  const status = String(row.item_status || '');
  const lines = [
    '✅ Публичная ссылка канала обновлена.',
    '',
    `${objectLabel} #**${row.item_id}**: **${displayValue(row.item_title, 'Без названия')}**`,
    `Канал: **${displayValue(row.channel_title, cleanChannelId)}**`,
    row.old_channel_link
      ? `Старая ссылка: ${markdownLink('открыть', row.old_channel_link)}`
      : '',
    `Новая ссылка: ${markdownLink('открыть', cleanUrl)}`,
    ''
  ];

  if (isHorseRace && status === 'ready') {
    lines.push('Скачки ещё не опубликованы. После запуска посты сразу выйдут с новой ссылкой.');
  } else if (!isHorseRace && status === 'scheduled') {
    lines.push('Розыгрыш ещё запланирован, поэтому будущий пост уже выйдет с новой ссылкой.');
  } else {
    lines.push(
      `Опубликованные посты обновлены принудительно: обновлено **${Number(updateResult.updated || 0)}**, ` +
      `ошибок **${Number(updateResult.failed || 0)}**, пропущено **${Number(updateResult.skipped || 0)}**.`
    );
  }

  if (updateResult.error) {
    lines.push('', `⚠️ Ошибка обновления постов: ${safeText(updateResult.error, 500)}`);
  }

  return sendMessage(target, lines.filter(Boolean).join('\n'));
}
...
let text = mode === 'collab'
  ...
  : `${buildCreateStepProgress(7, 'Каналы')}\n\n📡 **Каналы для розыгрыша**\n\n`;

  if (mode === 'collab') {
    text += 'Выберите каналы из списка ниже. Подписка на каждый добавленный канал будет обязательной.\n';
    text += 'Для выбранного канала можно переключить только:\n';
    text += '📣 **с размещением** или 🙈 **без размещения**.\n\n';
  } else {
  ...
  if (!userChannels.length) {
    ...
    text += 'Каналы пока **не** выбраны. Можно продолжить без каналов или выбрать канал ниже,нажав ⬜';
  } else {
    text += '**Выбрано:**\n';
  ...
  for (const ch of userChannels) {
    ...
    if (stored) {
      if (mode === 'collab') {
        keyboard.push([
          {
            text: stored.publish_post ? '🙈 Выбрать без размещения' : '📣 Выбрать с размещением',
            callback_data: `collab_ch_pub:${id}`
          }
        ]);
      } else {
        keyboard.push([
          {
            ...
            callback_data: `raffle_ch_req:${id}`
          },
          {
            text: stored.publish_post ? '🙈 Без размещения' : '📣 С размещением',
            callback_data: `raffle_ch_pub:${id}`
          }
        ]);
      }
    }
...
async function createRaffleFromSession(userId, data) {
  ...
  try {
    raffle = await createRaffleDraft(userId);
    ...
    // Считаем только полностью созданный розыгрыш. Если любой предыдущий шаг
    // упал, запись будет удалена и в пожизненную статистику не попадёт.
    await markRaffleInLifetimeStats(raffle.id);

    return { raffle, inviteToken };
  } catch (error) {
    // Не оставляем частично созданный draft/scheduled-розыгрыш, если один из
    // последующих шагов записи завершился ошибкой.
    if (raffle?.id) {
      await pool.query('DELETE FROM raffles WHERE id = $1', [raffle.id]).catch(cleanupError => {
        console.warn('Не удалось удалить частично созданный розыгрыш:', cleanupError.message);
      });
    }
    throw error;
  }
...
async function addCollabChannelsFromSession(target, userId, data) {
  ...
  for (const ch of channels) {
    ...
    if (raffle.status === 'active' && ch.publish_post) {
      const alreadyPublished = await hasRafflePostInChannel(raffle.id, ch.channel_id);
...
async function sendMainMenu(target, userId = null) {
  const keyboard = [
    ...
    [{ text: '🐎 Создать / Участвовать в скачках', callback_data: 'horse_menu' }],
...
class RaffleCreationLimitError extends Error {
  constructor(code, state = {}) {
    super(code === 'active_limit'
      ? `Достигнут лимит активных розыгрышей: ${MAX_ACTIVE_RAFFLES_PER_USER}`
      : `Достигнут дневной лимит розыгрышей: ${MAX_RAFFLES_PER_DAY}`);
    this.name = 'RaffleCreationLimitError';
    this.code = code;
    this.state = state;
  }
}

async function getRaffleCreationLimitState(userId, db = pool, options = {}) {
  const cleanUserId = String(userId || '').trim();

  if (!cleanUserId) {
    return { allowed: false, dailyCount: 0, activeCount: 0, dailyLimited: true, activeLimited: false };
  }

  if (options.lock === true) {
    // Блокировка действует в рамках транзакции и не даёт двум параллельным
    // подтверждениям создания одновременно обойти лимиты.
    await db.query(
      `SELECT pg_advisory_xact_lock(hashtext($1), hashtext($2))`,
      ['raffle_create_limit', cleanUserId]
    );
  }
  ...
  const res = await db.query(`
    SELECT
      COUNT(*) FILTER (
        WHERE created_at >= (
          date_trunc('day', NOW() + ($2::text || ' minutes')::interval)
      ...
      COUNT(*) FILTER (
        WHERE status IN ('scheduled', 'active')
           OR (status = 'draft' AND created_at > NOW() - INTERVAL '30 minutes')
      )::int AS active_count
    FROM raffles
    WHERE creator_user_id = $1
  `, [cleanUserId, BOT_UTC_OFFSET_MINUTES]);

  const dailyCount = Number(res.rows[0]?.daily_count || 0);
  const activeCount = Number(res.rows[0]?.active_count || 0);

  return {
    allowed: dailyCount < MAX_RAFFLES_PER_DAY && activeCount < MAX_ACTIVE_RAFFLES_PER_USER,
    dailyCount,
    activeCount,
    dailyLimited: dailyCount >= MAX_RAFFLES_PER_DAY,
    activeLimited: activeCount >= MAX_ACTIVE_RAFFLES_PER_USER
  };
}

function buildRaffleCreationLimitMessage(state = {}) {
  if (state.activeLimited) {
    return [
      `⚠️ У вас уже есть **${state.activeCount || MAX_ACTIVE_RAFFLES_PER_USER}** активных или запланированных розыгрыша.`,
      '',
      `Одновременно можно иметь не больше **${MAX_ACTIVE_RAFFLES_PER_USER}**. Сначала завершите один из них.`
    ].join('\n');
  }

  return [
    `⚠️ Сегодня вы уже создали **${state.dailyCount || MAX_RAFFLES_PER_DAY}** розыгрышей.`,
    '',
    `В сутки можно создать не больше **${MAX_RAFFLES_PER_DAY}**. Новый лимит начнётся после **00:00 ${BOT_TIMEZONE_LABEL}**.`
  ].join('\n');
}

async function startRaffleCreation(target, userId) {
  const state = await getRaffleCreationLimitState(userId);

  if (!state.allowed) {
    await sendMessage(target, buildRaffleCreationLimitMessage(state));
    return false;
  }
...
async function createRaffleDraft(creatorId) {
  ...
  try {
    ...
    const state = await getRaffleCreationLimitState(creatorId, client, { lock: true });

    if (!state.allowed) {
      throw new RaffleCreationLimitError(
        state.activeLimited ? 'active_limit' : 'daily_limit',
        state
      );
    }
...
async function updateRaffle(raffleId, fields) {
  ...
  if (Object.prototype.hasOwnProperty.call(fields, 'status')) {
    const nextStatus = String(fields.status || '').toLowerCase();
...
async function addRaffleChannel(
  ...
  await pool.query(`
    ...
    ON CONFLICT (raffle_id, channel_id)
    DO UPDATE SET
      channel_title = EXCLUDED.channel_title,
...
async function hasRafflePostInChannel(raffleId, channelId) {
  const res = await pool.query(`
    SELECT 1
    FROM raffle_posts
    WHERE raffle_id = $1
...
async function getAdminCommunityEligibleChannel(raffleId, userId, options = {}) {
  const cleanUserId = String(userId || '').trim();
  if (!raffleId || !cleanUserId) return null;

  const res = await pool.query(`
    SELECT rc.*
    FROM raffle_channels rc
    WHERE rc.raffle_id = $1
      AND rc.owner_user_id::text = $2
      AND COALESCE(rc.publish_post, true) = true
      AND ($3::text = '' OR rc.channel_id::text <> $3)
    ORDER BY COALESCE(rc.subscriber_count, 0) DESC, rc.id ASC
  `, [raffleId, cleanUserId, String(GENERAL_CHANNEL_ID || '').trim()]);

  for (const channel of res.rows) {
    let count = null;

    if (options.refresh !== false) {
      const info = await getChatInfoSafe(channel.channel_id);
      count = extractParticipantsCountFromChatInfo(info);
    }

    if (count === null) {
      const savedCount = Number(channel.subscriber_count);
      if (Number.isFinite(savedCount) && savedCount >= 0) {
        count = Math.floor(savedCount);
      }
    }

    if (count === null || count < ADMIN_COMMUNITY_MIN_SUBSCRIBERS) {
      continue;
    }

    await pool.query(`
      UPDATE raffle_channels
      SET subscriber_count = $3, updated_at = NOW()
      WHERE raffle_id = $1 AND channel_id = $2
    `, [raffleId, channel.channel_id, count]).catch(() => {});

    return { ...channel, subscriber_count: count };
  }
...
async function scheduleAdminCommunityInviteAfterRaffleCreate(raffleId, userId) {
  const cleanUserId = String(userId || '').trim();
  if (!cleanUserId || !ADMIN_COMMUNITY_INVITE_URL) return null;

  const eligibleChannel = await getAdminCommunityEligibleChannel(raffleId, cleanUserId, { refresh: true });
  if (!eligibleChannel) return null;
  ...
  try {
    ...
    await client.query(
      `SELECT pg_advisory_xact_lock(hashtext($1), hashtext($2))`,
      ['admin_community_invite', cleanUserId]
    );

    const cooldownRes = await client.query(`
      SELECT admin_community_invited_at
      FROM users
      WHERE max_user_id = $1
      FOR UPDATE
    `, [cleanUserId]);

    const invitedAt = cooldownRes.rows[0]?.admin_community_invited_at || null;
    if (invitedAt) {
      const nextAllowedAt = dayjs.utc(invitedAt).add(ADMIN_COMMUNITY_INVITE_COOLDOWN_DAYS, 'day');
      if (nextAllowedAt.isAfter(dayjs.utc())) {
        await client.query('COMMIT');
        return null;
      }
    }
    ...
    const existingRes = await client.query(`
      SELECT id
      FROM raffle_queue
      WHERE queue_type = 'admin_community_invite'
    ...
    `, [
      ...
      JSON.stringify({
        user_id: cleanUserId,
        channel_id: String(eligibleChannel.channel_id),
        subscriber_count: Number(eligibleChannel.subscriber_count || 0),
        delay_minutes: ADMIN_COMMUNITY_INVITE_DELAY_MINUTES,
        reason: 'owned_channel_after_raffle_created'
      })
    ]);
...
async function sendAdminCommunityInviteQueuedItem(item) {
  ...
  const invitedAt = userRes.rows[0]?.admin_community_invited_at || null;
  if (invitedAt && dayjs.utc(invitedAt).add(ADMIN_COMMUNITY_INVITE_COOLDOWN_DAYS, 'day').isAfter(dayjs.utc())) {
    return { status: 'cooldown', userId };
  }

  const eligibleChannel = await getAdminCommunityEligibleChannel(item.raffle_id, userId, { refresh: true });
  if (!eligibleChannel) {
    return { status: 'not_eligible', userId };
  }
  ...
  await pool.query(`
    UPDATE users
    SET admin_community_invited_at = NOW(), updated_at = NOW()
    WHERE max_user_id = $1
  `, [userId]);
  ...
  return {
    status: 'sent',
    userId,
    channelId: String(eligibleChannel.channel_id),
    subscriberCount: Number(eligibleChannel.subscriber_count || 0)
  };
}
...
async function schedulePaidGeneralPublish(raffleId, options = {}) {
  ...
  try {
    ...
    const product = isAdminManualGeneralPublish
      ? PROMO_GENERAL_PRODUCT_CODE
      : String(options.product || PROMO_GENERAL_PRODUCT_CODE).trim();

    if (!isAdminManualGeneralPublish && !isGeneralPublicationProduct(product)) {
      await client.query('COMMIT');
      return { status: 'unsupported_product', raffle };
    }
    ...
    if (isAdminManualGeneralPublish) {
      ...
      const paidRes = await client.query(`
        ...
        WHERE payment_id = $1
          ...
          AND status = 'succeeded'
        LIMIT 1
      `, [requestedPaymentId, raffle.id, product]);
    ...
    const scheduledAt = findNextGeneralPromoSlot(busyRes.rows, options.startAt || new Date());
    ...
    `, [
      ...
      JSON.stringify({
        ...
        requested_start_at: options.requestedStartAt || null
      })
    ]);
...
function getBotLocalCalendarDay(value = dayjs.utc()) {
  const moment = toUtcMoment(value) || dayjs.utc();
  return moment.add(BOT_UTC_OFFSET_MINUTES, 'minute').format('YYYY-MM-DD');
}

function getFreeGeneralCooldownUntil(value = dayjs.utc()) {
  const moment = toUtcMoment(value) || dayjs.utc();
  return moment.add(FREE_GENERAL_COOLDOWN_DAYS, 'day').toDate();
}

function formatFreeGeneralNextAvailable(value) {
  const moment = toUtcMoment(value);
  return moment ? formatDateTime(moment) : 'после окончания текущего ограничения';
}

async function getFreeGeneralClaimByRaffle(raffleId) {
  const res = await pool.query(`
    SELECT *
    FROM free_general_publications
    WHERE raffle_id = $1
    ORDER BY id DESC
    LIMIT 1
  `, [raffleId]);
...
async function getActiveFreeGeneralCooldown(scopeType, scopeId, db = pool, options = {}) {
  const type = String(scopeType || '').trim();
  const id = String(scopeId || '').trim();

  if (!['user', 'channel'].includes(type) || !id) return null;

  const forUpdate = options.forUpdate ? ' FOR UPDATE' : '';
  const res = await db.query(`
    SELECT *
    FROM free_general_cooldowns
    WHERE scope_type = $1
      AND scope_id = $2
...
async function upsertFreeGeneralCooldown(db, scopeType, scopeId, nextAvailableAt, claimId) {
  const type = String(scopeType || '').trim();
  const id = String(scopeId || '').trim();

  if (!['user', 'channel'].includes(type) || !id) {
    throw new Error('Некорректная область free_general cooldown');
  }

  const res = await db.query(`
    INSERT INTO free_general_cooldowns (
      scope_type,
      scope_id,
      next_available_at,
      last_claim_id,
    ...
    DO UPDATE SET
      next_available_at = GREATEST(free_general_cooldowns.next_available_at, EXCLUDED.next_available_at),
      last_claim_id = EXCLUDED.last_claim_id,
      updated_at = NOW()
    RETURNING *
  `, [type, id, nextAvailableAt, claimId || null]);

  return res.rows[0] || null;
}

async function lockFreeGeneralScope(db, scopeType, scopeId) {
  await db.query(
    `SELECT pg_advisory_xact_lock(hashtext($1), hashtext($2))`,
    [`free_general_${String(scopeType || '').trim()}`, String(scopeId || '').trim()]
  );
}

async function getQualifyingOrganizerChannelResult(raffle, options = {}) {
  if (!raffle) return { channel: null, reason: 'missing_raffle' };

  const refresh = options.refresh !== false;
  const creatorId = String(raffle.creator_user_id || '').trim();

  const res = await pool.query(`
    SELECT
      rc.*,
      COALESCE(uc.is_active, false) AS owner_channel_active,
      COALESCE(uc.can_publish, false) AS owner_channel_can_publish
    FROM raffle_channels rc
    LEFT JOIN user_channels uc
      ON uc.channel_id = rc.channel_id
     AND uc.owner_user_id = rc.owner_user_id
    WHERE rc.raffle_id = $1
      AND ($2::text = '' OR rc.channel_id::text <> $2)
      AND rc.owner_user_id::text = $3
      AND COALESCE(rc.is_required, true) = true
      AND COALESCE(rc.publish_post, true) = true
      AND COALESCE(uc.is_active, false) = true
      AND COALESCE(uc.can_publish, false) = true
    ORDER BY rc.id ASC
  `, [
    raffle.id,
    String(GENERAL_CHANNEL_ID || '').trim(),
    creatorId
  ]);

  if (!res.rows.length) {
    return { channel: null, reason: 'no_owned_publish_channel' };
  }

  let best = null;
  let lastReason = 'no_qualifying_channel';
  ...
  for (const channel of res.rows) {
    ...
    if (refresh) {
      info = await getChatInfoSafe(channel.channel_id);
      if (!info) {
        lastReason = 'channel_check_unavailable';
        continue;
      }

      count = extractParticipantsCountFromChatInfo(info);
      if (count === null) {
        lastReason = 'subscriber_count_unavailable';
        continue;
      }
    } else {
      const savedCount = Number(channel.subscriber_count);
    ...
    if (count <= FREE_GENERAL_MIN_SUBSCRIBERS) {
      lastReason = 'below_min_subscribers';
      continue;
    }

    // Для FREE GENERAL не делаем отдельную живую проверку прав публикации
    // через MAX API: она может давать ложный 403. Достаточно сохранённых
    // признаков user_channels.is_active и user_channels.can_publish,
    // которые уже проверены SQL-запросом выше.

    await pool.query(`
      UPDATE raffle_channels
      SET subscriber_count = $3, updated_at = NOW()
      WHERE raffle_id = $1 AND channel_id = $2
    `, [raffle.id, channel.channel_id, count]).catch(() => {});

    if (!best || count > Number(best.subscriber_count || 0)) {
      best = {
        ...channel,
  ...
  return best
    ? { channel: best, reason: 'eligible_channel' }
    : { channel: null, reason: lastReason };
}

async function getQualifyingOrganizerChannel(raffle, options = {}) {
  const result = await getQualifyingOrganizerChannelResult(raffle, options);
  return result.channel || null;
}

async function getFreeGeneralEligibility(raffleId, userId, options = {}) {
  if (!FREE_GENERAL_ENABLED) {
    return { eligible: false, reason: 'disabled' };
  }

  if (!GENERAL_CHANNEL_ID) {
    return { eligible: false, reason: 'missing_general_channel' };
  }

  const raffle = await getRaffleById(raffleId);
  if (!raffle) {
    return { eligible: false, reason: 'missing_raffle' };
  }

  const requesterId = String(userId || '').trim();
  const creatorId = String(raffle.creator_user_id || '').trim();

  if (!requesterId || requesterId !== creatorId) {
    return { eligible: false, reason: 'creator_only', raffle };
  }

  if (!['scheduled', 'active'].includes(String(raffle.status || ''))) {
    return { eligible: false, reason: 'not_publishable', raffle };
  }

  if (toUtcMoment(raffle.end_at)?.isBefore(dayjs.utc())) {
    return { eligible: false, reason: 'ended', raffle };
  }

  const existingForRaffle = await getFreeGeneralClaimByRaffle(raffle.id);
  if (existingForRaffle) {
    return {
      eligible: false,
      reason: 'already_claimed_for_raffle',
      raffle,
      claim: existingForRaffle
    };
  }

  const userCooldown = await getActiveFreeGeneralCooldown('user', creatorId);
  if (userCooldown) {
    return {
      eligible: false,
      ...
      nextAvailableAt: userCooldown.next_available_at
    };
  }

  const channelResult = await getQualifyingOrganizerChannelResult(raffle, {
    refresh: options.refresh !== false
  });
  const qualifyingChannel = channelResult.channel;

  if (!qualifyingChannel) {
    return {
      eligible: false,
      reason: channelResult.reason || 'no_qualifying_channel',
      raffle
    };
  }

  const channelCooldown = await getActiveFreeGeneralCooldown(
    'channel',
    qualifyingChannel.channel_id
  );

  if (channelCooldown) {
    return {
      eligible: false,
      reason: 'channel_weekly_limit',
      raffle,
      qualifyingChannel,
      cooldown: channelCooldown,
      nextAvailableAt: channelCooldown.next_available_at
    };
  }
  ...
  return {
    ...
    raffle,
    qualifyingChannel,
    claimDay: getBotLocalCalendarDay(),
    cooldownUntil: getFreeGeneralCooldownUntil()
  };
}

async function ensureGeneralChannelRequiredForRaffle(raffleId, db = pool) {
  const generalId = String(GENERAL_CHANNEL_ID || '').trim();
  if (!generalId) throw new Error('GENERAL_CHANNEL_ID не задан');

  const updateRes = await db.query(`
    UPDATE raffle_channels
    SET
      channel_title = $3,
      channel_link = COALESCE(NULLIF($4, ''), channel_link),
      owner_user_id = NULL,
      is_required = true,
      publish_post = false,
      updated_at = NOW()
    WHERE raffle_id = $1
      AND channel_id::text = $2
    RETURNING *
  `, [raffleId, generalId, GENERAL_CHANNEL_PUBLIC_NAME, GENERAL_CHANNEL_URL]);
...
function describeFreeGeneralValidationFailure(reason) {
  const messages = {
    missing_protected_channel: 'подходящий канал удалён из розыгрыша',
    protected_channel_flags_changed: 'для подходящего канала отключена обязательная подписка или размещение',
    protected_channel_owner_changed: 'канал больше не закреплён за организатором',
    owner_channel_inactive: 'канал отключён или бот потерял право публикации',
    channel_check_unavailable: 'MAX временно не вернул информацию о канале',
    subscriber_count_unavailable: 'не удалось подтвердить актуальное число подписчиков',
    below_min_subscribers: `в канале стало ${FREE_GENERAL_MIN_SUBSCRIBERS} подписчиков или меньше`,
    bot_cannot_publish: 'бот не имеет подтверждённого права публикации в канале',
    creator_mismatch: 'создатель розыгрыша не совпадает с владельцем бесплатной заявки'
  };

  return messages[String(reason || '')] || 'условия бесплатной публикации больше не подтверждены';
}

async function validateFreeGeneralClaimForPublish(claim, raffle) {
  if (!claim || !raffle) {
    return { ok: false, temporary: false, reason: 'missing_claim_or_raffle' };
  }

  const claimUserId = String(claim.user_id || '').trim();
  const creatorId = String(raffle.creator_user_id || '').trim();
  const channelId = String(claim.qualifying_channel_id || '').trim();

  if (!claimUserId || claimUserId !== creatorId) {
    return { ok: false, temporary: false, reason: 'creator_mismatch' };
  }
  ...
  const channelRes = await pool.query(`
    ...
    LEFT JOIN user_channels uc
     ...
     AND uc.owner_user_id = rc.owner_user_id
    WHERE rc.raffle_id = $1
      AND rc.channel_id::text = $2
    LIMIT 1
  `, [raffle.id, channelId]);

  const channel = channelRes.rows[0] || null;
  if (!channel) {
    return { ok: false, temporary: false, reason: 'missing_protected_channel' };
  }

  if (String(channel.owner_user_id || '').trim() !== claimUserId) {
    return { ok: false, temporary: false, reason: 'protected_channel_owner_changed' };
  }

  if (!channel.is_required || !channel.publish_post) {
    return { ok: false, temporary: false, reason: 'protected_channel_flags_changed' };
  }

  if (!channel.owner_channel_active || !channel.owner_channel_can_publish) {
    return { ok: false, temporary: true, reason: 'owner_channel_inactive' };
  }

  const info = await getChatInfoSafe(channelId);
  if (!info) {
    return { ok: false, temporary: true, reason: 'channel_check_unavailable' };
  }

  const count = extractParticipantsCountFromChatInfo(info);
  if (count === null) {
    return { ok: false, temporary: true, reason: 'subscriber_count_unavailable' };
  }
  ...
  if (count <= FREE_GENERAL_MIN_SUBSCRIBERS) {
    return {
      ok: false,
      temporary: false,
      reason: 'below_min_subscribers',
      subscriberCount: count
    };
  }

  // Перед FREE GENERAL-публикацией не повторяем отдельную живую проверку
  // прав бота через MAX API, потому что она может возвращать ложный 403.
  // Актуальность канала уже подтверждена через user_channels.is_active / can_publish,
  // а реальная отправка в канал всё равно завершится ошибкой, если публикация невозможна.

  await pool.query(`
    UPDATE raffle_channels
    SET subscriber_count = $3, updated_at = NOW()
    WHERE raffle_id = $1 AND channel_id::text = $2
  `, [raffle.id, channelId, count]).catch(() => {});

  await pool.query(`
    UPDATE free_general_publications
    SET
      subscriber_count = $2,
      ...
      updated_at = NOW()
    WHERE id = $1
  `, [claim.id, count]).catch(() => {});

  return { ok: true, channel, subscriberCount: count };
}

async function deferFreeGeneralValidation(item, claim, validation) {
  const retryMinutes = FREE_GENERAL_VALIDATION_RETRY_MINUTES;
  const reason = String(validation?.reason || 'temporary_validation_failure');
  ...
  await pool.query(`
    ...
    SET
      status = 'pending',
      scheduled_at = NOW() + ($2::text || ' minutes')::interval,
  ...
  `, [
    ...
    retryMinutes,
    JSON.stringify({
      free_validation_retry_reason: reason,
      free_validation_retry_at: new Date().toISOString()
    })
  ]);

  return {
    status: 'free_validation_deferred',
    reason,
    keepPending: true
  };
}

async function scheduleFreeGeneralPublish(raffleId, claimId, options = {}) {
  if (!GENERAL_CHANNEL_ID) {
    return { status: 'missing_general_channel' };
  }
  ...
  try {
    ...
    if (!claim) {
      await client.query('COMMIT');
      return { status: 'missing_free_claim' };
    }
    ...
    if (String(raffle.status || '') !== 'active') {
      await client.query('COMMIT');
      return { status: 'not_active', raffle, claim };
    }
    ...
    const existingRes = await client.query(`
      ...
      WHERE queue_type = 'general_publish'
        AND status IN ('pending', 'processing', 'done')
        AND payload->>'free_general_claim_id' = $1
      ORDER BY scheduled_at ASC
      LIMIT 1
    `, [String(claim.id)]);
    ...
    if (existingRes.rows[0]) {
      ...
      return {
        ...
        raffle,
        claim,
        queue: existingRes.rows[0],
        scheduledAt: existingRes.rows[0].scheduled_at
      };
    }

    await ensureGeneralChannelRequiredForRaffle(raffle.id, client);
    ...
    `, [
      ...
      scheduledAt,
      JSON.stringify({
        product: 'free_large_channel_publish',
        free_large_channel_publish: true,
        free_general_claim_id: String(claim.id),
        qualifying_channel_id: String(claim.qualifying_channel_id || ''),
        subscriber_count: Number(claim.subscriber_count || 0),
        requested_by: String(claim.user_id || raffle.creator_user_id || ''),
        reason: options.reason || 'free_large_channel_publish'
      })
    ]);

    await client.query(`
      UPDATE free_general_publications
      SET status = 'queued', queue_id = $2, updated_at = NOW()
      WHERE id = $1
    `, [claim.id, insertRes.rows[0].id]);
...
async function scheduleFreeGeneralPublishRequest(claim, raffle) {
  if (!claim || !raffle) {
    return { status: 'missing_claim_or_raffle' };
  }

  if (!['scheduled', 'active'].includes(String(raffle.status || ''))) {
    return { status: 'raffle_not_publishable', claim, raffle };
  }

  if (toUtcMoment(raffle.end_at)?.isBefore(dayjs.utc())) {
    return { status: 'raffle_already_ended', claim, raffle };
  }

  const existingPublish = await pool.query(`
    SELECT *
    FROM raffle_queue
    WHERE queue_type = 'general_publish'
      AND status IN ('pending', 'processing', 'done')
  ...
  if (existingPublish.rows[0]) {
    return {
      status: 'already_queued',
      claim,
      raffle,
      queue: existingPublish.rows[0],
      scheduledAt: existingPublish.rows[0].scheduled_at
    };
  }
  ...
  const existingRequest = await pool.query(`
    SELECT *
    FROM raffle_queue
    WHERE queue_type = 'general_publish_request'
      AND status IN ('pending', 'processing')
      AND payload->>'free_general_claim_id' = $1
    ORDER BY scheduled_at ASC
    LIMIT 1
  ...
  if (existingRequest.rows[0]) {
    return {
      ...
      claim,
      raffle,
      queue: existingRequest.rows[0],
      scheduledAt: existingRequest.rows[0].scheduled_at
    };
  }

  if (String(raffle.status || '') === 'active') {
    return scheduleFreeGeneralPublish(raffle.id, claim.id, {
      reason: 'free_large_channel_now'
    });
  }

  const publishAt = toUtcMoment(getRafflePublishAt(raffle)) || dayjs.utc();
  const scheduledAt = publishAt.isAfter(dayjs.utc())
    ? publishAt.add(5, 'second').toDate()
    : dayjs.utc().add(5, 'second').toDate();

  const insertRes = await pool.query(`
    INSERT INTO raffle_queue (raffle_id, queue_type, scheduled_at, payload, status)
    VALUES ($1, 'general_publish_request', $2, $3::jsonb, 'pending')
    RETURNING *
  `, [
    raffle.id,
    scheduledAt,
    JSON.stringify({
      ...
      free_general_claim_id: String(claim.id),
      requested_by: String(claim.user_id || raffle.creator_user_id || ''),
      reason: 'free_large_channel_wait_until_active'
    })
  ]);

  await pool.query(`
    UPDATE free_general_publications
    SET status = 'waiting_for_active', queue_id = $2, updated_at = NOW()
    WHERE id = $1
  `, [claim.id, insertRes.rows[0].id]);
...
async function claimFreeGeneralPublication(target, userId, raffleId) {
  const eligibility = await getFreeGeneralEligibility(raffleId, userId, { refresh: true });

  if (!eligibility.eligible) {
    if (eligibility.reason === 'already_claimed_for_raffle') {
      if (String(eligibility.claim?.status || '') === 'blocked_validation_failed') {
        return sendMessage(
          target,
          [
            '⛔ **Бесплатная публикация для этого розыгрыша была остановлена защитой.**',
            '',
            `Причина: **${describeFreeGeneralValidationFailure(eligibility.claim?.validation_error)}**.`,
            '',
            `Новый бесплатный слот будет доступен после: **${formatFreeGeneralNextAvailable(eligibility.claim?.cooldown_until)}**.`
          ].join('\n')
        );
      }

      const result = await scheduleFreeGeneralPublishRequest(
        eligibility.claim,
        eligibility.raffle
      );
      ...
      return sendMessage(
        target,
        [
          'ℹ️ **Бесплатная публикация для этого розыгрыша уже активирована.**',
          '',
          ['queued', 'already_queued'].includes(String(result?.status || ''))
            ? formatFreeGeneralQueueResult(result)
            : (
                ['request_queued', 'already_requested'].includes(String(result?.status || ''))
                  ? 'Публикация ожидает запуска розыгрыша и затем автоматически попадёт в очередь General.'
                  : `Текущий статус: **${displayValue(result?.status, eligibility.claim?.status || 'активирована')}**.`
              ),
          scheduledAt ? `Плановое время: **${formatDateTime(scheduledAt)}**` : ''
        ].filter(Boolean).join('\n')
      );
    }

    if (eligibility.reason === 'user_weekly_limit') {
      return sendMessage(
        target,
        [
          '⏳ **Недельный лимит уже использован.**',
          '',
          `Следующая бесплатная публикация для вашего аккаунта будет доступна: **${formatFreeGeneralNextAvailable(eligibility.nextAvailableAt)}**.`
        ].join('\n')
      );
    }

    if (eligibility.reason === 'channel_weekly_limit') {
      return sendMessage(
        target,
        [
          '⏳ **Этот канал уже использовал бесплатное размещение.**',
          '',
          `Повторно использовать этот канал можно после: **${formatFreeGeneralNextAvailable(eligibility.nextAvailableAt)}**.`,
          '',
          'Смена аккаунта организатора не снимает ограничение с канала.'
        ].join('\n')
      );
    }

    if (['channel_check_unavailable', 'subscriber_count_unavailable'].includes(eligibility.reason)) {
      return sendMessage(
        target,
        '⚠️ Сейчас MAX не смог подтвердить актуальное число подписчиков канала. Попробуйте нажать кнопку немного позже.'
      );
    }

    if (eligibility.reason === 'below_min_subscribers') {
      return sendMessage(
        target,
        `⚠️ Для бесплатной публикации нужен ваш канал с числом подписчиков **больше ${FREE_GENERAL_MIN_SUBSCRIBERS}**.`
      );
    }

    if (eligibility.reason === 'bot_cannot_publish') {
      return sendMessage(
        target,
        '⚠️ У бота нет подтверждённого права публикации в подходящем канале. Выдайте боту права администратора на размещение постов.'
      );
    }

    if (['no_owned_publish_channel', 'no_qualifying_channel'].includes(eligibility.reason)) {
      return sendMessage(
        target,
        [
          '⚠️ Для бесплатной публикации в розыгрыш должен быть добавлен ваш канал:',
          `• **больше ${FREE_GENERAL_MIN_SUBSCRIBERS} подписчиков**;`,
          '• обязательная подписка включена;',
          '• размещение поста в канале включено;',
          '• бот имеет право публиковать посты.'
        ].join('\n')
      );
    }

    if (eligibility.reason === 'creator_only') {
      return sendMessage(target, '⛔ Бесплатную публикацию может активировать только создатель розыгрыша.');
    }

    return sendMessage(target, '⚠️ Бесплатная публикация сейчас недоступна для этого розыгрыша.');
  }

  const { raffle, qualifyingChannel, claimDay, cooldownUntil } = eligibility;
  const creatorId = String(raffle.creator_user_id || '').trim();
  const channelId = String(qualifyingChannel.channel_id || '').trim();
  const client = await pool.connect();
  let claim = null;
  ...
  try {
    ...
    // Всегда берём блокировки в одном порядке: пользователь, затем канал.
    // Это закрывает двойной клик и параллельную попытку с другого аккаунта на тот же канал.
    await lockFreeGeneralScope(client, 'user', creatorId);
    await lockFreeGeneralScope(client, 'channel', channelId);

    const sameRaffleRes = await client.query(`
      SELECT *
      FROM free_general_publications
      WHERE raffle_id = $1
      LIMIT 1
    ...
    if (sameRaffleRes.rows[0]) {
      claim = sameRaffleRes.rows[0];
      await client.query('COMMIT');
    } else {
      const userCooldown = await getActiveFreeGeneralCooldown(
        'user',
        creatorId,
        client,
        { forUpdate: true }
      ...
      if (userCooldown) {
        ...
        return sendMessage(
          target,
          `⏳ Бесплатная публикация для вашего аккаунта будет доступна после **${formatFreeGeneralNextAvailable(userCooldown.next_available_at)}**.`
        );
      }
      ...
      if (channelCooldown) {
        ...
        return sendMessage(
          target,
          `⏳ Этот канал уже использовал бесплатное размещение. Следующий слот: **${formatFreeGeneralNextAvailable(channelCooldown.next_available_at)}**.`
        );
      }

      const protectedChannelRes = await client.query(`
        SELECT rc.*
        FROM raffle_channels rc
        ...
        LIMIT 1
        FOR UPDATE OF rc
      `, [raffle.id, channelId, creatorId]);

      if (!protectedChannelRes.rows[0]) {
        await client.query('ROLLBACK');
        return sendMessage(
          target,
          '⚠️ Условия подходящего канала изменились. Канал должен принадлежать организатору, быть обязательным и публиковать пост розыгрыша.'
        );
      }
      ...
      const insertRes = await client.query(`
        INSERT INTO free_general_publications (
          user_id,
          raffle_id,
          qualifying_channel_id,
          subscriber_count,
          claim_day,
          cooldown_until,
          status,
        ...
        VALUES ($1, $2, $3, $4, $5::date, $6, 'claimed', NOW(), NOW(), NOW())
        RETURNING *
      `, [
        creatorId,
        raffle.id,
        channelId,
        qualifyingChannel.subscriber_count,
        claimDay,
        cooldownUntil
      ]);
      ...
      await upsertFreeGeneralCooldown(
        client,
        'user',
        creatorId,
        cooldownUntil,
    ...
    if (String(error?.code || '') === '23505') {
      return sendMessage(target, '⏳ Бесплатная публикация уже активирована или недельный лимит занят другой параллельной заявкой.');
    }
  ...
  const freshRaffle = await getRaffleById(raffle.id) || raffle;
  const result = await scheduleFreeGeneralPublishRequest(claim, freshRaffle);
  const scheduledAt = result?.scheduledAt || result?.queue?.scheduled_at;

  return sendMessage(
    target,
    [
      '🎁 **БЕСПЛАТНАЯ публикация активирована!**',
      '',
      `Ваш канал: **${displayValue(qualifyingChannel.channel_title, qualifyingChannel.channel_id)}**`,
      `Подписчиков подтверждено: **${Number(qualifyingChannel.subscriber_count || 0)}**`,
      '',
      `${formatGeneralChannelMention()} добавлен в условия розыгрыша как **обязательная подписка без встречного размещения**.`,
      '',
      String(result?.status || '') === 'request_queued'
        ? 'Розыгрыш будет поставлен в очередь General сразу после его запуска.'
        : formatFreeGeneralQueueResult(result),
      scheduledAt ? `Плановое время: **${formatDateTime(scheduledAt)}**` : '',
      '',
      `Лимит: **1 бесплатная публикация каждые ${FREE_GENERAL_COOLDOWN_DAYS} суток** одновременно для аккаунта и для канала.`,
      `Следующий бесплатный слот: **${formatFreeGeneralNextAvailable(claim.cooldown_until || cooldownUntil)}**.`
    ].filter(Boolean).join('\n')
  );
}

async function sendFreeGeneralOfferAfterCreate(target, userId, raffle) {
  const raffleId = Number(raffle?.id || 0);
  const requesterId = String(userId || '').trim();

  console.log('🎁 FREE GENERAL offer check started:', {
    raffle_id: raffleId || null,
    user_id: requesterId || null,
    raffle_status: String(raffle?.status || '') || null,
    general_channel_id: String(GENERAL_CHANNEL_ID || '') || null,
    enabled: FREE_GENERAL_ENABLED,
    min_subscribers_exclusive: FREE_GENERAL_MIN_SUBSCRIBERS,
    cooldown_days: FREE_GENERAL_COOLDOWN_DAYS
  });

  let eligibility;

  try {
    eligibility = await getFreeGeneralEligibility(raffleId, requesterId, { refresh: true });
  } catch (error) {
    console.error('❌ FREE GENERAL offer eligibility error:', {
      raffle_id: raffleId || null,
      user_id: requesterId || null,
  ...
  const channel = eligibility?.qualifyingChannel || null;

  console.log('🎁 FREE GENERAL offer eligibility result:', {
    raffle_id: raffleId || null,
    user_id: requesterId || null,
    eligible: Boolean(eligibility?.eligible),
    reason: String(eligibility?.reason || 'unknown'),
    raffle_status: String(eligibility?.raffle?.status || raffle?.status || '') || null,
    qualifying_channel_id: channel?.channel_id ? String(channel.channel_id) : null,
    qualifying_channel_title: channel?.channel_title || null,
    subscriber_count: channel?.subscriber_count !== undefined && channel?.subscriber_count !== null
      ? Number(channel.subscriber_count)
      : null,
    next_available_at: eligibility?.nextAvailableAt || eligibility?.cooldown?.next_available_at || null,
    existing_claim_id: eligibility?.claim?.id || null,
    existing_claim_status: eligibility?.claim?.status || null
  });

  if (!eligibility?.eligible) {
    console.warn('⚠️ FREE GENERAL offer not shown:', {
      raffle_id: raffleId || null,
      user_id: requesterId || null,
      reason: String(eligibility?.reason || 'unknown')
    });
  ...
  try {
    await sendMessage(
      target,
      [
        '🎁 **Вам доступна БЕСПЛАТНАЯ публикация!**',
        '',
        `В розыгрыш добавлен ваш канал **${displayValue(channel.channel_title, channel.channel_id)}** с подтверждённой аудиторией **${Number(channel.subscriber_count || 0)}** подписчиков.`,
        '',
        `Вы можете бесплатно опубликовать этот розыгрыш в ${formatGeneralChannelMention()}.`,
        '',
        '⭐ **Преимущество:** в нашем канале собрана целевая аудитория людей, которые специально ищут розыгрыши и активно участвуют в них.',
        '',
        `Условие: ${formatGeneralChannelMention()} автоматически добавится в розыгрыш как обязательная подписка **без размещения поста в вашем канале от нашего имени**.`,
        '',
        `Доступно **1 раз каждые ${FREE_GENERAL_COOLDOWN_DAYS} суток**. Лимит действует и на ваш аккаунт, и на сам канал.`
      ].join('\n'),
      [[{ text: '🎁 БЕСПЛАТНО', callback_data: `free_general_claim:${raffleId}` }]]
    );
  } catch (error) {
    console.error('❌ FREE GENERAL offer send failed:', {
      raffle_id: raffleId || null,
      user_id: requesterId || null,
  ...
  console.log('✅ FREE GENERAL offer sent:', {
    raffle_id: raffleId || null,
    user_id: requesterId || null,
    target: String(target || '') || null,
    channel_id: String(channel.channel_id || ''),
...
async function processFreeGeneralPublishRequestQueuedItem(item) {
  const payload = item?.payload && typeof item.payload === 'object' ? item.payload : {};
  const claimId = Number(payload.free_general_claim_id);
  const queueId = Number(item?.id || 0);
  const queueRaffleId = Number(item?.raffle_id || 0);

  console.log('📥 FREE GENERAL request queue processing started:', {
    queue_id: queueId || null,
    queue_raffle_id: queueRaffleId || null,
    queue_status: String(item?.status || '') || null,
    scheduled_at: item?.scheduled_at || null,
    claim_id: Number.isInteger(claimId) ? claimId : null,
    payload_reason: payload.reason || null
  });

  if (!Number.isInteger(claimId)) {
    console.error('❌ FREE GENERAL request missing claim id:', {
      queue_id: queueId || null,
      queue_raffle_id: queueRaffleId || null,
      payload
    });
    return { status: 'missing_free_claim_id' };
  }
  ...
  const claim = claimRes.rows[0] || null;
  if (!claim) {
    console.error('❌ FREE GENERAL request claim not found:', {
      queue_id: queueId || null,
      claim_id: claimId
    });
    return { status: 'missing_free_claim' };
  ...
  console.log('🔎 FREE GENERAL request claim loaded:', {
    queue_id: queueId || null,
    claim_id: claim.id,
    raffle_id: claim.raffle_id,
    user_id: String(claim.user_id || '') || null,
    qualifying_channel_id: String(claim.qualifying_channel_id || '') || null,
    claim_status: String(claim.status || '') || null,
    cooldown_until: claim.cooldown_until || null,
    validation_error: claim.validation_error || null
  });

  const raffle = await getRaffleById(claim.raffle_id);
  if (!raffle) {
    console.error('❌ FREE GENERAL request raffle not found:', {
      queue_id: queueId || null,
      claim_id: claim.id,
      raffle_id: claim.raffle_id
    });
    return { status: 'missing_raffle', claim };
  }

  console.log('🔎 FREE GENERAL request raffle loaded:', {
    queue_id: queueId || null,
    claim_id: claim.id,
    raffle_id: raffle.id,
    raffle_status: String(raffle.status || '') || null,
    publish_at: getRafflePublishAt(raffle) || null,
    end_at: raffle.end_at || null
  });

  if (!['scheduled', 'active'].includes(String(raffle.status || ''))) {
    console.warn('⚠️ FREE GENERAL request skipped: raffle is not publishable:', {
      queue_id: queueId || null,
      claim_id: claim.id,
      raffle_id: raffle.id,
      raffle_status: String(raffle.status || '') || null
    });
    return { status: 'skipped_not_publishable', raffle, claim };
  }
  ...
  if (toUtcMoment(raffle.end_at)?.isBefore(dayjs.utc())) {
    ...
    console.warn('⚠️ FREE GENERAL request skipped: raffle already ended:', {
      queue_id: queueId || null,
      claim_id: claim.id,
      raffle_id: raffle.id,
      end_at: raffle.end_at || null
    });
    return { status: 'skipped_ended', raffle, claim };
  }
  ...
  if (String(raffle.status || '') !== 'active') {
    ...
    console.log('⏳ FREE GENERAL request deferred until raffle becomes active:', {
      queue_id: queueId || null,
      claim_id: claim.id,
      raffle_id: raffle.id,
      raffle_status: String(raffle.status || '') || null,
    ...
    return { status: 'deferred_until_active', raffle, claim, keepPending: true };
  }

  let result;

  try {
    result = await scheduleFreeGeneralPublish(raffle.id, claim.id, {
      reason: payload.reason || 'free_large_channel_request_due'
    });
  } catch (error) {
    console.error('❌ FREE GENERAL request scheduling failed:', {
      queue_id: queueId || null,
      claim_id: claim.id,
      raffle_id: raffle.id,
      error: safeText(error?.message || error, 1000),
  ...
  console.log('📤 FREE GENERAL request scheduling result:', {
    queue_id: queueId || null,
    claim_id: claim.id,
    raffle_id: raffle.id,
    result_status: String(result?.status || 'unknown'),
    publish_queue_id: result?.queue?.id || null,
    scheduled_at: result?.scheduledAt || result?.queue?.scheduled_at || null
  });

  if (['queued', 'already_queued'].includes(String(result?.status || ''))) {
    const notifyUserId = claim.user_id || raffle.creator_user_id;

    try {
      await sendMessage(
        notifyUserId,
        [
          `🎁 **Бесплатная публикация в ${formatGeneralChannelMention()} поставлена в очередь.**`,
          '',
          `Розыгрыш № **${getRafflePublicNumber(raffle)}**`,
          `Название: **${displayValue(raffle.title, 'Без названия')}**`,
          '',
      ...
      console.log('✅ FREE GENERAL queue notification sent:', {
        queue_id: queueId || null,
        claim_id: claim.id,
        raffle_id: raffle.id,
        user_id: String(notifyUserId || '') || null,
        result_status: String(result?.status || '')
      });
    } catch (error) {
      console.warn('⚠️ FREE GENERAL queue notification failed:', {
        queue_id: queueId || null,
        claim_id: claim.id,
        raffle_id: raffle.id,
        user_id: String(notifyUserId || '') || null,
...
function formatFreeGeneralQueueResult(result) {
  const scheduledAt = result?.scheduledAt || result?.queue?.scheduled_at;
  const generalChannel = formatGeneralChannelMention();

  if (['queued', 'already_queued'].includes(String(result?.status || '')) && scheduledAt) {
    return `Публикация в ${generalChannel} запланирована на **${formatDateTime(scheduledAt)}**. Интервал между публикациями — **${GENERAL_PROMO_SPACING_MINUTES} мин.**`;
  }

  return `Бот поставит розыгрыш в очередь ${generalChannel} и опубликует его в ближайшее свободное окно.`;
}
...
async function getPaidGeneralPaymentContext(paymentId) {
  const row = await getPromoPaymentByPaymentId(paymentId);
  const product = String(row?.product || '').trim();

  if (!row || row.status !== 'succeeded' || !isGeneralPublicationProduct(product)) {
    return { ok: false, status: 'payment_not_found_or_not_paid', payment: row || null };
  }

  const raffle = await getRaffleById(row.raffle_id);

  if (!raffle) {
    return { ok: false, status: 'raffle_not_found', payment: row };
  }
  ...
  return {
    ...
    raffle,
    product,
    userId: String(row.user_id || raffle.creator_user_id || '').trim()
  };
}

async function deferGeneralPublishRequest(item, delaySeconds = 60, reason = 'waiting_for_active_raffle') {
  ...
  const res = await pool.query(`
    ...
    ORDER BY scheduled_at ASC
    LIMIT 1
  `, [String(payload.payment_id || '').trim(), item.id]);

  const nextAtSql = `NOW() + ($2::text || ' seconds')::interval`;
  const nextPayload = JSON.stringify({
    ...payload,
    deferred_reason: reason,
    deferred_from_queue_id: item.id,
  ...
  if (res.rows[0]) {
    await pool.query(`
      UPDATE raffle_queue
      SET scheduled_at = ${nextAtSql},
          payload = COALESCE(payload, '{}'::jsonb) || $3::jsonb,
          updated_at = NOW()
      WHERE id = $1
    `, [res.rows[0].id, Math.max(30, Number(delaySeconds || 60)), nextPayload]);
  ...
  const insertRes = await pool.query(`
    INSERT INTO raffle_queue (raffle_id, queue_type, scheduled_at, payload, status)
    VALUES ($1, 'general_publish_request', NOW() + ($2::text || ' seconds')::interval, $3::jsonb, 'pending')
    RETURNING *
  `, [
    item.raffle_id,
    Math.max(30, Number(delaySeconds || 60)),
    nextPayload
  ]);
...
async function schedulePaidGeneralPublishRequest(paymentId, options = {}) {
  const ctx = await getPaidGeneralPaymentContext(paymentId);

  if (!ctx.ok) {
    return ctx;
  }

  const { payment, raffle, product, userId } = ctx;
  const requestedAt = toUtcMoment(options.requestedAt) || dayjs.utc();
  const now = dayjs.utc();

  if (!['scheduled', 'active'].includes(String(raffle.status || ''))) {
    return { ok: false, status: 'raffle_not_publishable', raffle, payment };
  }

  if (toUtcMoment(raffle.end_at)?.isBefore(now)) {
    return { ok: false, status: 'raffle_already_ended', raffle, payment };
  }
  ...
  // Один платёж = один выбранный запуск General.
  // Если пользователь нажал «Сейчас»/ввёл время повторно, не создаём и не переносим задачу.
  const existingRequest = await pool.query(`
    SELECT *
    FROM raffle_queue
    WHERE queue_type = 'general_publish_request'
  ...
  if (String(raffle.status || '') === 'active' && !requestedAt.isAfter(now)) {
    const result = await schedulePaidGeneralPublish(raffle.id, {
      userId: options.userId || userId,
      paymentId: payment.payment_id,
      product,
      reason: options.reason || 'paid_general_time_now',
      requestedStartAt: requestedAt.toISOString()
    });

    return {
      ok: ['queued', 'already_queued'].includes(String(result?.status || '')),
      ...result
    };
  }

  const scheduledAt = requestedAt.isAfter(now) ? requestedAt.toDate() : now.toDate();
  const payload = {
    payment_id: payment.payment_id,
    product,
    requested_by: String(options.userId || userId || ''),
    requested_start_at: requestedAt.toISOString(),
    reason: options.reason || 'paid_general_time_selected'
  };
  ...
  const insertRes = await pool.query(`
    ...
    VALUES ($1, 'general_publish_request', $2, $3::jsonb, 'pending')
    RETURNING *
  `, [raffle.id, scheduledAt, JSON.stringify(payload)]);

  const queue = insertRes.rows[0];
...
async function processGeneralPublishRequestQueuedItem(item) {
  const payload = item?.payload && typeof item.payload === 'object' ? item.payload : {};

  if (payload.free_large_channel_publish) {
    return processFreeGeneralPublishRequestQueuedItem(item);
  }
  ...
  const { raffle, payment, product, userId } = ctx;

  if (!['scheduled', 'active'].includes(String(raffle.status || ''))) {
    return { status: 'skipped_not_publishable', raffle, payment };
  }

  if (toUtcMoment(raffle.end_at)?.isBefore(dayjs.utc())) {
    return { status: 'skipped_ended', raffle, payment };
  }

  if (String(raffle.status || '') !== 'active') {
    await deferGeneralPublishRequest(item, 60, 'raffle_not_active_yet');
    return { status: 'deferred_until_active', raffle, payment };
  }
  ...
  const result = await schedulePaidGeneralPublish(raffle.id, {
    ...
    paymentId: payment.payment_id,
    product,
    reason: payload.reason || 'paid_general_request_due',
    requestedStartAt: payload.requested_start_at || null
  });
...
async function handlePaidGeneralPublishNow(target, userId, paymentId) {
  const ctx = await getPaidGeneralPaymentContext(paymentId);

  if (!ctx.ok) {
    return sendMessage(target, 'Оплата не найдена или ещё не подтверждена.');
  }

  if (String(ctx.userId) !== String(userId) && !isAdmin(userId)) {
    return sendMessage(target, '⛔ Управлять этой платной публикацией может только оплативший пользователь или админ.');
  }

  const result = await schedulePaidGeneralPublishRequest(paymentId, {
    userId,
    requestedAt: dayjs.utc().toISOString(),
    reason: 'paid_general_now_button'
  });

  if (['already_queued', 'already_requested'].includes(String(result?.status || ''))) {
    const scheduledAt = result?.scheduledAt || result?.queue?.scheduled_at;

    return sendMessage(
      target,
      [
        'ℹ️ **Для этой оплаты время публикации уже принято.**',
        '',
        scheduledAt ? `Текущий план: **${formatDateTime(scheduledAt)}**` : '',
        'Повторный клик не создаёт новую публикацию и не списывает новую оплату.'
      ].filter(Boolean).join('\n')
    );
  }
  ...
  if (['queued', 'request_queued'].includes(String(result?.status || ''))) {
    ...
    return sendMessage(
      target,
      [
        '✅ **Время публикации принято: сейчас.**',
        '',
        scheduledAt ? `План: **${formatDateTime(scheduledAt)}**` : '',
        line
      ].filter(Boolean).join('\n')
    );
  }

  return sendMessage(target, `⚠️ Не удалось поставить публикацию. Статус: ${displayValue(result?.status, 'неизвестно')}.`);
}

async function startPaidGeneralPublishTimeInput(target, userId, paymentId) {
  const ctx = await getPaidGeneralPaymentContext(paymentId);
  ...
  await setSession(userId, 'await_paid_general_publish_time', {
  ...
  return sendMessage(
    ...
    [
      ...
      '',
      'Когда это время наступит, бот найдёт ближайшее свободное окно и опубликует пост.'
    ].join('\n'),
    [[{ text: '❌ Отмена', callback_data: 'cancel_session' }]]
  );
}

async function handlePaidGeneralPublishTimeMessage(message, data = {}) {
  const userId = message.from.id;
  const target = message.chat.id;
  const paymentId = String(data.payment_id || '').trim();
  const parsed = parseEndDate(message.text || '');

  if (!paymentId) {
    await clearSession(userId);
    await sendMessage(target, 'Платёж не найден. Откройте оплату ещё раз.');
    return true;
  }

  if (!parsed || !parsed.isValid()) {
    await sendMessage(target, 'Неверный формат времени. Пример: `2026-06-10 20:00`');
    return true;
  }

  if (parsed.isBefore(dayjs.utc())) {
    await sendMessage(target, `Время должно быть в будущем. Текущее время: **${formatDateTime(dayjs.utc())}**.`);
    return true;
  }
  ...
  const result = await schedulePaidGeneralPublishRequest(paymentId, {
    ...
    reason: 'paid_general_manual_time'
  });
  ...
  if (['already_queued', 'already_requested'].includes(String(result?.status || ''))) {
    ...
    return sendMessage(
      ...
      [
        ...
        '',
        scheduledAt ? `Текущий план: **${formatDateTime(scheduledAt)}**` : '',
        'Повторный ввод времени не создаёт новую публикацию и не переносит уже выбранное время.'
      ].filter(Boolean).join('\n')
    );
  }
  ...
  if (['queued', 'request_queued'].includes(String(result?.status || ''))) {
    return sendMessage(
      target,
      [
        '✅ **Время платной публикации сохранено.**',
        '',
  ...
  return sendMessage(target, `⚠️ Не удалось сохранить публикацию. Статус: ${displayValue(result?.status, 'неизвестно')}.`);
}
...
async function publishPaidGeneralQueuedItem(item) {
  ...
  if (!raffle) {
    throw new Error(`Розыгрыш ${item.raffle_id} не найден для публикации в General`);
  }
  ...
  const payload = item?.payload && typeof item.payload === 'object' ? item.payload : {};
  const isFreeLargeChannelPublish = Boolean(payload.free_large_channel_publish);
  const freeClaimId = Number(payload.free_general_claim_id);
  const isAdminManualGeneralPublish = Boolean(payload.admin_manual_publish);
  const product = String(payload.product || PROMO_GENERAL_PRODUCT_CODE).trim();
  const paymentId = String(payload.payment_id || '').trim();
  let freeClaim = null;

  if (isFreeLargeChannelPublish) {
    if (!Number.isInteger(freeClaimId)) {
      throw new Error('Для бесплатной General-публикации не найден free_general_claim_id');
    }
    ...
    const claimRes = await pool.query(`
      ...
      WHERE id = $1
        AND raffle_id = $2
      LIMIT 1
    `, [freeClaimId, raffle.id]);

    freeClaim = claimRes.rows[0] || null;
    if (!freeClaim) {
      throw new Error(`Бесплатная заявка ${freeClaimId} для розыгрыша ${raffle.id} не найдена`);
    }
    ...
    const duplicateRes = await pool.query(`
      ...
      WHERE queue_type = 'general_publish'
        ...
        AND id <> $2
      LIMIT 1
    `, [String(freeClaimId), item.id]);

    if (duplicateRes.rows.length || String(freeClaim.status || '') === 'published') {
      return { status: 'already_published_for_free_claim', raffle, freeClaim };
    }

    const validation = await validateFreeGeneralClaimForPublish(freeClaim, raffle);

    if (!validation.ok) {
      if (validation.temporary) {
        return deferFreeGeneralValidation(item, freeClaim, validation);
      }

      const reason = String(validation.reason || 'validation_failed');
      await pool.query(`
        UPDATE free_general_publications
        SET
      ...
      await sendMessage(
        ...
        [
          '⛔ **Бесплатная публикация отменена проверкой защиты.**',
          '',
          `Причина: **${describeFreeGeneralValidationFailure(reason)}**.`,
          '',
          'Недельный лимит при этом не возвращается: слот считается использованным с момента активации.'
        ].join('\n')
      ).catch(() => {});

      return {
        status: 'free_validation_blocked',
        reason,
        raffle,
        freeClaim
      };
    ...
    freeClaim.subscriber_count = validation.subscriberCount;
  } else if (!isAdminManualGeneralPublish) {
    if (!isGeneralPublicationProduct(product)) {
      throw new Error(`Неподдерживаемый продукт General-публикации: ${product}`);
    }
    ...
    const paidRes = await pool.query(`
      ...
      WHERE raffle_id = $1
        AND product = $2
        AND status = 'succeeded'
        AND ($3::text = '' OR payment_id = $3)
      LIMIT 1
    `, [raffle.id, product, paymentId]);

    if (!paidRes.rows.length) {
      throw new Error(`Для розыгрыша ${raffle.id} нет успешной оплаты ${product}`);
    }
  }

  if (!isFreeLargeChannelPublish && paymentId) {
  ...
  // General становится обязательным условием только для бесплатной акции.
  // Платные, ТОП-1 и ручные админ-публикации сохраняют прежнее поведение.
  if (isFreeLargeChannelPublish) {
    await ensureGeneralChannelRequiredForRaffle(raffle.id);
  }

  await publishRaffleToChannel(raffle, GENERAL_CHANNEL_ID);

  if (isFreeLargeChannelPublish && freeClaim) {
    await pool.query(`
      UPDATE free_general_publications
  ...
  await sendMessage(
    raffle.creator_user_id,
    [
      isFreeLargeChannelPublish
        ? '🎁 **Бесплатное размещение опубликовано в General-канале.**'
        : (
            isAdminManualGeneralPublish
              ? '✅ **Админ-размещение опубликовано в General-канале.**'
              : (
                  product === PROMO_TOP1_PRODUCT_CODE
                    ? '✅ **ТОП 1 опубликован в General-канале.**'
  ...
  return { status: 'published', raffle, freeClaim };
}
...
async function createParticipantEntry(raffleId, userId, invitedBy = null, sourceChannelId = null) {
  ...
  try {
    ...
    const createdTicketsCount = 1 + (bonusAdded ? 1 : 0);
...
async function markRaffleInLifetimeStats(raffleId) {
  const id = Number(raffleId);
  ...
  try {
    ...
    const countedRes = await client.query(`
      UPDATE raffles
      SET lifetime_counted_at = NOW()
      WHERE id = $1
        AND status IN ('scheduled', 'active', 'finished', 'archived', 'cancelled')
        AND lifetime_counted_at IS NULL
      RETURNING id
    `, [id]);

    if (countedRes.rowCount) {
      await client.query(`
        UPDATE bot_lifetime_stats
        SET raffles_created = raffles_created + 1,
            updated_at = NOW()
        WHERE id = 1
      `);
    }
    ...
    await client.query('COMMIT');
    return Boolean(countedRes.rowCount);
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    throw error;
  } finally {
...
async function getGlobalStats() {
  ...
  const lifetime = await pool.query(`
    SELECT raffles_created, tickets_created
    FROM bot_lifetime_stats
    WHERE id = 1
    LIMIT 1
  `);

  // Fallback нужен только на случай ручного удаления строки счётчиков.
  const fallbackRaffles = lifetime.rows.length
    ? null
    : await pool.query(`
        SELECT COUNT(*)::bigint AS count
        FROM raffles
        WHERE status IN ('scheduled', 'active', 'finished', 'archived', 'cancelled')
      `);
  const fallbackTickets = lifetime.rows.length
    ? null
    : await pool.query(`SELECT COUNT(*)::bigint AS count FROM raffle_participants`);

  return {
    users: users.rows[0].count,
    activeUsers: activeUsers.rows[0].count,
    inactiveUsers: inactiveUsers.rows[0].count,
    raffles: Number(lifetime.rows[0]?.raffles_created ?? fallbackRaffles?.rows[0]?.count ?? 0),
    participants: Number(lifetime.rows[0]?.tickets_created ?? fallbackTickets?.rows[0]?.count ?? 0)
  };
}
...
function buildDateQuickKeyboard(kind, options = {}) {
  ...
  if (options.backStep) {
    keyboard.push([{ text: options.backText || '⬅️ Назад', callback_data: `raffle_step_back:${options.backStep}` }]);
  }
...
async function sendEndDatePrompt(target, data = {}, options = {}) {
  ...
  const text = isEditingExisting
    ...
    : buildCreateStepMessage(6, 'Окончание', [
        '⏰ **Время окончания розыгрыша**',
        '',
        'Время указывается по МСК.',
        `Публикация поста: **${formatDateTime(publishAt)}**.`,
...
const previewText = isEditingExisting
  ...
  : buildCreateStepMessage(8, 'Проверка шаблона', [
      buildRaffleDraftPreviewText(data)
    ]);
...
function buildCreatedRaffleMessage(sessionData, raffle, invite) {
  return [
    buildCreateStepProgress(9, 'Готово'),
    '',
    '✅ **Розыгрыш создан!**',
    '',
...
async function buildRefreshedRaffleMessage(raffle, invite = null, options = {}) {
  ...
  const header = options.header || '🔄 **Розыгрыш обновлён**';
...
function buildCreatedRaffleKeyboard(raffle, userId = null) {
  ...
  const isOwnerOrAdmin = raffle && userId && (
    Number(raffle.creator_user_id) === Number(userId) ||
    isAdmin(userId)
  );

  // Организатор/админ видит полный набор управления.
  if (isOwnerOrAdmin) {
    const isActiveRaffle = status === 'active';
    ...
    if (isActiveRaffle) {
      keyboard.splice(1, 0, [{
        ...
        callback_data: `republish_raffle:${raffle.id}`
      }]);
    }
    ...
    if (status === 'finished' && !raffle.full_data_pruned_at) {
  ...
  // Соадмин/коллаборант видит только безопасные действия.
  // Запуск, завершение, редактирование, переигровка и закрытие ссылки доступны только организатору/админу.
  const keyboard = [
    [{ text: '👀 Предпросмотр поста', callback_data: `preview_raffle:${raffle.id}` }],
    [{ text: '📊 Статистика', callback_data: `raffle_stats:${raffle.id}` }]
  ];
...
async function buildCollabRaffleDetailsMessage(raffle, userId) {
  const res = await pool.query(`
    SELECT
      COUNT(*)::int AS channels_count,
      STRING_AGG(DISTINCT COALESCE(channel_title, channel_id::text), ', ' ORDER BY COALESCE(channel_title, channel_id::text)) AS channels_titles
    FROM raffle_channels
    WHERE raffle_id = $1
      AND owner_user_id::text = $2
  `, [raffle.id, String(userId || '').trim()]);

  const channelsCount = Number(res.rows[0]?.channels_count || 0);
  const channelsTitles = displayValue(res.rows[0]?.channels_titles, 'канал не указан');

  return [
    '🤝 **Розыгрыш в коллаборации**',
    '',
    `№: **${getRafflePublicNumber(raffle)}**`,
    `Название: **${displayValue(raffle.title, 'Без названия')}**`,
    `Статус: **${formatRaffleStatus(raffle.status)}**`,
    ...
    `Окончание: **${formatDateTime(raffle.end_at)}**`,
    '',
    `Ваших каналов в розыгрыше: **${channelsCount}** — ${channelsTitles}`,
    '',
    'Вам доступны только безопасные действия: предпросмотр, статистика и увеличение охвата.',
    'Запуск, завершение, редактирование и закрытие ссылки доступны только организатору розыгрыша.'
  ].join('\n');
}

async function sendRaffleDetails(target, userId, raffleId) {
  const raffle = await getRaffleById(raffleId);
  ...
  const isOwnerOrAdmin = isAdmin(userId) || Number(raffle.creator_user_id) === Number(userId);

  if (!isOwnerOrAdmin) {
    return sendMessage(
      target,
      await buildCollabRaffleDetailsMessage(raffle, userId),
      buildCreatedRaffleKeyboard(raffle, userId)
    );
  }
  ...
  return sendMessage(
    target,
    await buildRefreshedRaffleMessage(raffle, invite, { header: '🎁 **Розыгрыш**' }),
    buildCreatedRaffleKeyboard(raffle, userId)
  );
}
...
async function saveEditedRaffleFromSession(target, userId, data = {}) {
  ...
  const freeGeneralClaim = await getFreeGeneralClaimByRaffle(raffle.id);

  if (freeGeneralClaim) {
    const protectedChannelId = String(freeGeneralClaim.qualifying_channel_id || '').trim();
    const protectedChannel = (data.channels || []).find(
      ch => String(ch.channel_id || '').trim() === protectedChannelId
    );

    if (!protectedChannel) {
      return sendMessage(
        target,
        '⛔ Нельзя удалить канал, по которому уже активирована бесплатная публикация в РОЗЫГРЫШ ТОП.'
      );
    }

    if (!protectedChannel.is_required || !protectedChannel.publish_post) {
      return sendMessage(
        target,
        '⛔ Для канала, который дал бесплатное размещение, до окончания розыгрыша должны оставаться включены обязательная подписка и публикация поста.'
      );
    }

    if (String(protectedChannel.owner_user_id || '').trim() !== String(freeGeneralClaim.user_id || '').trim()) {
      return sendMessage(
        target,
        '⛔ Нельзя изменить владельца канала, по которому активирована бесплатная публикация.'
      );
    }
  }
  ...
  for (const ch of data.channels || []) {
    // General-канал бесплатной акции всегда восстанавливается сервером с безопасными флагами.
    if (freeGeneralClaim && String(ch.channel_id || '').trim() === String(GENERAL_CHANNEL_ID || '').trim()) {
      continue;
    }
...
async function createRaffleAndSendCreatedMessage(target, userId, sessionData) {
  ...
  try {
    created = await createRaffleFromSession(userId, sessionData);
  } catch (error) {
    if (error instanceof RaffleCreationLimitError) {
      await clearSession(userId);
      await sendMessage(target, buildRaffleCreationLimitMessage(error.state));
      return null;
    }
    throw error;
  }

  const { raffle, inviteToken } = created;
  ...
  // Если у организатора есть выбранный канал с аудиторией больше лимита,
  // отдельно показываем бесплатную публикацию в General.
  await sendFreeGeneralOfferAfterCreate(target, userId, createdRaffle).catch(error => {
    console.warn('Не удалось показать бесплатное General-предложение:', error.message);
  });

  // Платное предложение автоматически не отправляем.
...
function buildRaffleText(raffle, participantsCount = 0, channels = [], sourceChannelId = null) {
  ...
  const actionUrl = isRaffleFinishedStatus(raffle)
    ? buildRaffleResultsUrl(raffle.id)
    : buildBotDeepLink(buildRaffleJoinPayload(raffle.id, sourceChannelId));

  const actionLine = isRaffleFinishedStatus(raffle)
    ? (
        actionUrl
          ? `*Розыгрыш завершён. Итоги доступны по кнопке ${markdownLink('👀Глянуть Итоги', actionUrl)}*`
          : '*Розыгрыш завершён. Нажмите кнопку 👀Глянуть Итоги, чтобы посмотреть победителей.*'
      )
    : (
        actionUrl
          ? `*Чтобы участвовать, нажмите кнопку ${markdownLink('🎁 Участвовать', actionUrl)}*`
...
function buildRaffleResultsPayload(raffleId) {
  return `results_${Number(raffleId)}`;
}

function buildRaffleResultsUrl(raffleId) {
  return buildBotDeepLink(buildRaffleResultsPayload(raffleId));
}

function isRaffleFinishedStatus(raffle) {
  return String(raffle?.status || '').toLowerCase() === 'finished';
}

function buildRafflePostActionButton(raffle, sourceChannelId = null) {
  if (isRaffleFinishedStatus(raffle)) {
    return {
      text: '👀Глянуть Итоги',
      url: buildRaffleResultsUrl(raffle.id)
    };
  }

  return {
    text: '🎁 Участвовать',
    url: buildBotDeepLink(buildRaffleJoinPayload(raffle.id, sourceChannelId))
  };
}
...
async function updateRafflePublishedPosts(raffleId, options = {}) {
  ...
  for (const post of postsRes.rows) {
    ...
    const actionButton = buildRafflePostActionButton(raffle, post.channel_id);
...
async function publishRaffleToChannel(raffle, channelId) {
  ...
  const actionButton = buildRafflePostActionButton(raffle, channelId);
...
function normalizeRaffleRepublishChannelIds(values = []) {
...
async function getRaffleRepublishContext(raffleId, userId) {
  const id = Number(raffleId);
  const cleanUserId = String(userId || '').trim();

  if (!Number.isInteger(id) || id <= 0) {
    throw new Error('Некорректный ID розыгрыша');
  }
  ...
  const canManage =
    String(raffle.creator_user_id || '') === cleanUserId ||
    isAdmin(cleanUserId);

  if (!canManage) {
    throw new Error(
      'Повторно публиковать розыгрыш может только организатор или администратор'
    );
  }
  ...
  if (
    String(raffle.status || '') !== 'active' ||
    !endAt ||
    !endAt.isAfter(dayjs.utc())
  ) {
    throw new Error(
      'Повторная публикация доступна только для активного незавершённого розыгрыша'
    );
  }

  const channels = (await getRaffleChannels(id))
    .filter(channel => Boolean(channel.publish_post));
...
function getRaffleRepublishSessionData(session) {
  const data = typeof session?.data === 'string'
    ? JSON.parse(session.data || '{}')
    : session?.data || {};
  ...
  return {
    ...
    selected_channel_ids: normalizeRaffleRepublishChannelIds(
      data.selected_channel_ids
    )
  };
}

async function sendRaffleRepublishChannelSelection(
  target,
  userId,
  raffleId,
  options = {}
) {
  const context = await getRaffleRepublishContext(raffleId, userId);
  const session = await getSession(userId);
  const currentData = getRaffleRepublishSessionData(session);

  const shouldKeepSession =
    ['await_raffle_republish_channels', 'confirm_raffle_republish'].includes(
      String(session?.state || '')
    ) &&
    Number(currentData.raffle_id) === Number(context.raffle.id) &&
    options.reset !== true;

  const allowedIds = new Set(
    context.channels.map(channel => String(channel.channel_id))
  );

  const selected = shouldKeepSession
    ? currentData.selected_channel_ids.filter(channelId => allowedIds.has(channelId))
    : [];

  const totalPages = Math.max(
    1,
    Math.ceil(context.channels.length / RAFFLE_REPUBLISH_PAGE_SIZE)
  );

  const requestedPage = options.page !== undefined
    ? Number(options.page)
    : Number(currentData.page || 0);

  const page = Math.min(
    totalPages - 1,
    Math.max(0, Number.isFinite(requestedPage) ? requestedPage : 0)
  );

  const data = {
    raffle_id: Number(context.raffle.id),
  ...
  await setSession(
    userId,
    'await_raffle_republish_channels',
    data
  );

  const selectedSet = new Set(selected);
  const pageChannels = context.channels.slice(
    page * RAFFLE_REPUBLISH_PAGE_SIZE,
    (page + 1) * RAFFLE_REPUBLISH_PAGE_SIZE
  );

  const lines = [
    '📣 **Повторная публикация розыгрыша**',
    '',
    `№: **${getRafflePublicNumber(context.raffle)}**`,
    `Название: **${displayValue(context.raffle.title, 'Без названия')}**`,
    '',
    'Выберите каналы, в которых бот создаст **новый пост** с этим розыгрышем.',
    'Доступны только каналы, для которых ранее было выбрано **«с размещением»**.',
    '',
    'Старые посты не удаляются и не изменяются.'
  ];

  if (options.notice) {
    lines.push('', String(options.notice));
  }

  if (!context.channels.length) {
    lines.push(
      '',
      '⚠️ В этом розыгрыше нет каналов с включённым размещением.'
    );
  } else if (!selected.length) {
    lines.push('', 'Пока не выбран ни один канал.');
  } else {
    lines.push('', '**Выбрано:**');

    for (const channel of context.channels) {
      if (!selectedSet.has(String(channel.channel_id))) continue;
      lines.push(`• ${formatChannelWithLink(channel)}`);
    }
  }
  ...
  if (totalPages > 1) {
    lines.push(
      '',
      `Страница каналов: **${page + 1}/${totalPages}**`
    );
  }
  ...
  for (const channel of pageChannels) {
    ...
    const selectedNow = selectedSet.has(channelId);

    keyboard.push([{
      text: `${selectedNow ? '✅' : '⬜'} ${truncateButtonText(
        formatChannelName(channel),
        34
      )}`,
      callback_data:
        `republish_ch_toggle:${context.raffle.id}:${safeCallbackPart(channelId)}`
    }]);
  }
  ...
  if (totalPages > 1) {
    ...
    if (page > 0) {
      navigation.push({
        text: '⬅️',
        callback_data: `republish_page:${context.raffle.id}:${page - 1}`
      });
    }

    navigation.push({
      text: `${page + 1}/${totalPages}`,
      callback_data: `republish_page:${context.raffle.id}:${page}`
    });
  ...
  if (selected.length) {
    keyboard.push([{
      text: `➡️ Продолжить (${selected.length})`,
      callback_data: `republish_review:${context.raffle.id}`
    }]);
  }

  keyboard.push([{
    text: '❌ Отмена',
    callback_data: `republish_cancel:${context.raffle.id}`
  }]);

  const editMessageId = String(options.editMessageId || '').trim();

  if (editMessageId) {
    const edited = await editMaxMessageText(
      target,
      editMessageId,
      lines.join('\n'),
      keyboard
...
async function toggleRaffleRepublishChannel(
  userId,
  raffleId,
  channelId
) {
  const context = await getRaffleRepublishContext(raffleId, userId);
  const session = await getSession(userId);
  const data = getRaffleRepublishSessionData(session);

  if (
    String(session?.state || '') !== 'await_raffle_republish_channels' ||
    Number(data.raffle_id) !== Number(context.raffle.id)
  ) {
    throw new Error(
      'Выбор каналов устарел. Откройте повторную публикацию заново'
    );
  }

  const cleanChannelId = String(channelId || '').trim();
  const allowed = context.channels.some(
    channel => String(channel.channel_id) === cleanChannelId
  );

  if (!allowed) {
    throw new Error(
      'Этот канал больше недоступен для повторной публикации'
    );
  }

  const selected = new Set(data.selected_channel_ids);

  if (selected.has(cleanChannelId)) {
    selected.delete(cleanChannelId);
  } else {
    selected.add(cleanChannelId);
  }

  data.selected_channel_ids = [...selected];
...
async function sendRaffleRepublishConfirmation(
  target,
  userId,
  raffleId
) {
  ...
  const selectedIds = data.selected_channel_ids.filter(
    channelId => allowedMap.has(channelId)
  );

  if (!selectedIds.length) {
    return sendRaffleRepublishChannelSelection(
      target,
      userId,
      context.raffle.id,
      {
        notice: '⚠️ Выберите хотя бы один канал.'
      }
    );
  }

  data.selected_channel_ids = selectedIds;

  await setSession(
    userId,
    'confirm_raffle_republish',
    data
  );

  const lines = [
    '⚠️ **Подтвердите повторную публикацию**',
    '',
    `Розыгрыш: **${displayValue(context.raffle.title, 'Без названия')}**`,
    '',
    'Новые посты будут отправлены в следующие каналы:',
    ...selectedIds.map(
      channelId => `• ${formatChannelWithLink(allowedMap.get(channelId))}`
    ),
    '',
    `Всего каналов: **${selectedIds.length}**`,
    '',
    'Старые публикации останутся в каналах.'
  ];

  return sendMessage(target, lines.join('\n'), [
    [{
      text: '✅ Да, опубликовать',
      callback_data: `republish_confirm:${context.raffle.id}`
    }],
    [{
      text: '⬅️ Изменить выбор',
      callback_data: `republish_back:${context.raffle.id}`
    }],
    [{
      text: '❌ Отмена',
      callback_data: `republish_cancel:${context.raffle.id}`
...
async function enqueueRaffleRepublishFromSession(
  target,
  userId,
  raffleId
) {
  ...
  if (
    String(session?.state || '') !== 'confirm_raffle_republish' ||
    Number(data.raffle_id) !== Number(context.raffle.id)
  ) {
    throw new Error(
      'Подтверждение устарело. Откройте повторную публикацию заново'
    );
  }
  ...
  if (!selectedIds.length) {
    throw new Error('Не выбран ни один доступный канал');
  }

  const payload = {
    requested_by: String(userId),
    target: String(target),
    selected_channel_ids: selectedIds,
    requested_at: new Date().toISOString()
  };

  const inserted = await pool.query(`
    INSERT INTO raffle_queue (
      raffle_id,
      queue_type,
      scheduled_at,
      payload,
    ...
    VALUES ($1, 'raffle_republish', NOW(), $2::jsonb, 'pending', NOW())
  ...
  if (!inserted.rows[0]) {
    return sendMessage(
      target,
      [
        '⏳ Для этого розыгрыша уже выполняется повторная публикация.',
        '',
        'Дождитесь итогового отчёта, прежде чем запускать её снова.'
      ].join('\n'),
      buildCreatedRaffleKeyboard(context.raffle, userId)
    );
  }
  ...
  await sendMessage(
    ...
    [
      ...
      '',
      'Бот проверит каждый канал отдельно и пришлёт отчёт: где публикация прошла, а где возникла ошибка.'
    ].join('\n'),
    buildCreatedRaffleKeyboard(context.raffle, userId)
  );

  // Не ждём следующего фонового интервала — мягко запускаем обработчик сразу.
  setImmediate(() => {
    processQueue().catch(error => {
      console.error(
        'Не удалось сразу запустить очередь повторной публикации:',
        error.message
      );
    });
  });
...
async function processRaffleRepublishQueuedItem(item) {
  const raffleId = Number(item?.raffle_id);
  const payload = typeof item?.payload === 'string'
    ? JSON.parse(item.payload || '{}')
    : item?.payload || {};
  ...
  const target = String(payload.target || requestedBy || '').trim();
  const selectedIds = normalizeRaffleRepublishChannelIds(
  ...
  if (
    ...
    if (target) {
      await sendMessage(
        target,
        '⚠️ Повторная публикация отменена: розыгрыш уже не активен.'
      ).catch(() => {});
    }
  ...
  const allChannels = await getRaffleChannels(raffle.id);
  const allowedMap = new Map(
    allChannels
      .filter(channel => Boolean(channel.publish_post))
      .map(channel => [String(channel.channel_id), channel])
  );

  const channels = selectedIds
    .map(channelId => allowedMap.get(channelId))
    .filter(Boolean);
  ...
  for (const channel of channels) {
    try {
      ...
      console.error('Ошибка повторной публикации розыгрыша:', {
        raffle_id: raffle.id,
        channel_id: String(channel.channel_id),
        error: error.message
      });
    ...
    if (RAFFLE_REPUBLISH_SEND_DELAY_MS > 0) {
      await sleep(RAFFLE_REPUBLISH_SEND_DELAY_MS);
    }
  }

  for (const channelId of selectedIds) {
    if (allowedMap.has(channelId)) continue;

    failed.push({
      channel: {
        channel_id: channelId,
        channel_title: `Канал ${channelId}`
      },
      error: 'Канал больше не отмечен как «с размещением»'
    });
  }

  if (target) {
    const lines = [
      '📣 **Повторная публикация завершена**',
      '',
      `Розыгрыш: **${displayValue(raffle.title, 'Без названия')}**`,
    ...
    if (failed.length) {
      lines.push('', '⚠️ **Не удалось опубликовать:**');
      for (const itemResult of failed) {
        lines.push(
          `• ${formatChannelWithLink(itemResult.channel)} — ${displayValue(
            itemResult.error,
            'неизвестная ошибка'
          )}`
        );
    ...
    await sendMessage(
      target,
      lines.join('\n'),
      buildCreatedRaffleKeyboard(raffle, requestedBy)
    ).catch(error => {
      console.warn(
        'Не удалось отправить отчёт о повторной публикации:',
        error.message
      );
    });
  }
...
async function startRaffleNow(target, userId, raffleId) {
  ...
  // Если FREE GENERAL был активирован для запланированного розыгрыша,
  // при ручном досрочном запуске переносим только ожидающий бесплатный запрос на сейчас.
  // Платные запросы General и уже выполняющиеся задачи не затрагиваются.
  await pool.query(`
    UPDATE raffle_queue
    SET scheduled_at = NOW(), updated_at = NOW()
    WHERE raffle_id = $1
      AND queue_type = 'general_publish_request'
      AND status = 'pending'
      AND payload->>'free_large_channel_publish' = 'true'
  `, [raffle.id]).then(result => {
    if (result.rowCount > 0) {
      console.log('🚀 FREE GENERAL request accelerated after manual raffle start:', {
        raffle_id: raffle.id,
        queue_count: result.rowCount
      });
    }
  }).catch(error => {
    console.warn('Не удалось ускорить FREE GENERAL после ручного запуска:', error.message);
  });
...
async function sendRaffleResultsToUser(target, userId, raffleId) {
  const raffle = await getRaffleById(raffleId);
  ...
  if (!isRaffleFinishedStatus(raffle)) {
    return sendMessage(
      target,
      [
        `Розыгрыш **${displayValue(raffle.title, 'Без названия')}** ещё не завершён.`,
        '',
        `Окончание: **${formatDateTime(raffle.end_at)}**`
      ].join('\n'),
...
async function refreshFinishedRafflePosts(raffleId, reason = 'raffle_finished') {
  const result = await updateRafflePublishedPosts(raffleId, {
    force: true,
    reason
  });

  console.log('🏁 Посты завершённого розыгрыша обновлены:', {
    raffle_id: raffleId,
    reason,
    updated: result.updated,
    skipped: result.skipped,
...
async function cancelPendingFreeGeneralForFinishedRaffle(raffleId) {
  const client = await pool.connect();
  ...
  try {
    ...
    const queueRes = await client.query(`
      ...
      WHERE raffle_id = $1
        AND queue_type IN ('general_publish_request', 'general_publish')
        AND status = 'pending'
        AND payload->>'free_large_channel_publish' = 'true'
      RETURNING id
    `, [raffleId]);
    ...
    const claimRes = await client.query(`
      UPDATE free_general_publications
      SET
        status = 'cancelled_raffle_finished',
        validation_error = 'raffle_finished_before_publication',
        updated_at = NOW()
      WHERE raffle_id = $1
        AND status IN ('claimed', 'waiting_for_active', 'queued', 'validation_wait')
      RETURNING id
    `, [raffleId]);

    await client.query('COMMIT');

    if (queueRes.rowCount > 0 || claimRes.rowCount > 0) {
      console.log('⏹ FREE GENERAL cancelled because raffle finished:', {
        raffle_id: raffleId,
        cancelled_queue_count: queueRes.rowCount,
        cancelled_claim_count: claimRes.rowCount
      });
    }

    return {
      cancelledQueueCount: queueRes.rowCount,
      cancelledClaimCount: claimRes.rowCount
    };
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    throw error;
...
async function finishRaffle(raffleId) {
  ...
  if (!participants.length) {
    ...
    await cancelPendingFreeGeneralForFinishedRaffle(raffle.id).catch(error => {
      console.warn('Не удалось очистить FREE GENERAL после завершения розыгрыша без победителей:', error.message);
    });
    await publishRaffleResults(raffle, []);
    await refreshFinishedRafflePosts(raffle.id).catch(error => {
      console.warn('Не удалось обновить посты после завершения розыгрыша без победителей:', error.message);
    });
    return;
  }
  ...
  await updateRaffle(raffle.id, { status: 'finished' });
  await cancelPendingFreeGeneralForFinishedRaffle(raffle.id).catch(error => {
    console.warn('Не удалось очистить FREE GENERAL после завершения розыгрыша:', error.message);
  });
  ...
  await refreshFinishedRafflePosts(raffle.id).catch(error => {
    console.warn('Не удалось обновить посты после завершения розыгрыша:', error.message);
  });
}
...
async function rerollRaffleWinners(target, userId, raffleId) {
  ...
  if (raffle.full_data_pruned_at) {
    return sendMessage(
      target,
      `Архив участников этого розыгрыша уже очищен после ${FINISHED_RAFFLE_FULL_DATA_DAYS} дней хранения. Переигровка победителей больше недоступна, старые итоги сохранены.`
    );
  }
  ...
  await notifyWinners(raffle, winnersRows, prizes);
  await publishRaffleResults(raffle, winnersRows);
  await refreshFinishedRafflePosts(raffle.id, 'winners_rerolled').catch(error => {
    console.warn('Не удалось обновить посты после переигровки победителей:', error.message);
  });
...
async function processQueue() {
  ...
  for (const item of res.rows) {
    try {
      ...
      if (item.queue_type === 'general_publish_request') {
        queueResult = await processGeneralPublishRequestQueuedItem(item);
      }

      if (item.queue_type === 'general_publish') {
        queueResult = await publishPaidGeneralQueuedItem(item);
      }
      ...
      if (item.queue_type === 'raffle_republish') {
        queueResult = await processRaffleRepublishQueuedItem(item);
      }

      if (!queueResult?.keepPending) {
        await pool.query(`
          UPDATE raffle_queue
          SET status = 'done', updated_at = NOW()
          WHERE id = $1
...
async function sendRaffleStats(target, raffleId, requesterUserId = null) {
  ...
  const isParticipantArchivePruned = Boolean(raffle.full_data_pruned_at);
  const displayedParticipantsCount = isParticipantArchivePruned
    ? Number(raffle.archived_participants_count || 0)
    : Number(totalUsers.rows[0]?.count || 0);

  text += `Участников: ${displayedParticipantsCount}\n`;

  if (isParticipantArchivePruned) {
    text += `Архив: подробные данные билетов, источников и подписок очищены после ${FINISHED_RAFFLE_FULL_DATA_DAYS} дней хранения.\n`;
  } else {
  ...
  } else if (isParticipantArchivePruned) {
    for (const ch of channels) {
      text += `• ${formatChannelWithLink(ch)}\n`;
    }

    text += '\nℹ️ Детальные показатели по каналам больше не показываются, потому что архив участников уже очищен.\n';
  } else {
...
async function handleSessionMessage(message) {
  ...
  if (await horseRacesModule.handleSessionMessage(message, session)) {
    return true;
  }
  ...
  if (state === 'await_paid_general_publish_time') {
    return handlePaidGeneralPublishTimeMessage(message, data);
  }
  ...
  if (state === 'await_photo') {
    ...
    if (!photo) {
      await sendRafflePhotoStepPrompt(target, data);
      return true;
    }
    ...
    if (!photo) {
      await sendMessage(
        ...
        [
          ...
          '',
          `👆 Нету своего крутого фото❓ **${markdownLink('Наш ИИ', 'https://max.ru/id236700415542_bot')}** сделает его бесплатно за секунды.`,
          '',
          'Если фото не нужно, вернитесь к шаблону и нажмите «Убрать фото».'
        ].join('\n')
      );
      return true;
    }
  ...
  if (state === 'await_prizes') {
    ...
    await setSession(userId, 'await_photo', data);
    await sendRafflePhotoStepPrompt(target, data);
    return true;
  }
  if (state === 'await_publish_date' || state === 'edit_publish_date') {
    ...
    await setSession(userId, 'await_end_date', data);
    await sendEndDatePrompt(target, data, {
      backStep: 'publish_date',
      backText: '⬅️ Назад к публикации'
    });
    return true;
  }
...
function buildWhyUsText() {
  const generalChannel = formatGeneralChannelMention();
  const aiPhotoBot = markdownLink('ИИ-фото', 'https://max.ru/id236700415542_bot');

  return [
    '🍒 **Почему мы?**',
    '',
    'Мы сделали не просто бота для кнопки «участвовать», а набор инструментов для роста ваших каналов и честных розыгрышей:',
    '',
    '1. ✅ **Честные розыгрыши** — билеты, участники и победители фиксируются в системе.',
    `2. 🖼 **Бесплатное ${aiPhotoBot}** — можно быстро сделать красивое фото к вашему розыгрышу.`,
    '3. ⏰ **АВТОПОСТ** — помогает заранее готовить и публиковать посты в ваши каналы.',
    '4. 🧠 **Развлекательные тесты** — дополнительный контент для вовлечения вашей аудитории.',
    `5. 📣 **Публикация в ${generalChannel}** — продвигаем ваш розыгрыш в публичном канале и даём повторное размещение с большой скидкой.`,
    '6. 🔔 **Напоминания участникам** — бот напомнит людям подписаться на ваши каналы, если они нажали участие, но не завершили проверку.',
    '',
    'Это помогает не просто провести розыгрыш, а получить больше живых участников и пользы для канала.'
  ].join('\n');
}

async function sendWhyUsMessage(target) {
  return sendMessage(
    target,
    buildWhyUsText(),
    [
...
async function sendUserJoinedRafflesMenu(target, userId) {
  const res = await pool.query(`
    SELECT
      r.id,
      r.title,
      ...
      ARRAY_AGG(p.ticket_number ORDER BY p.created_at ASC) AS ticket_numbers
    ...
    WHERE p.user_id = $1
      ...
      AND r.status = 'active'
      AND r.end_at > NOW()
    GROUP BY r.id, r.title, r.end_at
    ORDER BY r.end_at ASC, r.id DESC
    LIMIT 20
  `, [userId]);
  ...
  if (!res.rows.length) {
    return sendMessage(
      target,
      [
        '🎟️ **Вы пока не участвуете в активных розыгрышах.**',
        '',
        'Откройте список активных розыгрышей и выберите приз, где хотите получить билет.'
      ].join('\n'),
      [
        [{ text: '🎁 Активные розыгрыши', callback_data: 'join_latest' }],
        [{ text: '🏠 Главное меню', callback_data: 'main_menu' }]
      ]
    );
  }
  ...
  const lines = [
    '🎟️ **Розыгрыши, где у вас уже есть билет:**',
    ''
  ];
  ...
  for (const raffle of res.rows) {
    const number = getRafflePublicNumber(raffle);
    const title = displayValue(raffle.title, 'Без названия');
    const buttonTitle = truncateButtonText(title, 28);
    const tickets = Array.isArray(raffle.ticket_numbers)
      ? raffle.ticket_numbers.slice(0, 5).map(ticket => `№${ticket}`).join(', ')
      : '';
    const ticketsTail = Number(raffle.ticket_count || 0) > 5
      ? ` и ещё ${Number(raffle.ticket_count || 0) - 5}`
      : '';

    lines.push(
      `• № **${number}** — ${title}`,
      `  🎟️ Билетов: **${raffle.ticket_count}**${tickets ? ` (${tickets}${ticketsTail})` : ''}`,
      `  ⏰ До: **${formatDateTime(raffle.end_at)}**`,
      ''
    );

    keyboard.push([{ text: `🎟️ №${number} ${buttonTitle}`, callback_data: `join_raffle:${raffle.id}` }]);
  }

  keyboard.push([{ text: '🎁 Активные розыгрыши', callback_data: 'join_latest' }]);
  keyboard.push([{ text: '🏠 Главное меню', callback_data: 'main_menu' }]);

  return sendMessage(target, lines.join('\n').trim(), keyboard);
}

async function sendTop1RaffleSelection(target, userId) {
  const res = await pool.query(`
  ...
  if (!res.rows.length) {
    return sendMessage(
      target,
      [
        'У вас сейчас нет активных розыгрышей для покупки ТОП 1.',
        '',
        'ТОП 1 можно купить только для розыгрыша, который уже активен и ещё не завершён.'
      ].join('\n'),
      [
        [{ text: '🎉 Создать розыгрыш', callback_data: 'create_raffle' }],
        [{ text: '🏠 Главное меню', callback_data: 'main_menu' }]
  ...
  const lines = [
    '🥇 **Купить ТОП 1**',
    '',
    `Стоимость: **${Number(PROMO_TOP1_PRICE_RUB).toFixed(0)} ₽**.`,
    '',
    'Выберите активный розыгрыш, который нужно поднять на первое место в меню «Участвовать»:'
  ];
  ...
  for (const raffle of res.rows) {
    const number = getRafflePublicNumber(raffle);
    const title = truncateButtonText(displayValue(raffle.title, 'Без названия'), 30);

    lines.push(`• № **${number}** — ${displayValue(raffle.title, 'Без названия')} | до ${formatDateTime(raffle.end_at)}`);
    keyboard.push([{ text: `🥇 №${number} ${title}`, callback_data: `top1_select:${raffle.id}` }]);
  }
...
async function sendActiveRafflesJoinMenu(target, userId) {
  const res = await pool.query(`
    ...
    ORDER BY
      ...
      r.id DESC
    LIMIT $1
  `, [ACTIVE_RAFFLES_JOIN_LIMIT]);


  if (!res.rows.length) {
    const keyboard = [
      [{ text: `🥇 Купить ТОП 1 — ${Number(PROMO_TOP1_PRICE_RUB).toFixed(0)} ₽`, callback_data: 'top1_buy' }],
      [{ text: '🎟️ Участвую', callback_data: 'joined_raffles' }]
    ];
    ...
    return sendMessage(
      ...
      [
        ...
        '',
        'Если у вас есть активный розыгрыш, можно купить ТОП 1 и поднять его в меню участия.',
        '',
        'Уже участвуете в каком-то розыгрыше? Нажмите **🎟️ Участвую**.'
      ].join('\n'),
      keyboard
    );
  }
  ...
  const lines = [
    `🎁 **Активные розыгрыши:** показываем до **${ACTIVE_RAFFLES_JOIN_LIMIT}** розыгрышей`,
    '',
  ...
  for (let i = 0; i < res.rows.length; i++) {
    const raffle = res.rows[i];
    const number = getRafflePublicNumber(raffle);
    const title = truncateButtonText(displayValue(raffle.title, 'Без названия'), 28);
    const isTop = i === 0 && raffle.boosted_at;

    lines.push(`${isTop ? '🥇 ТОП 1 — ' : '• '}№ **${number}** — ${displayValue(raffle.title, 'Без названия')} | до ${formatDateTime(raffle.end_at)}`);

    keyboard.push([
      {
        text: `${isTop ? '🥇' : '🎁'} №${number} ${title}`,
...
async function sendAdminPanel(target, userId) {
  ...
  return sendMessage(
    ...
    [
      ...
      [{ text: '🔥 Активные розыгрыши', callback_data: 'admin_active' }],
      [{ text: '🐎 Активные скачки', callback_data: 'horse_admin_active' }],
...
async function sendAdminActiveRaffles(target, userId, page = 0) {
  if (!isAdmin(userId)) {
    return sendMessage(target, '⛔ Нет доступа.');
  }

  // MAX ограничивает количество строк inline-кнопок.
  ...
  let text = [
    ...
    '',
    `Страница **${currentPage + 1}/${maxPage + 1}**, всего: **${total}**`,
    ''
  ].join('\n');
  ...
  for (const r of res.rows) {
    ...
    if (String(r.status || '') === 'active') {
      keyboard.push([{
        text: `📨 В бот всем #${r.id}`,
        callback_data: `admin_raffle_broadcast_preview:${r.id}`
      }]);
    }
  }
  ...
  if (maxPage > 0) {
    ...
    if (currentPage > 0) {
      navRow.push({ text: '⬅️ Назад', callback_data: `admin_active:${currentPage - 1}` });
    }

    if (currentPage < maxPage) {
      navRow.push({ text: '➡️ Далее', callback_data: `admin_active:${currentPage + 1}` });
    }
...
async function buildActiveRaffleBroadcastPayload(raffleId) {
  const id = Number(raffleId);

  if (!Number.isInteger(id) || id <= 0) {
    return { ok: false, error: 'Некорректный ID розыгрыша.' };
  }
  ...
  if (String(raffle.status || '') !== 'active') {
    return {
      ok: false,
      error: 'Рассылку можно отправить только для уже активного розыгрыша.'
    };
  }

  const endAt = toUtcMoment(raffle.end_at);
  if (!endAt || !endAt.isAfter(dayjs.utc())) {
    return { ok: false, error: 'Розыгрыш уже завершён или время окончания некорректно.' };
  }

  const [participantsCount, channels] = await Promise.all([
    getRaffleParticipantsCount(id),
    getRaffleChannels(id)
  ]);

  return {
    ok: true,
    raffle,
    text: buildRaffleText(raffle, participantsCount, channels),
    keyboard: [[buildRafflePostActionButton(raffle)]],
    attachments: getRafflePhotoAttachments(raffle),
    sourceType: 'raffle',
    sourceId: id,
    sourceLabel: `Розыгрыш #${id}: ${displayValue(raffle.title, 'Без названия')}`,
    expiresAt: raffle.end_at
  };
}

async function sendAdminRaffleBroadcastPreview(target, userId, raffleId) {
  if (!isAdmin(userId)) {
    return sendMessage(target, '⛔ Нет доступа.');
  }

  const payload = await buildActiveRaffleBroadcastPayload(raffleId);

  if (!payload.ok) {
    return sendMessage(
      target,
      `⚠️ ${payload.error}`,
      [[{ text: '⬅️ К активным розыгрышам', callback_data: 'admin_active' }]]
    );
  }
  ...
  return sendMessage(
    target,
    [
      '📨 **Предпросмотр рассылки активного розыгрыша**',
      '',
      'Этот пост получат все доступные пользователи бота:',
      '────────────',
      payload.text,
      '────────────',
      '',
    ...
    [
      ...
      [{
        ...
        callback_data: `admin_raffle_broadcast_send:${payload.sourceId}`
      }],
      [{ text: '❌ Отмена', callback_data: 'admin_active' }]
    ],
    payload.attachments
  );
}

async function confirmAdminRaffleBroadcast(target, userId, raffleId) {
  if (!isAdmin(userId)) {
    return sendMessage(target, '⛔ Нет доступа.');
  }
  ...
  if (!payload.ok) {
    return sendMessage(
      target,
      `⚠️ Рассылка не начата: ${payload.error}`,
      [[{ text: '⬅️ К активным розыгрышам', callback_data: 'admin_active' }]]
    );
  }

  return createAdminBroadcastJob(target, userId, payload.text, {
    keyboard: payload.keyboard,
    attachments: payload.attachments,
    broadcastType: 'raffle',
...
function parseAdminBroadcastJson(value, fallback) {
...
function normalizeAdminBroadcastKeyboard(value) {
  const parsed = parseAdminBroadcastJson(value, []);
  ...
  return parsed
    ...
    .map(row => row
      ...
      .map(button => {
        const text = safeText(button.text, 120);
        const url = safeText(button.url, 2000);
        const callbackData = safeText(
          button.callback_data || button.payload || button.data,
          500
        );

        if (!text || (!url && !callbackData)) return null;
...
function normalizeAdminBroadcastAttachments(value) {
  const parsed = parseAdminBroadcastJson(value, []);
  if (!Array.isArray(parsed)) return [];
...
async function createAdminBroadcastJob(target, adminUserId, text, options = {}) {
  if (!isAdmin(adminUserId)) {
    return sendMessage(target, '⛔ Нет доступа.');
  }

  const cleanText = safeText(text, 12000);

  if (!cleanText) {
    return sendMessage(target, 'Текст рассылки пустой. Рассылка отменена.');
  }

  const keyboard = normalizeAdminBroadcastKeyboard(options.keyboard);
  const attachments = normalizeAdminBroadcastAttachments(options.attachments);
  const broadcastType = safeText(options.broadcastType || 'text', 50) || 'text';
  const sourceType = safeText(options.sourceType || '', 50) || null;
  const sourceIdNumber = Number(options.sourceId);
  const sourceId = Number.isInteger(sourceIdNumber) && sourceIdNumber > 0
    ? sourceIdNumber
    : null;
  const sourceLabel = safeText(options.sourceLabel || '', 300) || null;
  const expiresAtMoment = options.expiresAt ? toUtcMoment(options.expiresAt) : null;
  const expiresAt = expiresAtMoment?.isValid() ? expiresAtMoment.toDate() : null;

  if (expiresAtMoment && !expiresAtMoment.isAfter(dayjs.utc())) {
    return sendMessage(target, '⚠️ Событие уже завершилось. Рассылка не создана.');
  }
  ...
  try {
    ...
    if (sourceType && sourceId) {
      ...
      existingJob = existingRes.rows[0] || null;
    }
    ...
    if (error?.code === '23505' && sourceType && sourceId) {
      existingJob = { id: null, status: 'pending' };
    } else {
      throw error;
    }
  } finally {
  ...
  if (existingJob) {
    return sendMessage(
      target,
      [
        'ℹ️ **Такая рассылка уже выполняется**',
        '',
        sourceLabel || 'Выбранный активный пост',
        existingJob.id ? `ID рассылки: **${existingJob.id}**` : null,
        '',
        'Дождитесь завершения текущей очереди, чтобы пользователи не получили дубликат.'
      ].filter(Boolean).join('\n'),
  ...
  await sendMessage(
    ...
    [
      ...
      '',
      sourceLabel ? `Пост: ${sourceLabel}` : null,
      `ID рассылки: **${job.id}**`,
      `Получателей: **${total}**`,
      '',
...
async function processAdminBroadcastJobs() {
  ...
  const expiresAt = job.expires_at ? toUtcMoment(job.expires_at) : null;
  if (expiresAt && !expiresAt.isAfter(dayjs.utc())) {
    await pool.query(`
      UPDATE admin_broadcast_recipients
      SET
        status = 'failed',
        error_text = 'Рассылка остановлена: активное событие уже завершилось',
        updated_at = NOW()
      WHERE job_id = $1
        AND status IN ('pending', 'processing')
    `, [job.id]);
    ...
    await sendMessage(
      ...
      [
        ...
        '',
        job.source_label ? `Пост: ${job.source_label}` : null,
        'Активное событие уже завершилось, поэтому оставшимся пользователям пост не отправлен.',
        '',
        `Успешно отправлено до остановки: **${stats.sent}**`
      ].filter(Boolean).join('\n'),
      [[{ text: '👑 Админ-панель', callback_data: 'admin_panel' }]]
    ).catch(() => {});

    return { processed: 0, done: true, expired: true };
  }
  ...
  const jobKeyboard = normalizeAdminBroadcastKeyboard(job.keyboard_json);
  const jobAttachments = normalizeAdminBroadcastAttachments(job.attachments_json);
  const recipientKeyboard = jobKeyboard.length
    ? jobKeyboard
    : buildUserBroadcastKeyboard();

  for (const recipient of recipientsRes.rows) {
    const recipientId = String(recipient.user_id || '').trim();

    try {
      await sendMessage(recipientId, job.text, recipientKeyboard, jobAttachments);
...
async function sendMyRafflesMenu(target, userId) {
  ...
  const [
    ...
    pool.query(`
      SELECT COUNT(DISTINCT hr.id)::int AS count
      FROM horse_races hr
      JOIN horse_race_channels hrc
        ON hrc.horse_race_id = hr.id
      WHERE hrc.owner_user_id::text = $1
        AND hr.creator_user_id::text <> $1
    `, [String(userId || '').trim()])
  ]);

  const ownCount =
    Number(ownCountRes.rows[0]?.count || 0);

  const collabCount =
    Number(collabCountRes.rows[0]?.count || 0);

  const horseOwnCount =
    Number(horseOwnCountRes.rows[0]?.count || 0);

  const horseCollabCount =
    Number(horseCollabCountRes.rows[0]?.count || 0);

  return sendMessage(
    target,
    [
      '📁 **Мои розыгрыши и скачки**',
      '',
      'Выберите раздел:',
      '',
      `🎁 Ваши розыгрыши: **${ownCount}**`,
      `🤝 Коллаборация розыгрышей: **${collabCount}**`,
      `🐎 Мои скачки: **${horseOwnCount}**`,
      `🏇 Скачки, где участвуют мои каналы: **${horseCollabCount}**`,
      '',
      'В разделах коллаборации показаны проекты других организаторов, где подключены ваши каналы.'
    ].join('\n'),
    [
      [{
        text: '🎁 Ваши розыгрыши',
        callback_data: 'my_raffles_own'
      }],
      [{
        text: '🤝 Коллаборация розыгрышей',
        callback_data: 'my_raffles_collab'
      }],
      [{
        text: '🐎 Мои скачки',
        callback_data: 'horse_my_races'
      }],
      [{
        text: '🏇 Где участвуют мои каналы',
        callback_data: 'horse_my_collab'
      }],
      [{
      ...
      [{
        text: '🐎 Создать скачки',
        callback_data: 'horse_create_start'
      }],
      [{
        text: '🏠 Главное меню',
...
async function sendUserRaffles(target, userId) {
  ...
  text += 'Нажмите на название розыгрыша в списке, чтобы открыть управление.\n\n';
  ...
  const keyboard = [
    [{ text: '🎉 Создать розыгрыш', callback_data: 'create_raffle' }],
    [{ text: '⬅️ Назад к разделам', callback_data: 'my_raffles' }],
    [{ text: '🏠 Главное меню', callback_data: 'main_menu' }]
  ];
...
async function sendUserCollaborationRaffles(target, userId) {
  ...
  text += 'Нажмите на название розыгрыша в списке, чтобы открыть статистику, предпросмотр или продвижение.\n\n';

  for (const r of raffles) {
    const title = displayValue(r.title, 'Без названия');
    const titleForLine = markdownLink(title, buildBotDeepLink(`collabraffle_${r.id}`));
...
async function handleCallbackQuery(cb) {
  ...
  if (await horseRacesModule.handleCallback(cb)) {
    return;
  }
  ...
  if (data === 'why_us') {
    await sendWhyUsMessage(target);
    return;
  }

  if (data === 'joined_raffles') {
    await sendUserJoinedRafflesMenu(target, userId);
    return;
  }
  ...
  if (data.startsWith('raffle_step_back:')) {
    const session = await getSession(userId);
    const sessionData = typeof session?.data === 'string'
      ? JSON.parse(session.data || '{}')
      : session?.data || {};
    const step = String(data.split(':')[1] || '').trim();

    if (!session || !['await_description', 'await_prizes', 'await_photo', 'await_publish_date', 'await_end_date', 'await_channel_selection'].includes(session.state)) {
      await sendMessage(target, 'Сейчас возврат на предыдущий шаг не активен.');
      return;
    }

    if (step === 'title' && session.state === 'await_description') {
      await setSession(userId, 'await_title', sessionData);
      await sendMessage(target, buildRaffleTitlePrompt());
      return;
    }
    ...
    if (step === 'prizes' && session.state === 'await_photo') {
    ...
    if (step === 'photo' && session.state === 'await_publish_date') {
      await setSession(userId, 'await_photo', sessionData);
      await sendRafflePhotoStepPrompt(target, sessionData);
      return;
    }

    // Совместимость со старыми сообщениями, созданными до добавления шага фото.
    ...
    if (step === 'publish_date' && session.state === 'await_end_date') {
      ...
      await sendPublishDatePrompt(target, sessionData, {
        backStep: 'prizes',
        backText: '⬅️ Назад к призам'
      });
      return;
    }

    if (step === 'end_date' && session.state === 'await_channel_selection') {
    ...
    await sendMessage(target, 'Эта кнопка «Назад» уже не подходит для текущего шага.');
    return;
  }
  ...
  if (data === 'create_raffle') {
    await startRaffleCreation(target, userId);
    return;
  }
  ...
  if (data.startsWith('free_general_claim:')) {
    const raffleId = Number(data.split(':')[1]);
    ...
    await claimFreeGeneralPublication(target, userId, raffleId);
    return;
  }
  ...
  if (data === 'top1_buy') {
    await sendTop1RaffleSelection(target, userId);
    return;
  }

  if (data.startsWith('top1_select:')) {
    const raffleId = Number(data.split(':')[1]);
    ...
    await startPromotionPaymentEmailFlow(target, userId, raffleId, PROMO_TOP1_PRODUCT_CODE);
    return;
  }

  if (data.startsWith('paid_general_now:')) {
    const paymentId = data.replace(/^paid_general_now:/, '').trim();
    await handlePaidGeneralPublishNow(target, userId, paymentId);
    return;
  }

  if (data.startsWith('paid_general_input:')) {
    const paymentId = data.replace(/^paid_general_input:/, '').trim();
    await startPaidGeneralPublishTimeInput(target, userId, paymentId);
    return;
  }

  if (data.startsWith('republish_raffle:')) {
    const raffleId = Number(data.split(':')[1]);
    ...
    try {
      await sendRaffleRepublishChannelSelection(
        target,
        userId,
        raffleId,
        { reset: true }
      ...
      await sendMessage(target, `⚠️ ${safeText(error.message, 500)}`);
    }
    return;
  }

  if (data.startsWith('republish_ch_toggle:')) {
    const [, raffleIdRaw, channelIdRaw] = data.split(':');
    const raffleId = Number(raffleIdRaw);
    const channelId = unsafeCallbackPart(channelIdRaw);

    if (!Number.isInteger(raffleId) || !/^-?\d+$/.test(channelId)) {
      await sendMessage(target, 'Некорректные данные канала.');
      return;
    }

    try {
      await toggleRaffleRepublishChannel(
        userId,
        raffleId,
        channelId
      );
  ...
  if (data.startsWith('republish_page:')) {
    const [, raffleIdRaw, pageRaw] = data.split(':');
    const raffleId = Number(raffleIdRaw);
    const page = Number(pageRaw);

    if (!Number.isInteger(raffleId) || !Number.isInteger(page)) {
      await sendMessage(target, 'Некорректная страница каналов.');
      return;
    }
  ...
  if (data.startsWith('republish_review:')) {
    const raffleId = Number(data.split(':')[1]);
    ...
    try {
      await sendRaffleRepublishConfirmation(
        target,
        userId,
        raffleId
      );
  ...
  if (data.startsWith('republish_back:')) {
    const raffleId = Number(data.split(':')[1]);
  ...
  if (data.startsWith('republish_confirm:')) {
    const raffleId = Number(data.split(':')[1]);
    ...
    try {
      await enqueueRaffleRepublishFromSession(
        target,
        userId,
        raffleId
      );
  ...
  if (data.startsWith('republish_cancel:')) {
    const raffleId = Number(data.split(':')[1]);
    await clearSession(userId);

    const raffle = Number.isInteger(raffleId)
  ...
  if (data.startsWith('show_raffle_results:')) {
    const raffleId = Number(data.split(':')[1]);
    ...
    await sendRaffleResultsToUser(target, userId, raffleId);
    return;
  }

  if (data === 'raffle_photo_skip' || data === 'raffle_photo_continue') {
    const session = await getSession(userId);

    if (!session || session.state !== 'await_photo') {
      await sendMessage(target, 'Сейчас шаг добавления фото не активен.');
      return;
    }
  ...
  if (data.startsWith('quick_end:')) {
    ...
    enableCreateChannelBack(sessionData);
  ...
  if (data === 'raffle_channels_done') {
    ...
    clearCreateFlowBackFlags(sessionData);
    await sendRaffleDraftPreview(target, userId, sessionData);
    return;
  }
  ...
  if (data === 'draft_edit_photo') {
    ...
    await sendMessage(
      target,
      [
        'Отправьте фото, которое нужно прикрепить к посту розыгрыша.',
        '',
        `👆 Нету своего крутого фото❓ **${markdownLink('Наш ИИ', 'https://max.ru/id236700415542_bot')}** сделает его бесплатно за секунды.`
      ].join('\n'),
      [[{ text: '🤖 Сделать фото бесплатно', url: 'https://max.ru/id236700415542_bot' }]]
    );
    return;
  }
  ...
  if (data === 'admin_active' || data.startsWith('admin_active:')) {
    const page = data.startsWith('admin_active:')
      ? data.split(':')[1]
      : 0;

    await sendAdminActiveRaffles(target, userId, page);
    return;
  }
  ...
  if (data.startsWith('admin_raffle_broadcast_preview:')) {
    const raffleId = Number(data.split(':')[1]);
    await sendAdminRaffleBroadcastPreview(target, userId, raffleId);
    return;
  }

  if (data.startsWith('admin_raffle_broadcast_send:')) {
    const raffleId = Number(data.split(':')[1]);
    await confirmAdminRaffleBroadcast(target, userId, raffleId);
    return;
  }
...
async function handleStartPayload(target, from, payload) {
  ...
  if (normalized === 'top1_buy') {
    await sendTop1RaffleSelection(target, from.id);
    return true;
  }

  if (await horseRacesModule.handleStartPayload({ target, from, payload: normalized })) {
    return true;
  }
  ...
  if (normalized.startsWith('results_')) {
    const raffleId = Number(normalized.replace(/^results_/, '').trim());

    if (Number.isInteger(raffleId)) {
      await sendRaffleResultsToUser(target, from.id, raffleId);
      return true;
    }
  }
  ...
  if (normalized.startsWith('collabraffle_')) {
    const raffleId = Number(normalized.replace(/^collabraffle_/, '').trim());

    if (Number.isInteger(raffleId)) {
      await sendRaffleDetails(target, from.id, raffleId);
      return true;
    }
  }
...
async function handleMaxUpdate(update) {
  try {
    ...
    // Передаём реальные события входа/выхода из канала в модуль скачек.
    // Внутри модуля состояние хранится по ключу «скачки + канал + пользователь»,
    // поэтому повторный webhook или повторная проверка не создадут двойной +1/-1.
    const horseMemberDelta = detectChannelMemberDelta(update);
    if (horseMemberDelta && isChannelTarget(target)) {
      const horseMemberUserId = getActorUserIdFromUpdate(update);

      if (
        horseMemberUserId &&
        (!BOT_USER_ID || String(horseMemberUserId) !== String(BOT_USER_ID))
      ) {
        await horseRacesModule.recordHorseRaceMemberDeltaFromWebhook({
          channelId: target.id,
          userId: horseMemberUserId,
          delta: horseMemberDelta,
          source: safeText(update?.update_type || 'channel_member_webhook', 100)
        });
      }
    }
    ...
    if (playModule.isPlayCommand(text)) {
      await playModule.sendPlayGameMessage(chatTarget);
      return;
    }
    ...
    const channelLinkCommand = parseChannelLinkCommand(text);
    if (channelLinkCommand) {
      await updateRaffleChannelPublicLink(
        chatTarget,
        from.id,
        channelLinkCommand.channelId,
        channelLinkCommand.url,
        channelLinkCommand.raffleId,
        channelLinkCommand.entityType
      );
      return;
    }
    ...
    if (text === '/create' || text.toLowerCase() === 'создать розыгрыш') {
      await startRaffleCreation(chatTarget, from.id);
      return;
    }
...
app.get('/', (req, res) => {
  res.status(200).json({
    ...
    miniAppWebUrl: horseRacesModule.getWebUrl(),
    miniAppLaunchUrl: horseRacesModule.getLaunchUrl('home')
  });
});
...
// =========================
const RAFFLE_DRAFT_SESSION_STATES = [
  'await_title',
  'await_description',
  'await_prizes',
  'await_photo',
...
async function detachPaymentsFromRaffles(db, raffleIds = []) {
  const ids = raffleIds.map(Number).filter(Number.isInteger);
  if (!ids.length) return 0;

  const res = await db.query(`
    UPDATE raffle_promo_payments
    SET raffle_id = NULL, updated_at = NOW()
...
async function runRaffleDataCleanup() {
  const client = await pool.connect();
  ...
  try {
    ...
    `, [RAFFLE_DRAFT_SESSION_STATES, DRAFT_SESSION_RETENTION_HOURS]);

    // Если прежняя задача завершения потерялась или стала failed, возвращаем
    // просроченный розыгрыш в очередь. Иначе он мог бы навсегда остаться active.
    const recoveredFinishQueueRes = await client.query(`
      INSERT INTO raffle_queue (raffle_id, queue_type, scheduled_at, payload, status)
      SELECT r.id, 'raffle_finish', NOW(), '{}'::jsonb, 'pending'
      FROM raffles r
      WHERE r.status IN ('scheduled', 'active')
        AND r.end_at <= NOW()
        AND NOT EXISTS (
          SELECT 1
          FROM raffle_queue q
          WHERE q.raffle_id = r.id
            AND q.queue_type = 'raffle_finish'
    ...
    const pruneRes = await client.query(`
      WITH candidates AS (
        ...
        WHERE status IN ('finished', 'archived')
          ...
          AND full_data_pruned_at IS NULL
          AND finished_at < NOW() - ($1::text || ' days')::interval
        FOR UPDATE
      )
      UPDATE raffles r
      SET
        archived_participants_count = COALESCE(
          (SELECT COUNT(DISTINCT rue.user_id)::int FROM raffle_user_entry rue WHERE rue.raffle_id = r.id),
          0
        ),
        full_data_pruned_at = NOW(),
        updated_at = NOW()
      FROM candidates c
      WHERE r.id = c.id
      RETURNING r.id
    `, [FINISHED_RAFFLE_FULL_DATA_DAYS]);

    const prunedIds = pruneRes.rows.map(row => Number(row.id)).filter(Number.isInteger);

    if (prunedIds.length) {
      await client.query(`DELETE FROM raffle_pending_joins WHERE raffle_id = ANY($1::int[])`, [prunedIds]);
      await client.query(`DELETE FROM raffle_participants WHERE raffle_id = ANY($1::int[])`, [prunedIds]);
      await client.query(`DELETE FROM raffle_user_entry WHERE raffle_id = ANY($1::int[])`, [prunedIds]);
      await client.query(`DELETE FROM raffle_channel_subscription_events WHERE raffle_id = ANY($1::int[])`, [prunedIds]);
      await client.query(`DELETE FROM raffle_channel_subscription_states WHERE raffle_id = ANY($1::int[])`, [prunedIds]);
      await client.query(`DELETE FROM raffle_permission_alerts WHERE raffle_id = ANY($1::int[])`, [prunedIds]);
    }

    const archiveIdsRes = await client.query(`
      SELECT id
      FROM raffles
      WHERE status IN ('finished', 'archived')
        AND finished_at IS NOT NULL
        AND finished_at < NOW() - ($1::text || ' days')::interval
      FOR UPDATE
    `, [FINISHED_RAFFLE_ARCHIVE_DAYS]);
    const archiveIds = archiveIdsRes.rows.map(row => Number(row.id)).filter(Number.isInteger);

    let deletedArchivedRaffles = 0;
    let detachedArchivePayments = 0;
    if (archiveIds.length) {
      detachedArchivePayments = await detachPaymentsFromRaffles(client, archiveIds);
      const deleteRes = await client.query(`DELETE FROM raffles WHERE id = ANY($1::int[])`, [archiveIds]);
      deletedArchivedRaffles = deleteRes.rowCount || 0;
    }

    const cancelledIdsRes = await client.query(`
      SELECT id
      FROM raffles
      WHERE (
          status = 'draft'
          ...
          AND COALESCE(cancelled_at, updated_at, created_at) < NOW() - ($1::text || ' days')::interval
        )
      FOR UPDATE
    `, [CANCELLED_RAFFLE_RETENTION_DAYS]);
    const cancelledIds = cancelledIdsRes.rows.map(row => Number(row.id)).filter(Number.isInteger);

    let deletedCancelledRaffles = 0;
    let detachedCancelledPayments = 0;
    if (cancelledIds.length) {
      detachedCancelledPayments = await detachPaymentsFromRaffles(client, cancelledIds);
      const deleteRes = await client.query(`DELETE FROM raffles WHERE id = ANY($1::int[])`, [cancelledIds]);
      deletedCancelledRaffles = deleteRes.rowCount || 0;
    }

    const doneQueueRes = await client.query(`
      DELETE FROM raffle_queue
    ...
    const result = {
      skipped: false,
      deletedDraftSessions: sessionsRes.rowCount || 0,
      recoveredOverdueFinishJobs: recoveredFinishQueueRes.rowCount || 0,
      prunedFinishedRaffles: prunedIds.length,
      deletedArchivedRaffles,
      deletedCancelledRaffles,
      deletedDoneQueue: doneQueueRes.rowCount || 0,
      deletedFailedQueue: failedQueueRes.rowCount || 0,
      detachedPayments: detachedArchivePayments + detachedCancelledPayments
    };

    console.log('🧹 Очистка данных розыгрышей завершена:', result);
    return result;
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    throw error;
...
async function createTablesIfNotExist() {
  await pool.query(`
    ...
    ALTER TABLE users
      ADD COLUMN IF NOT EXISTS admin_community_invited_at TIMESTAMP;
    ...
    CREATE TABLE IF NOT EXISTS admin_broadcast_jobs (
      ...
      text TEXT NOT NULL,
      keyboard_json JSONB NOT NULL DEFAULT '[]'::jsonb,
    ...
    ALTER TABLE admin_broadcast_jobs
      ADD COLUMN IF NOT EXISTS keyboard_json JSONB NOT NULL DEFAULT '[]'::jsonb;

    ALTER TABLE admin_broadcast_jobs
      ADD COLUMN IF NOT EXISTS attachments_json JSONB NOT NULL DEFAULT '[]'::jsonb;

    ALTER TABLE admin_broadcast_jobs
      ADD COLUMN IF NOT EXISTS broadcast_type TEXT NOT NULL DEFAULT 'text';
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS ux_admin_broadcast_active_source
      ON admin_broadcast_jobs (source_type, source_id)
    ...
    ALTER TABLE raffles
      ADD COLUMN IF NOT EXISTS full_data_pruned_at TIMESTAMP;

    ALTER TABLE raffles
      ADD COLUMN IF NOT EXISTS archived_participants_count INT;

    ALTER TABLE raffles
      ADD COLUMN IF NOT EXISTS lifetime_counted_at TIMESTAMP;

    CREATE TABLE IF NOT EXISTS bot_lifetime_stats (
      id SMALLINT PRIMARY KEY,
      raffles_created BIGINT NOT NULL DEFAULT 0,
      tickets_created BIGINT NOT NULL DEFAULT 0,
      initialized_at TIMESTAMP NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
      CONSTRAINT bot_lifetime_stats_singleton CHECK (id = 1)
    );

    UPDATE raffles
    SET finished_at = COALESCE(finished_at, end_at, updated_at, created_at)
    WHERE status IN ('finished', 'archived')
    ...
    -- Один и тот же канал в одном обычном розыгрыше должен быть только один раз.
    -- Если соадмин ошибся и потом выбрал заново с размещением/без размещения,
    -- оставляем последнюю запись, а дальше addRaffleChannel будет просто обновлять настройки.
    DELETE FROM raffle_channels older
    USING raffle_channels newer
    WHERE older.raffle_id = newer.raffle_id
      AND older.channel_id = newer.channel_id
      AND older.id < newer.id;

    CREATE UNIQUE INDEX IF NOT EXISTS ux_raffle_channels_raffle_channel
    ...
    -- Один платёж не должен иметь несколько активных запросов на запуск General.
    -- Старые дубли мягко отменяем перед созданием уникального индекса.
    WITH ranked_general_requests AS (
      SELECT
        id,
        ROW_NUMBER() OVER (
    ...
    UPDATE raffle_queue rq
    SET status = 'cancelled',
        updated_at = NOW()
    FROM ranked_general_requests r
    WHERE rq.id = r.id
      AND r.rn > 1;

    CREATE UNIQUE INDEX IF NOT EXISTS ux_raffle_queue_general_request_payment
      ON raffle_queue ((payload->>'payment_id'))
      WHERE queue_type = 'general_publish_request'
        AND status IN ('pending', 'processing')
        AND COALESCE(payload->>'payment_id', '') <> '';

    -- Зависшая после аварийного перезапуска задача не должна блокировать
    -- повторную публикацию навсегда.
    UPDATE raffle_queue
    SET
      status = 'failed',
      updated_at = NOW()
    WHERE queue_type = 'raffle_republish'
      AND status = 'processing'
      AND updated_at < NOW() - INTERVAL '15 minutes';

    -- Одновременно может выполняться только одна повторная публикация
    -- одного и того же розыгрыша. Это защищает от двойного нажатия.
    WITH ranked_republish_jobs AS (
      SELECT
        id,
        ROW_NUMBER() OVER (
          PARTITION BY raffle_id
          ORDER BY id ASC
        ) AS rn
      FROM raffle_queue
      WHERE queue_type = 'raffle_republish'
    ...
    SET
      status = 'cancelled',
      updated_at = NOW()
    FROM ranked_republish_jobs ranked
    WHERE rq.id = ranked.id
      AND ranked.rn > 1;

    CREATE UNIQUE INDEX IF NOT EXISTS ux_raffle_queue_active_republish
      ON raffle_queue (raffle_id)
      WHERE queue_type = 'raffle_republish'
        AND status IN ('pending', 'processing');
    ...
    CREATE TABLE IF NOT EXISTS free_general_publications (
      id SERIAL PRIMARY KEY,
      user_id BIGINT NOT NULL REFERENCES users(max_user_id) ON DELETE CASCADE,
      raffle_id INT NOT NULL REFERENCES raffles(id) ON DELETE CASCADE,
      qualifying_channel_id BIGINT NOT NULL,
      subscriber_count INT NOT NULL,
      claim_day DATE NOT NULL,
      cooldown_until TIMESTAMPTZ,
      status TEXT NOT NULL DEFAULT 'claimed',
      queue_id INT REFERENCES raffle_queue(id) ON DELETE SET NULL,
    ...
    ALTER TABLE free_general_publications
      ADD COLUMN IF NOT EXISTS queue_id INT REFERENCES raffle_queue(id) ON DELETE SET NULL;
    ...
    SET cooldown_until = COALESCE(
      cooldown_until,
      created_at + (${FREE_GENERAL_COOLDOWN_DAYS}::text || ' days')::interval
    )
    WHERE cooldown_until IS NULL;

    CREATE TABLE IF NOT EXISTS free_general_cooldowns (
    ...
    INSERT INTO free_general_cooldowns (scope_type, scope_id, next_available_at, last_claim_id)
    SELECT
      'user',
      user_id::text,
      MAX(COALESCE(cooldown_until, created_at + (${FREE_GENERAL_COOLDOWN_DAYS}::text || ' days')::interval)),
      NULL
    FROM free_general_publications
    ...
    DO UPDATE SET
      next_available_at = GREATEST(
        free_general_cooldowns.next_available_at,
    ...
    SELECT
      'channel',
      qualifying_channel_id::text,
      MAX(COALESCE(cooldown_until, created_at + (${FREE_GENERAL_COOLDOWN_DAYS}::text || ' days')::interval)),
      NULL
    FROM free_general_publications
    GROUP BY qualifying_channel_id::text
    ON CONFLICT (scope_type, scope_id)
    DO UPDATE SET
      next_available_at = GREATEST(
        free_general_cooldowns.next_available_at,
    ...
    CREATE INDEX IF NOT EXISTS ix_free_general_cooldowns_next
      ON free_general_cooldowns (next_available_at);

    CREATE INDEX IF NOT EXISTS ix_free_general_publications_user_day
      ON free_general_publications (user_id, claim_day);

    CREATE INDEX IF NOT EXISTS ix_free_general_publications_status
      ON free_general_publications (status, created_at);

    CREATE INDEX IF NOT EXISTS ix_free_general_publications_user_cooldown
      ON free_general_publications (user_id, cooldown_until);

    CREATE INDEX IF NOT EXISTS ix_free_general_publications_channel_cooldown
      ON free_general_publications (qualifying_channel_id, cooldown_until);

    CREATE TABLE IF NOT EXISTS raffle_top_boosts (
      id SERIAL PRIMARY KEY,
      raffle_id INT NOT NULL REFERENCES raffles(id) ON DELETE CASCADE,
      user_id BIGINT REFERENCES users(max_user_id) ON DELETE SET NULL,
      payment_id TEXT UNIQUE NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS ix_raffle_top_boosts_active
    ON raffle_top_boosts (created_at DESC, raffle_id);

    CREATE INDEX IF NOT EXISTS ix_raffle_top_boosts_raffle
    ON raffle_top_boosts (raffle_id, created_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS ix_raffles_creator_created_status
    ON raffles (creator_user_id, created_at, status);

    CREATE INDEX IF NOT EXISTS ix_raffles_finished_cleanup
    ON raffles (status, finished_at, full_data_pruned_at);

    CREATE INDEX IF NOT EXISTS ix_raffles_cancelled_cleanup
    ON raffles (status, cancelled_at, created_at);
    ...
    CREATE INDEX IF NOT EXISTS ix_raffle_queue_general_request_payment_id
    ON raffle_queue ((payload->>'payment_id'))
    WHERE queue_type = 'general_publish_request';
  ...
  // При первом запуске один раз переносим текущие значения в пожизненные
  // счётчики. При следующих запусках добираем только розыгрыши, которые могли
  // успешно создаться, но не успели отметиться из-за аварийного завершения процесса.
  const lifetimeInitRes = await pool.query(`
    INSERT INTO bot_lifetime_stats (
      id,
      raffles_created,
      tickets_created,
      initialized_at,
    ...
    SELECT
      ...
      (SELECT COUNT(*)::bigint FROM raffle_participants),
      NOW(),
      NOW()
  ...
  if (lifetimeInitRes.rowCount) {
    await pool.query(`
      UPDATE raffles
      SET lifetime_counted_at = COALESCE(lifetime_counted_at, created_at, NOW())
      WHERE status IN ('scheduled', 'active', 'finished', 'archived', 'cancelled')
        AND lifetime_counted_at IS NULL
    `);
  } else {
    await pool.query(`
      WITH newly_counted AS (
        UPDATE raffles
        SET lifetime_counted_at = COALESCE(created_at, NOW())
        WHERE status IN ('scheduled', 'active', 'finished', 'archived', 'cancelled')
          AND lifetime_counted_at IS NULL
        RETURNING id
      )
      UPDATE bot_lifetime_stats
      SET raffles_created = raffles_created + (SELECT COUNT(*)::bigint FROM newly_counted),
          updated_at = NOW()
      WHERE id = 1
    `);
  }
...
async function startServer() {
  try {
    ...
    setInterval(() => {
      runRaffleDataCleanup().catch(error => {
        console.error('raffle cleanup interval error:', error.message);
      });
    }, RAFFLE_CLEANUP_INTERVAL_HOURS * 60 * 60 * 1000).unref?.();

    // Намеренно не запускаем разрушительную очистку сразу после старта.
    // Первый автоматический запуск произойдёт только через заданный интервал.
