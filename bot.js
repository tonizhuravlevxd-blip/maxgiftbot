...
async function createRaffleFromSession(userId, data) {
  ...
  const permissionCheck = await checkCandidateChannelsBotRights(data.channels || [], {
    ownerUserId: userId,
    markUserChannels: true
  }).catch(error => {
    console.warn('Не удалось проверить права выбранных каналов перед созданием розыгрыша:', error.message);
    return { ok: data.channels || [], problems: [] };
  });

  if (permissionCheck.problems?.length) {
    console.warn('Некоторые каналы убраны перед созданием розыгрыша из-за отсутствия прав бота:', {
      user_id: String(userId),
      channels: permissionCheck.problems.map(ch => String(ch.channel_id))
    });
  }

  data.channels = permissionCheck.ok || [];
...
function formatRaffleChannelPermissionProblems(problems = []) {
  const lines = (problems || [])
    .map((ch, index) => {
      const title = displayValue(ch.channel_title, ch.channel_id ? `Канал ${ch.channel_id}` : 'канал');
...
async function markUserChannelPermissionLost(ownerUserId, channelId) {
...
async function checkCandidateChannelsBotRights(channels = [], options = {}) {
  const problems = [];
  const ok = [];
  const seen = new Set();
  ...
  for (const ch of channels || []) {
    ...
    const canUse = await checkBotCanPublishInChannel(channelId).catch(error => {
      console.warn('Не удалось проверить права бота перед добавлением канала в розыгрыш:', {
        channel_id: channelId,
        error: error.message
      });
      return false;
    ...
    if (!canUse) {
      problems.push({
        ...ch,
        permission_error: 'bot_has_no_admin_rights'
      });

      if (options.markUserChannels) {
        const ownerUserId = ch.owner_user_id || options.ownerUserId || null;
        await markUserChannelPermissionLost(ownerUserId, channelId).catch(error => {
          console.warn('Не удалось пометить канал пользователя как недоступный:', {
...
async function removeRaffleChannelBecausePermissionLost(raffle, channel, reason = 'permissions_lost') {
  const raffleId = Number(raffle?.id || channel?.raffle_id || 0);
  const channelId = String(channel?.channel_id || '').trim();

  if (!raffleId || !channelId) return false;

  await notifyRafflePermissionProblem({
    raffle_id: raffleId,
    public_number: getRafflePublicNumber(raffle || { id: raffleId }),
    raffle_title: raffle?.title || channel?.raffle_title || '',
    creator_user_id: raffle?.creator_user_id || channel?.creator_user_id || '',
    channel_id: channelId,
    channel_title: channel?.channel_title,
    channel_link: channel?.channel_link,
    owner_user_id: channel?.owner_user_id
  }, reason).catch(error => {
    console.warn('Не удалось отправить предупреждение перед удалением проблемного канала:', {
      raffle_id: raffleId,
      channel_id: channelId,
      error: error.message
    });
  ...
  await pool.query(`
    ...
    WHERE raffle_id = $1
      AND channel_id::text = $2
  `, [raffleId, channelId]).catch(() => {});

  await markUserChannelPermissionLost(channel?.owner_user_id, channelId).catch(() => {});
  await resolveRafflePermissionAlerts(raffleId, channelId).catch(() => {});

  if (String(raffle?.status || '').toLowerCase() === 'active') {
    await updateRafflePublishedPosts(raffleId, { force: true }).catch(error => {
      console.warn('Не удалось обновить посты после удаления проблемного канала:', {
        raffle_id: raffleId,
        channel_id: channelId,
        error: error.message
      });
  ...
  console.warn('🚫 Канал удалён из розыгрыша из-за отсутствия прав бота:', {
    raffle_id: raffleId,
    channel_id: channelId,
    reason
  });
...
async function validateExistingRaffleChannelsBeforeUse(raffle, options = {}) {
  ...
  for (const ch of channels) {
    ...
    // Если канал участвует только как необязательный и без публикации, он не влияет на механику розыгрыша.
    if (!ch.is_required && !ch.publish_post) {
      usable.push(ch);
      continue;
    }

    const canUse = await checkBotCanPublishInChannel(channelId).catch(error => {
      console.warn('Не удалось проверить права бота в уже добавленном канале розыгрыша:', {
        raffle_id: raffle.id,
        channel_id: channelId,
        error: error.message
      });
    ...
    if (!canUse) {
      await removeRaffleChannelBecausePermissionLost(raffle, ch, options.reason || 'permissions_lost');
      removed.push(ch);
      continue;
    }
...
async function addCollabChannelsFromSession(target, userId, data) {
  ...
  const permissionCheck = await checkCandidateChannelsBotRights(channels, {
    ownerUserId: userId,
    markUserChannels: true
  });
  ...
  if (permissionCheck.problems.length) {
    data.channels = permissionCheck.ok;
    await setSession(userId, 'collab_channel_selection', data);

    const warning = [
      '⚠️ У бота нет прав администратора в выбранном канале, поэтому канал не добавлен к розыгрышу.',
      '',
      formatRaffleChannelPermissionProblems(permissionCheck.problems),
      '',
      `Проверьте, что ${BOT_USERNAME} добавлен в администраторы и имеет права на публикацию/проверку участников. Затем нажмите **➕ Добавить канал** и вернитесь назад к выбору.`
    ].join('\n');

    return sendChannelSelectionMenu(target, userId, data, 'collab', {
      warningText: warning,
      sessionState: 'collab_channel_selection'
    });
  }

  for (const ch of permissionCheck.ok) {
    await addRaffleChannel(
      raffle.id,
      ch.channel_id,
      ch.channel_title,
    ...
    if (raffle.status === 'active' && ch.publish_post) {
      await publishRaffleToChannel(raffle, ch.channel_id).catch(async error => {
        console.warn(`Не удалось сразу опубликовать в канале ${ch.channel_id}:`, error.message);

        if (isMaxChannelPermissionError(error)) {
          await removeRaffleChannelBecausePermissionLost(raffle, ch, 'publish_403').catch(removeError => {
            console.warn('Не удалось убрать канал после ошибки публикации:', removeError.message);
          });
        }
      });
    }
...
async function checkUserAllSubscriptions(raffleId, userId, options = {}) {
  ...
  for (const ch of channels) {
    ...
    if (check.permissionProblem) {
      const problemChannel = {
        ...ch,
        permission_error: check.error || ''
      };

      permissionProblems.push(problemChannel);

      const raffle = await getRaffleById(raffleId).catch(() => null);
      if (raffle) {
        await removeRaffleChannelBecausePermissionLost(raffle, problemChannel, 'subscription_check_403').catch(error => {
          console.warn('Не удалось убрать канал после ошибки проверки подписки:', {
            raffle_id: raffleId,
      ...
      // Не блокируем участника каналом, который бот больше не может проверить.
      continue;
    }
...
async function publishRaffleToChannel(raffle, channelId) {
  const canPublish = await checkBotCanPublishInChannel(channelId).catch(error => {
    console.warn('Не удалось проверить права перед публикацией розыгрыша:', {
      raffle_id: raffle.id,
      channel_id: String(channelId),
      error: error.message
    });
  ...
  if (!canPublish) {
    const channel = (await getRaffleChannels(raffle.id)).find(ch => String(ch.channel_id) === String(channelId)) || {
      raffle_id: raffle.id,
      channel_id: channelId
    };

    await removeRaffleChannelBecausePermissionLost(raffle, channel, 'publish_precheck_failed');
    throw new Error(`У бота нет прав администратора в канале ${channelId}`);
  }
  ...
  try {
    ...
    if (isMaxChannelPermissionError(error)) {
      const channel = channels.find(ch => String(ch.channel_id) === String(channelId)) || {
        raffle_id: raffle.id,
        channel_id: channelId
      };

      await removeRaffleChannelBecausePermissionLost(raffle, channel, 'publish_403').catch(removeError => {
        console.warn('Не удалось убрать канал после MAX 403 при публикации:', removeError.message);
      });
    }
...
async function activateRaffle(raffleId) {
  ...
  const validation = await validateExistingRaffleChannelsBeforeUse(raffle, {
    reason: 'raffle_start_precheck'
  });
  const channels = validation.usable;
