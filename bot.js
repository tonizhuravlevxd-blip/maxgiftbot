...
<head>
  ...
  <title>РОЗЫГРЫШ БОТ — Египетская игра</title>
  <style>
    :root { color-scheme: dark; }
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    html, body { margin: 0; min-height: 100%; background: #100b1f; font-family: Arial, Helvetica, sans-serif; color: #fff; }
    body { display: flex; align-items: center; justify-content: center; padding: 10px; }
    .wrap { width: min(1120px, 100%); }
    .card { background: linear-gradient(180deg, #26163b, #120d20); border: 1px solid rgba(255,255,255,.14); border-radius: 24px; padding: 14px; box-shadow: 0 24px 80px rgba(0,0,0,.42); }
    .top { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 10px; }
    .brand { font-weight: 900; letter-spacing: .08em; font-size: clamp(18px, 4vw, 30px); }
    .score { font-weight: 900; font-size: clamp(16px, 4vw, 25px); }
    canvas { width: 100%; height: auto; display: block; border-radius: 20px; background: #f6c76b; touch-action: manipulation; box-shadow: inset 0 0 0 2px rgba(255,255,255,.10); }
    .hint { margin: 10px 2px 0; opacity: .88; line-height: 1.35; font-size: 14px; text-align: center; }
    .button { display: block; width: 100%; margin-top: 10px; border: 0; border-radius: 16px; padding: 14px 16px; font-size: 17px; font-weight: 900; color: #1d1300; background: linear-gradient(180deg,#ffe37a,#f5b833); cursor: pointer; }
...
<body>
  <main class="wrap">
    <section class="card">
      ...
      <canvas id="game" width="1040" height="520" aria-label="Игра: собака бежит по Египту и перепрыгивает пирамиды"></canvas>
      <button class="button" id="jumpBtn">Прыгнуть / Начать заново</button>
      <p class="hint">Нажмите кнопку, экран или пробел. Фон — Египет с пирамидами, скорость плавно растёт. Перепрыгивайте маленькие пирамиды.</p>
    </section>
  </main>
...
(function () {
  ...
  const groundY = 420;
  const dog = { x: 122, y: groundY - 94, w: 126, h: 94, vy: 0, onGround: true, frame: 0 };
  const gravity = 0.82;
  const jumpPower = -16.3;
  let obstacles = [];
  let speed = 6;
  let nextObstacle = 0;
  let score = 0;
  let best = Number(localStorage.getItem('raffleBotEgyptBest') || 0) || 0;
  let gameOver = false;
  ...
  function addObstacle() {
    ...
    const doubleChance = Math.random() > 0.72;
    obstacles.push({ x: W + 30, y: groundY - size, w: size * 1.05, h: size, type: doubleChance ? 'double' : 'pyramid' });
    nextObstacle = 70 + Math.random() * 72;
  }
  ...
  function collides(a, b) {
    ...
    const padTop = 18;
    const padBottom = 10;
    return a.x + padX < b.x + b.w && a.x + a.w - padX > b.x && a.y + padTop < b.y + b.h && a.y + a.h - padBottom > b.y;
  }
  ...
  function drawPyramid(x, baseY, width, height, main, shadow, stroke) {
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(x, baseY);
    ...
    ctx.strokeStyle = stroke || 'rgba(85,48,14,.28)';
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.restore();
  }
  ...
  function drawPalm(x, y, s) {
    ...
    ctx.translate(x, y);
    ctx.scale(s, s);
    ctx.strokeStyle = '#7a431e';
    ctx.lineWidth = 8;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(0, 58); ctx.quadraticCurveTo(8, 24, 1, 0); ctx.stroke();
    ...
    for (let i = 0; i < 7; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(2, 0);
      ctx.quadraticCurveTo(Math.cos(a) * 38, Math.sin(a) * 20 - 9, Math.cos(a) * 70, Math.sin(a) * 35 - 12);
      ctx.stroke();
    }
    ctx.restore();
  }
  ...
  function drawBackground() {
    ...
    sky.addColorStop(0, '#422064');
    sky.addColorStop(0.24, '#e48a4a');
    sky.addColorStop(0.54, '#ffd27a');
    sky.addColorStop(1, '#f6bd57');
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, W, H);

    ctx.fillStyle = 'rgba(255, 224, 139, .95)';
    ctx.beginPath();
    ctx.arc(850, 92, 52, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 292);
    for (let x = 0; x <= W + 80; x += 80) ctx.quadraticCurveTo(x + 40, 252 + Math.sin((x + bgOffset) * .006) * 14, x + 80, 292);
    ctx.lineTo(W, H); ctx.lineTo(0, H); ctx.closePath(); ctx.fill();

    // Far small pyramids — they move slowly, like they are far away
    for (let i = -1; i < 7; i++) {
      const x = ((i * 230 - farOffset * 0.24) % (W + 260)) - 90;
      drawPyramid(x, 318, 150, 112, '#d6a052', '#bc7d34', 'rgba(75,42,12,.18)');
    }

    // Big middle pyramids
    drawPyramid(450 - nearOffset * 0.055 % 1180, 380, 330, 250, '#e4b260', '#c58a3e', 'rgba(71,40,16,.24)');
    drawPyramid(750 - nearOffset * 0.05 % 1180, 392, 260, 190, '#efc374', '#c88a3c', 'rgba(71,40,16,.22)');
    drawPyramid(30 - nearOffset * 0.04 % 1180, 390, 250, 182, '#ecc070', '#be833b', 'rgba(71,40,16,.20)');

    // Sand road
    const road = ctx.createLinearGradient(0, groundY - 35, 0, H);
    road.addColorStop(0, '#e7ae58');
    road.addColorStop(1, '#b86f2f');
    ctx.fillStyle = road;
    ctx.fillRect(0, groundY - 34, W, H - groundY + 34);

    // Moving dunes/lines
    ctx.strokeStyle = 'rgba(105,61,20,.22)';
    ctx.lineWidth = 3;
    for (let y = groundY - 12; y < H; y += 26) {
      ctx.beginPath();
      for (let x = -80; x <= W + 100; x += 60) {
        const wave = Math.sin((x + bgOffset * 1.2 + y * 2) * .018) * 5;
    ...
    // A few palms in the distance
    drawPalm(140 - (nearOffset * .09 % 1250), 346, .68);
    drawPalm(980 - (nearOffset * .075 % 1250), 350, .58);
  }
  ...
  function drawDog() {
    ...
    // bigger cartoon dog inspired by Scooby, not an exact copy
    ctx.fillStyle = '#8b4a20';
    roundRect(14, 36, 78, 38, 20); ctx.fill();

    ctx.fillStyle = '#bf7435';
    roundRect(30, 44, 34, 20, 10); ctx.fill();

    ctx.fillStyle = '#8b4a20';
    roundRect(82, 14, 43, 42, 19); ctx.fill();
    ctx.fillStyle = '#b86f35';
    roundRect(104, 34, 44, 22, 12); ctx.fill();

    ctx.fillStyle = '#5a2d14';
    roundRect(82, 8, 17, 44, 10); ctx.fill();
    roundRect(67, 13, 16, 45, 10); ctx.fill();

    ctx.fillStyle = '#25b7dc';
    ctx.fillRect(82, 56, 30, 8);
    ctx.fillStyle = '#ffdc4f';
    ctx.beginPath(); ctx.arc(98, 70, 7, 0, Math.PI * 2); ctx.fill();

    ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(111, 27, 7, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#151515';
    ctx.beginPath(); ctx.arc(114, 28, 3, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(146, 42, 6, 0, Math.PI * 2); ctx.fill();

    ctx.fillStyle = '#3a2011';
    ctx.beginPath(); ctx.ellipse(42, 44, 12, 7, .15, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(70, 63, 9, 6, -.25, 0, Math.PI * 2); ctx.fill();

    const lift = dog.onGround ? Math.sin(dog.frame) * 5 : 2;
    ctx.fillStyle = '#6f3b1b';
    roundRect(28, 68, 14, 25 + lift, 7); ctx.fill();
    roundRect(72, 68, 14, 25 - lift, 7); ctx.fill();
    roundRect(45, 68, 12, 23 - lift, 7); ctx.fill();
    roundRect(88, 68, 12, 23 + lift, 7); ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(17, 45);
    ctx.quadraticCurveTo(-14, 19, -22, 33);
    ctx.stroke();
  ...
  function drawObstacle(o) {
    drawPyramid(o.x, o.y + o.h, o.w, o.h, '#e9bd68', '#b9782e', 'rgba(70,38,12,.42)');
    ctx.fillStyle = 'rgba(96,55,20,.30)';
    ctx.fillRect(o.x + 5, o.y + o.h - 8, o.w - 10, 5);
    if (o.type === 'double') {
      drawPyramid(o.x + o.w * .62, o.y + o.h, o.w * .7, o.h * .72, '#f0c977', '#c38537', 'rgba(70,38,12,.35)');
    }
  }

  function drawGameOver() {
    ctx.fillStyle = 'rgba(28, 15, 34, .74)';
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = '#fff7d6';
    ctx.textAlign = 'center';
    ctx.font = '900 54px Arial';
    ctx.fillText('Игра окончена', W / 2, 202);
    ctx.font = '800 27px Arial';
    ctx.fillText('Ваш счёт: ' + Math.floor(score) + '  •  Рекорд: ' + best, W / 2, 250);
    ctx.font = '700 20px Arial';
    ctx.fillText('Нажмите, чтобы начать заново', W / 2, 292);
    ctx.textAlign = 'left';
  }
  ...
  function draw() {
    ...
    ctx.fillStyle = 'rgba(52,31,14,.22)';
  ...
  function update(dt) {
    ...
    // Same smooth feeling as previous version, but with a little stronger visual movement.
    speed += dt * 0.00055;
    score += dt * 0.018;
    bgOffset += speed * dt * 0.025;
    farOffset += speed * dt * 0.050;
    nearOffset += speed * dt * 0.090;
    ...
    for (const o of obstacles) {
      if (collides(dog, o)) {
        ...
        localStorage.setItem('raffleBotEgyptBest', String(best));
        break;
      }
    }
  }
...
app.get('/health', (req, res) => {
  res.json({
    ok: true,
    play: true,
    version: 'play_egypt_pyramids_big_canvas_2026_06_10'
  });
});
