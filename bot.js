...
async function discoverUserChannelsForUser(userId) {
  ...
  for (const candidate of candidates) {
    ...
    if (!trustedByWebhook) {
      // Не проверяем каждый старый/чужой увиденный канал через MAX API.
      // Такие проверки часто дают 403 User is not admin / Not enough permissions
      // и сильно тормозят кнопку «Обновить каналы» при создании розыгрыша.
      // Канал добавляем пользователю только если MAX ранее прислал bot_added/bot_updated
      // именно от этого пользователя. Уже сохранённые user_channels всё равно показываются в меню.
...
async function refreshUserChannels(target, userId) {
  ...
  if (session && ['await_channel_selection', 'collab_channel_selection'].includes(session.state)) {
    ...
    // При создании/коллаборации не отправляем отдельное сообщение «Каналы обновлены»,
    // чтобы пользователь не видел два похожих блока подряд.
    // Сразу показываем актуальный список каналов для выбора.
