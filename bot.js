...
async function sendRafflePhotoStepPrompt(target, data = {}) {
  ...
  return sendMessage(
    ...
    buildCreateStepMessage(4, 'Фото', [
      ...
      '',
      hasPhoto
        ? 'Фото уже добавлено. Можно отправить другое фото, оставить текущее или продолжить без фото.'
        : 'Отправьте фото одним сообщением — оно будет прикреплено к посту розыгрыша.',
      '',
      `👆 Нету своего крутого фото❓ **${markdownLink('Наш ИИ', aiPhotoUrl)}** сделает его бесплатно за секунды.`,
      '',
      'Фото необязательно — этот шаг можно пропустить.'
    ]),
    keyboard
  );
}
