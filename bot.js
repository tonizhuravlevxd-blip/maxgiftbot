...
const MAX_USER_LINK_TEMPLATE = String(process.env.MAX_USER_LINK_TEMPLATE || '').trim();
...
function buildUserProfileLink(user) {
  ...
  if (userId && MAX_USER_LINK_TEMPLATE) {
    return MAX_USER_LINK_TEMPLATE.replace(/\{id\}/g, encodeURIComponent(userId));
  }
...
async function recordRaffleInterest(raffleId, userId) {
  await pool.query(`
...
async function getUserMainTicket(raffleId, userId) {
  const res = await pool.query(`
    SELECT *
    FROM raffle_participants
    WHERE raffle_id = $1
      AND user_id = $2
      AND COALESCE(ticket_type, 'main') = 'main'
    ORDER BY id ASC
    LIMIT 1
  `, [raffleId, userId]);
...
async function createParticipantEntry(raffleId, userId, invitedBy = null) {
  ...
  try {
    ...
    const cleanInvitedBy = invitedBy && Number(invitedBy) !== Number(userId)
      ? Number(invitedBy)
      : null;
    ...
    `, [raffleId, userId, ticketNumber, cleanInvitedBy]);
    ...
    if (cleanInvitedBy) {
      ...
      const refUserEntry = await client.query(`
        SELECT 1
        FROM raffle_user_entry
        WHERE raffle_id = $1
          AND user_id = $2
        LIMIT 1
      `, [raffleId, cleanInvitedBy]);
      ...
      const alreadyRewarded = await client.query(`
        ...
        LIMIT 1
      `, [raffleId, cleanInvitedBy, userId]);

      const bonusCount = await client.query(`
      ...
      const currentBonusCount = Number(bonusCount.rows[0]?.count || 0);

      if (refUser.rows.length && refUserEntry.rows.length && !alreadyRewarded.rows.length) {
        if (currentBonusCount < MAX_REFERRAL_BONUS_TICKETS) {
          bonusTicketNumber = randomTicketNumber();

          await client.query(`
            INSERT INTO raffle_participants (raffle_id, user_id, ticket_number, invited_by, is_valid, ticket_type)
            VALUES ($1, $2, $3, $4, true, 'referral_bonus')
          `, [raffleId, cleanInvitedBy, bonusTicketNumber, userId]);
        } else {
...
function buildJoinCheckCallbackData(raffleId, invitedBy = null) {
  const ref = invitedBy && Number(invitedBy) ? Number(invitedBy) : 0;
  return `verify_join:${raffleId}:${ref}`;
}

function buildRaffleSubscriptionInstructionText(raffle, channels = []) {
  const channelsText = buildRafflePublicChannelsText(channels) || 'Обязательные каналы не указаны.';
  ...
  return [
    ...
    channelsText,
    '',
    'После подписки нажмите **✅ Проверить подписку**. Если всё хорошо, бот создаст ваш билет участника.'
  ].join('\n');
}

async function sendJoinSubscriptionCheck(target, raffle, userId, invitedBy = null, missing = []) {
  await recordRaffleInterest(raffle.id, userId);

  const channels = await getRaffleChannels(raffle.id);
  const missingText = missing.length
    ? [
        '',
        'Сейчас бот ещё не видит подписку на:',
  ...
  return sendMessage(
    target,
    buildRaffleSubscriptionInstructionText(raffle, channels) + missingText,
    [[{ text: '✅ Проверить подписку', callback_data: buildJoinCheckCallbackData(raffle.id, invitedBy) }]]
  );
}
...
function getMinPublishAt() {
  // Минимальная задержка публикации отключена: пользователь может поставить публикацию на ближайшее будущее.
  // Быстрая кнопка «через 30 минут» остаётся как удобный вариант, но больше не является обязательным минимумом.
  return dayjs.utc();
}
...
async function sendPublishDatePrompt(target, data = {}) {
  ...
  return sendMessage(
    ...
    [
      ...
      'Обязательная задержка 30 минут отключена: можно выбрать любое будущее время.',
      `Ближайшее допустимое время: **${formatDateTime(minPublishAt)}**.`,
      '',
      'Быстрая кнопка «через 30 минут» оставлена для удобства. Также можно ввести дату вручную по МСК:',
      '`2026-06-10 20:00`',
      '`10.06.2026 20:00`'
    ].join('\n'),
...
function buildCreatedRaffleKeyboard(raffle, userId = null) {
  ...
  // Кнопка нужна после создания: если организатор не хочет ждать запланированное время,
  // он может сразу опубликовать розыгрыш в уже подключённые каналы.
  // Если соадмины добавят свои каналы позже, активный розыгрыш будет опубликован у них сразу
  // через существующую логику addCollabChannelsFromSession().
  const canShowStartNow = raffle && raffle.status === 'scheduled' && (
    !userId ||
...
function buildRaffleResultsText(raffle, winnersRows = []) {
  const lines = [
  ...
  if (!winnersRows.length) {
    lines.push('Победители не выбраны: участников не было.');
  } else {
...
async function publishRaffleResults(raffle, winnersRows = []) {
  const resultsText = buildRaffleResultsText(raffle, winnersRows);

  await sendMessage(raffle.creator_user_id, resultsText).catch(error => {
...
async function sendSubscriptionReminderToUser(raffle, user, missing, finalNotice = false) {
  const text = finalNotice
    ? [
        ...
        '',
        'Вы открывали участие в розыгрыше, но перед выбором победителей бот не подтвердил подписку на все обязательные каналы.',
        'Поэтому билет не был создан или не участвовал в финальном выборе.',
        '',
    ...
    : [
        ...
        '',
        'Вы открывали участие в розыгрыше, но сейчас бот не видит подписку на все обязательные каналы.',
        'Подпишитесь до окончания розыгрыша и нажмите кнопку проверки в боте, чтобы получить билет.',
        '',
  ...
  await sendMessage(
    user.user_id,
    text,
    [[{ text: '✅ Проверить подписку', callback_data: buildJoinCheckCallbackData(raffle.id, null) }]]
  ).catch(error => {
...
async function handleSessionMessage(message) {
  ...
  if (state === 'await_publish_date' || state === 'edit_publish_date') {
    ...
    if (parsed.isBefore(minPublishAt)) {
      await sendMessage(
        target,
        `Время публикации уже прошло. Ближайшее допустимое время: **${formatDateTime(minPublishAt)}**.`
      );
      return true;
    }
...
async function joinRaffle(target, userId, raffleId, invitedBy = null) {
  ...
  if (raffle.end_at && dayjs.utc(raffle.end_at).isBefore(dayjs.utc())) {
    return sendMessage(target, 'Время участия в этом розыгрыше уже закончилось.');
  }

  const existingTicket = await getUserMainTicket(raffle.id, userId);
  if (existingTicket) {
    ...
    return sendMessage(
      target,
      [
        `✅ Вы уже участвуете в розыгрыше #${raffle.id}.`,
        `Ваш билет: **№${existingTicket.ticket_number}**`,
        '',
        '🔗 Ваша пригласительная ссылка:',
        refLink,
        '',
        `За каждого приглашённого участника бот даст вам +1 дополнительный билет. Максимум: **${MAX_REFERRAL_BONUS_TICKETS}** бонусных билетов.`
      ].join('\n'),
  ...
  if (!sub.ok) {
    return sendJoinSubscriptionCheck(target, raffle, userId, invitedBy, sub.missing);
  }
  ...
  if (entry.alreadyJoined) {
    ...
    return sendMessage(
      ...
      [
        `✅ Вы уже участвуете в розыгрыше #${raffle.id}.`,
        entry.ticketNumber ? `Ваш билет: **№${entry.ticketNumber}**` : '',
        '',
        '🔗 Ваша пригласительная ссылка:',
        refLink
      ].filter(Boolean).join('\n'),
  ...
  const lines = [
    ...
    refLink,
    '',
    `Пригласите друга по этой ссылке. Если он тоже подпишется на каналы и получит билет, вы получите **+1 дополнительный билет**.`,
    `Максимум бонусных билетов за приглашения: **${MAX_REFERRAL_BONUS_TICKETS}**.`
  ];

  if (entry.bonusTicketNumber && entry.invitedBy) {
    lines.push('');
    lines.push('✅ Пригласившему вас участнику начислен +1 бонусный билет.');

    sendMessage(
      entry.invitedBy,
      [
        `🎁 По вашей ссылке зарегистрировался новый участник розыгрыша #${raffle.id}.`,
        `Вам начислен дополнительный билет: **№${entry.bonusTicketNumber}**`,
        '',
        `Лимит бонусных билетов: **${MAX_REFERRAL_BONUS_TICKETS}**.`
      ].join('\n')
    ).catch(error => {
      console.warn(`Не удалось уведомить пригласившего ${entry.invitedBy}:`, error.message);
    });
  }
...
async function handleCallbackQuery(cb) {
  ...
  if (data.startsWith('verify_join:')) {
    ...
    const ref = parts[2] && Number(parts[2]) ? Number(parts[2]) : null;
    ...
    await joinRaffle(target, userId, raffleId, ref);
    return;
  }
