...
<body>
  <main class="wrap">
    <section class="card">
      ...
      <canvas id="game" width="820" height="520" aria-label="Игра: мультяшная собака перепрыгивает препятствия"></canvas>
...
(function () {
  ...
  const groundY = 405;
  const dog = { x: 112, y: groundY - 92, w: 128, h: 92, vy: 0, onGround: true, frame: 0 };
  const gravity = 0.88;
  const jumpPower = -18.4;
  let obstacles = [];
  let speed = 6;
  let nextObstacle = 0;
  let score = 0;
  ...
  function addObstacle() {
    ...
    const h = scene === 'festival' ? 30 + Math.random() * 42 : 34 + Math.random() * 42;
    const w = scene === 'festival' ? 26 + Math.random() * 28 : 25 + Math.random() * 18;
    obstacles.push({ x: canvas.width + 20, y: groundY - h, w, h, scene });
    nextObstacle = 64 + Math.random() * 72;
  }
  ...
  function drawDog() {
    ...
    const run = dog.frame;
    const lift1 = Math.sin(run) * 10;
    const lift2 = Math.sin(run + Math.PI) * 10;
    ...
    ctx.beginPath(); ctx.ellipse(67, 93, 82, 12, 0, 0, Math.PI * 2); ctx.fill();
    ...
    ctx.fillStyle = '#9a5524';
    roundRect(91, 8, 32, 36, 16); ctx.fill();
    roundRect(113, 17, 31, 22, 11); ctx.fill();
    ctx.fillStyle = '#723916';
    roundRect(92, 2, 14, 34, 9); ctx.fill();
    roundRect(104, 4, 14, 36, 9); ctx.fill();
    ctx.fillStyle = '#05070a'; ctx.beginPath(); ctx.arc(145, 27, 5.4, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(121, 20, 6.3, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#111827'; ctx.beginPath(); ctx.arc(123, 21, 2.8, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = '#111827'; ctx.lineWidth = 2.4; ctx.beginPath(); ctx.moveTo(130, 33); ctx.quadraticCurveTo(136, 40, 143, 32); ctx.stroke();
    ...
    ctx.beginPath(); ctx.ellipse(111, 27, 6, 8, 0, 0, Math.PI * 2); ctx.fill();
  ...
  function draw() {
    ...
    if (gameOver) {
      ...
      ctx.fillText('Игра окончена', canvas.width / 2, 208);
      ctx.font = '700 24px Arial';
      ctx.fillText('Ваш счёт: ' + Math.floor(score) + '  •  Рекорд: ' + best, canvas.width / 2, 252);
      ctx.font = '600 18px Arial';
      ctx.fillText('Нажмите, чтобы начать заново', canvas.width / 2, 294);
      ctx.textAlign = 'left';
    }
  }
  ...
  function update(dt) {
    ...
    nextObstacle -= dt * 0.06;
    if (nextObstacle <= 0) addObstacle();

    for (const o of obstacles) o.x -= speed * dt * 0.06;
