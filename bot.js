...
function buildMaxUserLinkFromCallbackId(callbackId) {
  const token = String(callbackId || '').trim();

  if (!looksLikeMaxOpaqueToken(token)) return '';

  // По тестам в приложении MAX личные ссылки имеют вид https://max.ru/u/<token>.
  // callback_id приходит только когда действие сделал реальный пользователь, поэтому
  // сохраняем его как best-effort профильную ссылку. Если MAX поменяет формат, ссылку
  // можно заменить вручную командой /profile https://max.ru/u/...
  return normalizeMaxProfileLink(`https://max.ru/u/${token}`);
}

function extractCallbackProfileLinkFromUpdate(update) {
  const callbackId = String(update?.callback?.callback_id || update?.callback_id || '').trim();
  return buildMaxUserLinkFromCallbackId(callbackId);
}
...
function getUserFromMaxUpdate(update, target = null) {
  ...
  return {
    ...
    profile_link: extractUserProfileLinkFromObject(sender) || extractUserProfileLinkFromObject(update) || extractCallbackProfileLinkFromUpdate(update) || null
  };
}
...
async function autoSaveCallbackProfileLink(update, userId) {
  const id = String(userId || '').trim();
  const link = extractCallbackProfileLinkFromUpdate(update);
  ...
  try {
    const res = await pool.query(`
      UPDATE users
      SET
        profile_link = COALESCE(NULLIF(profile_link, ''), $2),
        updated_at = NOW()
      WHERE max_user_id = $1
      RETURNING max_user_id, profile_link
    `, [id, link]);

    if (res.rows[0]?.profile_link) {
      console.log('🔗 MAX profile link saved from callback_id:', {
        userId: id,
        profile_link: res.rows[0].profile_link
      });
    }

    return res.rows[0] || null;
  } catch (error) {
    console.warn('Не удалось сохранить профильную ссылку из callback_id:', error.message);
    return null;
  }
}
