...
function buildProfileLinkInstruction() {
  return [
    ...
    '',
    'Ссылку можно скопировать в приложении MAX из профиля. Без такой ссылки бот покажет имя и ID без клика, потому что numeric ID не превращается в рабочую ссылку.'
  ].join('\n');
}
