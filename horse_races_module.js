...
function setupHorseRacesModule(options = {}) {
  ...
  function buildRaceCollabInviteLink(token) {
    ...
    return `${String(APP_BASE_URL || '').replace(/\/+$/, '')}/horse-collab/${encodeURIComponent(cleanToken)}`;
  }
  ...
  function registerRoutes() {
    ...
    app.get('/horse-collab/:token', async (req, res) => {
      ...
      if (!isRaceCollabInviteActive(invite)) {
        res.status(410).type('html; charset=utf-8').send(renderHorseCollabLandingPage({
          title: 'Ссылка закрыта',
          heading: '🔒 Приглашение закрыто',
          description: 'Скачки уже начались, завершились или организатор закрыл приглашение.',
      ...
      res.type('html; charset=utf-8').send(renderHorseCollabLandingPage({
        title: 'Добавить канал в скачки',
        heading: '🤝 Совместные скачки',
        description: 'Откройте бота, выберите свой канал и режим: с размещением поста или без размещения. Подписка на канал будет обязательной.',
        buttonText: 'Открыть бота и выбрать канал',
        buttonUrl: getBotUrl(`horse_collab_${invite.token}`)
      }));
    });
