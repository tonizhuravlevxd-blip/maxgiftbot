...
function setupHorseRacesModule(options = {}) {
  ...
  const config = {
    ...
    audienceOnlineWindowSeconds: Math.min(
      300,
      Math.max(30, Number(process.env.MINIAPP_ONLINE_WINDOW_SECONDS || 45))
    ),
    audienceHeartbeatSeconds: Math.min(
      60,
      Math.max(10, Number(process.env.MINIAPP_HEARTBEAT_SECONDS || 20))
    ),
  ...
  async function initDb() {
    await pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS miniapp_audience_presence (
      ...
      CREATE INDEX IF NOT EXISTS ix_miniapp_audience_presence_last_seen
        ON miniapp_audience_presence (last_seen_at DESC);

      CREATE INDEX IF NOT EXISTS ix_miniapp_audience_presence_month
        ON miniapp_audience_presence (month_key);

      DELETE FROM miniapp_audience_presence
      WHERE first_seen_at < NOW() - INTERVAL '13 months';
  ...
  async function sendHorseRaceStats(target, raceId, requesterUserId) {
    ...
    const text = [
      ...
      Number(posts.failed_posts || 0)
        ...
        : '',
      '',
      '🎨 **Распределение по лошадям:**',
      ...(colorRows.length
        ? colorRows.map(row => {
            const color = serializeHorseColor(row.horse_color);
            return `• ${color?.label || row.horse_color}: **${Number(row.participants_count || 0)} участников / ${Number(row.tickets_count || 0)} билетов**`;
          })
        : ['• Пока нет участников']),
      '',
      '📢 **Подключённые каналы:**',
...
lines.push(
  ...
  async function touchMiniAppAudience(userId) {
    const cleanUserId = String(userId || '').trim();

    if (!/^\d+$/.test(cleanUserId)) {
      throw new Error('Некорректный ID пользователя Mini App');
    }

    const monthKeyResult = await pool.query(`
      SELECT to_char(
    ...
    const monthKey = String(
      monthKeyResult.rows[0]?.month_key || ''
    );

    await pool.query(`
      INSERT INTO miniapp_audience_presence (
        month_key,
        user_id,
        first_seen_at,
        last_seen_at
    ...
    const counts = await pool.query(`
      ...
      FROM miniapp_audience_presence
    `, [config.audienceOnlineWindowSeconds, monthKey]);

    return {
      online: Number(counts.rows[0]?.online_count || 0),
      monthly: Number(counts.rows[0]?.monthly_count || 0),
      heartbeatSeconds: config.audienceHeartbeatSeconds,
      onlineWindowSeconds: config.audienceOnlineWindowSeconds
    };
  }
  ...
  function registerRoutes() {
    ...
    app.post('/api/miniapp/audience', ...api, async (req, res) => {
      try {
        const audience = await touchMiniAppAudience(
          req.maxAuth.user.id
        );
        ...
        console.error(
          'POST /api/miniapp/audience error:',
          error.message
        );

        return res.status(500).json({
          ok: false,
          error: 'Не удалось обновить счётчик посетителей.'
        });
      }
    });
...
<head>
  ...
  <style nonce="${nonce}">
    ...
    *{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 20% 0,#1c3154 0,#09111f 42%,#050a12 100%);color:var(--text);font-family:Inter,system-ui,-apple-system,Segoe UI,Arial,sans-serif;min-height:100vh}
    button,a{font:inherit}.app{max-width:760px;margin:0 auto;padding:18px 14px 34px}.header{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;gap:10px;margin-bottom:14px}.brand{font-weight:900;letter-spacing:.5px;justify-self:start}.user{color:var(--muted);font-size:13px;text-align:right;justify-self:end;max-width:190px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.audience{justify-self:center;display:flex;flex-direction:column;align-items:center;gap:2px;padding:5px 9px;border:1px solid var(--line);border-radius:12px;background:rgba(17,28,46,.78);line-height:1.05;white-space:nowrap}.audience-online{font-size:11px;font-weight:900;color:#86efac}.audience-month{font-size:10px;color:var(--muted)}
    ...
    @media(max-width:520px){.header{gap:6px}.brand{font-size:12px}.user{font-size:10px;max-width:92px}.audience{padding:4px 6px}.audience-online{font-size:10px}.audience-month{font-size:9px}.card{grid-template-columns:82px 1fr}.image,.placeholder{width:82px;height:82px}.app{padding-left:10px;padding-right:10px}.horse{width:42px}.lane{height:54px}}
  </style>
</head>
<body>
<div class="app">
  <div class="header">
    <div class="brand">🍀 РОЗЫГРЫШ ТОП</div>
    <div class="audience" aria-label="Посетители Mini App">
      <div id="online-count" class="audience-online">— онлайн</div>
      <div id="month-count" class="audience-month">—/месяц 👁️</div>
    </div>
    <div id="user" class="user">MAX Mini App</div>
  </div>
...
(() => {
  ...
  const state={tab:'raffles',timers:[],serverOffset:0,currentRaceId:null,liveTimer:null,animationFrame:null,audio:null,soundEnabled:false,playedRounds:new Set(),replayMode:false,replayToken:0,lastLive:null,audienceTimer:null,audienceHeartbeatSeconds:20};
  ...
  function apiPost(path,body={}){if(!initData)return Promise.reject(new Error('Откройте Mini App внутри MAX'));return fetch(path,{method:'POST',headers:{'X-Max-Init-Data':initData,'Accept':'application/json','Content-Type':'application/json'},body:JSON.stringify(body),cache:'no-store'}).then(async r=>{const j=await r.json().catch(()=>({}));if(!r.ok||!j.ok)throw new Error(j.error||'Ошибка сервера');return j})}
  ...
  function compactAudienceCount(value){const n=Math.max(0,Number(value||0));if(n>=1000000){const v=Math.round(n/100000)/10;return String(v).replace('.',',')+'м'}if(n>=1000){const v=Math.round(n/100)/10;return String(v).replace('.',',')+'к'}return String(Math.floor(n))}
  function renderAudience(audience){const online=$('#online-count'),monthly=$('#month-count');if(online)online.textContent=compactAudienceCount(audience?.online)+' онлайн';if(monthly)monthly.textContent=compactAudienceCount(audience?.monthly)+'/месяц 👁️'}
  async function refreshAudience(){if(document.visibilityState!=='visible')return;try{const j=await apiPost('/api/miniapp/audience');syncServerTime(j.serverNow);renderAudience(j.audience);const seconds=Math.max(10,Number(j.audience?.heartbeatSeconds||20));if(seconds!==state.audienceHeartbeatSeconds){state.audienceHeartbeatSeconds=seconds;startAudienceTimer()}}catch(error){console.warn('Mini App audience update failed:',error.message)}}
  function startAudienceTimer(){if(state.audienceTimer)clearInterval(state.audienceTimer);state.audienceTimer=setInterval(refreshAudience,state.audienceHeartbeatSeconds*1000)}
  function startAudienceTracking(){refreshAudience();startAudienceTimer();document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')refreshAudience()})}
  ...
  loadMe().then(()=>{startAudienceTracking();const m=startParam.match(/^race_(\d+)$/);if(m){state.tab='races';$$('.tab').forEach(b=>b.classList.toggle('active',b.dataset.tab==='races'));openRace(Number(m[1]))}else if(startParam==='races'){setTab('races')}else loadRaffles()});
})();
</script>
</body>
</html>`;
