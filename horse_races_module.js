...
function setupHorseRacesModule(options = {}) {
  ...
  const launchBaseUrl = normalizeBaseUrl(
    MAX_MINIAPP_LAUNCH_URL || process.env.MAX_MINIAPP_LAUNCH_URL || BOT_PUBLIC_URL || webUrl
  );
  ...
  function getLaunchUrl(startParam = 'home') {
    ...
    try {
      const url = new URL(launchBaseUrl || webUrl);
      if (cleanParam) url.searchParams.set('startapp', cleanParam);
      return url.toString();
    } catch {
      const separator = String(launchBaseUrl || webUrl).includes('?') ? '&' : '?';
      return cleanParam
        ? `${launchBaseUrl || webUrl}${separator}startapp=${encodeURIComponent(cleanParam)}`
        : (launchBaseUrl || webUrl);
    }
  }
  ...
  async function getAvailableRaces(userId) {
    const result = await pool.query(`
      ...
      ORDER BY
        CASE WHEN hr.status = 'running' THEN 0 ELSE 1 END,
        hr.start_at ASC,
        hr.id DESC
      LIMIT $2
    `, [String(userId), config.racesLimit]);
  ...
  async function createRaceDraft(input = {}) {
    ...
    if (Number.isNaN(startAt.getTime()) || startAt.getTime() <= Date.now() + 60_000) {
      throw new Error('Время старта скачек должно быть минимум через 1 минуту');
    }
  ...
  async function finalizeRaceWinners(raceId) {
    ...
    try {
      ...
      const remaining = secureShuffle(participantsRes.rows);

      for (const round of roundsRes.rows) {
        if (round.winner_participant_id || round.result_json?.winner_selected === true) continue;
  ...
  function startWorker() {
    ...
    const run = () => {
      refreshRaceLifecycle().catch(error => {
...
(() => {
  ...
  const state={tab:'raffles',timers:[],serverOffset:0,currentRaceId:null,liveTimer:null,animationFrame:null,audio:null,soundEnabled:false,playedRounds:new Set()};
  ...
  async function loadRaces(){const box=$('#races');box.innerHTML='<div class="loading">Загрузка скачек…</div>';try{const j=await api('/api/miniapp/races');syncServerTime(j.serverNow);box.innerHTML=j.items.length?j.items.map(r=>'<article class="card">'+imageHtml(r.photoUrl,'🐎')+'<div><h3>'+escapeHtml(r.title)+'</h3><div class="meta"><span class="badge">№'+r.id+'</span><span class="badge">👥 '+r.participantsCount+'</span><span class="badge">🏆 '+r.prizeCount+'</span>'+(r.ownHorseColor?'<span class="badge">Ваша: '+escapeHtml(r.ownHorseColor.label)+'</span>':'')+'</div><div class="countdown">Старт: '+escapeHtml(fmtDate(r.startsAt))+' · <span data-countdown="'+escapeHtml(r.startsAt)+'"></span></div><div class="actions"><button class="small alt" data-join="'+escapeHtml(r.joinUrl)+'">🐎 Участвовать</button><button class="small" data-race="'+r.id+'" '+(r.canOpen?'':'disabled')+'>'+(r.canOpen?'Смотреть скачки':'Откроется за 12 часов')+'</button></div></div></article>').join(''):'<div class="empty">Доступных скачек пока нет.</div>';box.querySelectorAll('[data-join]').forEach(b=>b.onclick=()=>openMax(b.dataset.join));box.querySelectorAll('[data-race]:not([disabled])').forEach(b=>b.onclick=()=>openRace(Number(b.dataset.race)));attachCountdowns()}catch(e){box.innerHTML='<div class="error">'+escapeHtml(e.message)+'</div>'}}
  ...
  async function openRace(id){state.currentRaceId=id;$('#raffles-section').classList.add('hidden');$('#races-section').classList.add('hidden');const view=$('#race-view');view.classList.remove('hidden');view.innerHTML='<div class="loading">Открываем ипподром…</div>';try{const j=await api('/api/miniapp/races/'+id);syncServerTime(j.serverNow);const r=j.race;if(!r.canOpen){view.innerHTML='<button class="back">← Назад</button><div class="empty">Ипподром откроется '+escapeHtml(fmtDate(r.openAt))+'.</div>';view.querySelector('.back').onclick=()=>setTab('races');return}view.innerHTML='<button class="back">← К списку</button><div class="race-head"><h2>🐎 '+escapeHtml(r.title)+'</h2><div>'+escapeHtml(r.description||'Смотрите забег вживую.')+'</div><div class="countdown">Старт: '+escapeHtml(fmtDate(r.startsAt))+' · <span data-countdown="'+escapeHtml(r.startsAt)+'"></span></div>'+(r.ownHorseColor?'<div class="own">🎟 Ваш билет: <b>№'+escapeHtml(r.ownTicketNumber)+'</b><br>🐎 Ваша лошадь: <b>'+escapeHtml(r.ownHorseColor.label)+'</b></div>':'<div class="actions"><button class="small alt" data-join="'+escapeHtml(r.joinUrl)+'">🐎 Получить билет</button></div>')+'<button id="sound" class="secondary sound">🔊 Включить звук</button></div><div class="track-wrap"><div class="crowd"></div><div id="lanes"></div><div id="race-status" class="race-status">Подготовка…</div></div><div id="rounds" class="rounds"></div>';view.querySelector('.back').onclick=()=>setTab('races');view.querySelector('[data-join]')?.addEventListener('click',e=>openMax(e.currentTarget.dataset.join));$('#sound').onclick=enableSound;renderLanes();attachCountdowns();await pollLive();state.liveTimer=setInterval(pollLive,1000)}catch(e){view.innerHTML='<button class="back">← Назад</button><div class="error">'+escapeHtml(e.message)+'</div>';view.querySelector('.back').onclick=()=>setTab('races')}}
  function renderLanes(){const lanes=$('#lanes');if(!lanes)return;lanes.innerHTML=COLORS.map((c,i)=>'<div class="lane"><div class="horse" id="horse-'+c.code+'" style="color:'+c.css+';background:'+c.css+'33;border:2px solid '+c.css+'">🐎<span style="color:'+c.css+'">'+(i+1)+'</span></div><div class="lane-label">'+escapeHtml(c.label)+'</div></div>').join('')}
  async function pollLive(){if(!state.currentRaceId)return;try{const j=await api('/api/miniapp/races/'+state.currentRaceId+'/live');const l=j.live;syncServerTime(l.serverNow);if(l.locked){$('#race-status').textContent='Ипподром откроется '+fmtDate(l.opensAt);return}renderRounds(l.rounds,l.currentRound);animateRound(l.currentRound)}catch(e){const s=$('#race-status');if(s)s.textContent=e.message}}
  ...
  loadMe().then(()=>{const m=startParam.match(/^race_(\d+)$/);if(m){state.tab='races';$$('.tab').forEach(b=>b.classList.toggle('active',b.dataset.tab==='races'));openRace(Number(m[1]))}else loadRaffles()});
})();
</script>
</body>
</html>`;
